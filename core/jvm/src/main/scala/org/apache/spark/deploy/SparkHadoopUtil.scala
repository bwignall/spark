...
private[spark] class SparkHadoopUtil extends Logging {
  ...
  def loginUserFromKeytab(
    ...
    if (!new File(keytabFilename).exists()) {
      ...
      logInfo(
        log"Attempting to login to Kerberos using principal: " +
  ...
  private[spark] def getFSBytesReadOnThreadCallback(): () => Long = {
    val f = () =>
      FileSystem.getAllStatistics.asScala
        .map(_.getThreadStatistics.getBytesRead)
        .sum
  ...
  def listLeafStatuses(
    ...
    def recurse(status: FileStatus): Seq[FileStatus] = {
      ...
      (leaves ++ directories.flatMap(f =>
        listLeafStatuses(fs, f)
      )).toImmutableArraySeq
    }
  ...
  /** Convert a token to a string for logging.
    ...
    */
  private[spark] def tokenToString(
...
private[spark] object SparkHadoopUtil extends Logging {
  ...
  /** Source for configuration options with `spark.hive.` prefix copied
    * from spark-defaults.
    */
  private[deploy] val SOURCE_SPARK_HIVE =
  ...
  private[deploy] def appendS3CredentialsFromEnvironment(
    ...
    if (keyId != null && accessKey != null) {
      ...
      val source =
        SOURCE_SPARK + " on " + InetAddress.getLocalHost.toString + " from "
      hadoopConf.set(
        "fs.s3.awsAccessKeyId",
        keyId,
        source + ENV_VAR_AWS_ACCESS_KEY
      )
      hadoopConf.set(
      ...
      if (endpointUrl != null) {
        hadoopConf.set(
          "fs.s3a.endpoint",
          endpointUrl,
          source + ENV_VAR_AWS_ENDPOINT_URL
        )
      }
  ...
  private def appendSparkHadoopConfigs(
      conf: SparkConf,
      hadoopConf: Configuration
  ): Unit = {
    ...
    if (
      conf
        .getOption("spark.hadoop.fs.s3a.downgrade.syncable.exceptions")
        .isEmpty
    ) {
  ...
  private def appendSparkHiveConfigs(
      conf: SparkConf,
      hadoopConf: Configuration
  ): Unit = {
