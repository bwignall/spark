...
/** Represents an explanation for an executor or whole process failing or exiting.
  */
private[spark] class ExecutorLossReason(val message: String)
...
private[spark] object ExecutorKilled
    extends ExecutorLossReason("Executor killed by driver.")
...
/** A loss reason that means we don't yet know why the executor exited.
  ...
  * is known.
  */
private[spark] object LossReasonPending
    extends ExecutorLossReason("Pending loss reason.")
...
/** @param _message human readable loss reason
  ...
  */
private[spark] case class ExecutorProcessLost(
