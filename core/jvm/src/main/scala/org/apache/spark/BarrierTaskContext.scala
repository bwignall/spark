...
class BarrierTaskContext private[spark] (taskContext: TaskContext)
  ...
  def allGather(message: String): Array[String] =
    runBarrier(message, RequestMethod.ALL_GATHER)
  ...
  override private[spark] def setFetchFailed(
  ...
  override private[spark] def markTaskCompleted(
