...
private[spark] object ResourceAmountUtils {
  ...
  def toInternalResource(amount: Double): Long =
    (amount * ONE_ENTIRE_RESOURCE).toLong

  def toFractionalResource(amount: Long): Double =
    amount.toDouble / ONE_ENTIRE_RESOURCE
...
private[spark] trait ResourceAllocator {
  ...
  private lazy val addressAvailabilityMap = {
    mutable.HashMap(
      resourceAddresses.map(address => address -> ONE_ENTIRE_RESOURCE): _*
    )
  }
  ...
  def availableAddrs: Seq[String] = addressAvailabilityMap
    .filter(addresses => addresses._2 > 0)
    .keys
    .toSeq
    .sorted
  ...
  private[spark] def assignedAddrs: Seq[String] = addressAvailabilityMap
    .filter(addresses => addresses._2 < ONE_ENTIRE_RESOURCE)
    .keys
    .toSeq
    .sorted
  ...
  def acquire(addressesAmounts: Map[String, Long]): Unit = {
    addressesAmounts.foreach { case (address, amount) =>
      val prevAmount = addressAvailabilityMap.getOrElse(
        address,
        throw new SparkException(
          s"Try to acquire an address that doesn't exist. $resourceName " +
      ...
      if (left < 0) {
        throw new SparkException(
          s"Try to acquire $resourceName address $address " +
  ...
  def release(addressesAmounts: Map[String, Long]): Unit = {
    addressesAmounts.foreach { case (address, amount) =>
      val prevAmount = addressAvailabilityMap.getOrElse(
        address,
        throw new SparkException(
          s"Try to release an address that doesn't exist. $resourceName " +
            s"address $address doesn't exist."
        )
      )
      ...
      if (total > ONE_ENTIRE_RESOURCE) {
        throw new SparkException(
          s"Try to release $resourceName address $address " +
