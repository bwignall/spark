...
private[spark] trait ExecutorAllocationClient {
  ...
  def decommissionExecutors(
    ...
    killExecutors(
      executorsAndDecomInfo.map(_._1).toImmutableArraySeq,
  ...
  final def decommissionExecutor(
      ...
      adjustTargetNumExecutors: Boolean,
      triggeredByExecutor: Boolean = false
  ): Boolean = {
    ...
    decommissionedExecutors.nonEmpty && decommissionedExecutors(0).equals(
      executorId
    )
  }
  ...
  def killExecutor(executorId: String): Boolean = {
    val killedExecutors = killExecutors(
      Seq(executorId),
