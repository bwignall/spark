...
import org.apache.spark.internal.config.{
  BUFFER_SIZE,
  CACHE_CHECKPOINT_PREFERRED_LOCS_EXPIRE_TIME,
  CHECKPOINT_COMPRESS
}
...
private[spark] class ReliableCheckpointRDD[T: ClassTag](
  ...
  @transient private[spark] lazy val cachedPreferredLocations = CacheBuilder
    .newBuilder()
    .expireAfterWrite(
      SparkEnv.get.conf.get(CACHE_CHECKPOINT_PREFERRED_LOCS_EXPIRE_TIME).get,
      TimeUnit.MINUTES
    )
    .build(new CacheLoader[Partition, Seq[String]]() {
...
private[spark] object ReliableCheckpointRDD extends Logging {
  ...
  def writeRDDToCheckpointDirectory[T: ClassTag](
    ...
    logInfo(
      log"Checkpointing took ${MDC(TOTAL_TIME, checkpointDurationMs)} ms."
    )
  ...
  def writePartitionToCheckpointFile[T: ClassTag](
      ...
      blockSize: Int = -1
  )(ctx: TaskContext, iterator: Iterator[T]): Unit = {
    ...
    val tempOutputPath =
      new Path(outputDir, s".$finalOutputName-attempt-${ctx.taskAttemptId()}")
    ...
    Utils.tryWithSafeFinallyAndFailureCallbacks {
      ...
      catchBlock = {
        ...
        if (!deleted) {
          logInfo(
            log"Failed to delete tempOutputPath ${MDC(TEMP_OUTPUT_PATH, tempOutputPath)}."
          )
        }
      },
      finallyBlock = {
    ...
    if (!fs.rename(tempOutputPath, finalOutputPath)) {
      if (!fs.exists(finalOutputPath)) {
        logInfo(
          log"Deleting tempOutputPath ${MDC(TEMP_OUTPUT_PATH, tempOutputPath)}"
        )
        fs.delete(tempOutputPath, false)
        throw SparkCoreErrors.checkpointFailedToSaveError(
          ctx.attemptNumber(),
          finalOutputPath
        )
      } else {
  ...
  private def writePartitionerToCheckpointDir(
    ...
    try {
      ...
      case NonFatal(e) =>
        logWarning(
          log"Error writing partitioner ${MDC(PARTITIONER, partitioner)} to " +
  ...
  private def readCheckpointedPartitionerFile(
    ...
    try {
      ...
      case NonFatal(e) =>
        logWarning(
          log"Error reading partitioner from ${MDC(PATH, checkpointDirPath)}, " +
            log"partitioner will not be recovered which may lead to performance loss",
          e
        )
        None
    }
