...
private[scheduler] class HealthTracker(
  ...
  /** A map from executorId to information on task failures. Tracks the time of each task failure,
    ...
    */
  private val executorIdToFailureList =
    new HashMap[String, ExecutorFailureList]()
  ...
  def applyExcludeOnFailureTimeout(): Unit = {
    ...
    if (now > nextExpiryTime) {
      ...
      val execsToInclude =
        executorIdToExcludedStatus.filter(_._2.expiryTime < now).keys
      if (execsToInclude.nonEmpty) {
        // Include any executors that have been excluded longer than the excludeOnFailure timeout.
        logInfo(
          log"Removing executors ${MDC(EXECUTOR_IDS, execsToInclude)} from " +
      ...
      if (nodesToInclude.nonEmpty) {
        ...
        logInfo(
          log"Removing nodes ${MDC(NODES, nodesToInclude)} from exclude list because the " +
  ...
  private def killExecutor(exec: String, msg: String): Unit = {
    ...
    allocationClient match {
      ...
      case None =>
        logInfo(
          log"Not attempting to kill excluded executor id ${MDC(EXECUTOR_ID, exec)}" +
    ...
    if (conf.get(config.EXCLUDE_ON_FAILURE_KILL_ENABLED)) {
      allocationClient match {
        case Some(a) =>
          if (EXCLUDE_ON_FAILURE_DECOMMISSION_ENABLED) {
            logInfo(
              log"Decommissioning all executors on excluded host ${MDC(HOST, node)} " +
                log"since ${MDC(CONFIG, config.EXCLUDE_ON_FAILURE_KILL_ENABLED.key)} " +
                log"is set."
            )
            if (!a.decommissionExecutorsOnHost(node)) {
              logError(
                log"Decommissioning executors on ${MDC(HOST, node)} failed."
              )
            }
          } else {
            logInfo(
              log"Killing all executors on excluded host ${MDC(HOST, node)} " +
                log"since ${MDC(CONFIG, config.EXCLUDE_ON_FAILURE_KILL_ENABLED.key)} is set."
            )
            if (!a.killExecutorsOnHost(node)) {
              logError(
                log"Killing executors on node ${MDC(HOST, node)} failed."
              )
            }
          }
        case None =>
    ...
    if (EXCLUDE_FETCH_FAILURE_ENABLED) {
      ...
      if (conf.get(config.SHUFFLE_SERVICE_ENABLED)) {
        if (!nodeIdToExcludedExpiryTime.contains(host)) {
          logInfo(
            log"excluding node ${MDC(HOST, host)} due to fetch failure of " +
        ...
        logInfo(
          log"Excluding executor ${MDC(EXECUTOR_ID, exec)} due to fetch failure"
        )

        executorIdToExcludedStatus.put(
          exec,
          ExcludedExecutor(host, expiryTimeForNewExcludes)
        )
        ...
        val excludedExecsOnNode =
          nodeToExcludedExecs.getOrElseUpdate(host, HashSet[String]())
  ...
  def updateExcludedForSuccessfulTaskSet(
      stageId: Int,
      stageAttemptId: Int,
      failuresByExec: HashMap[String, ExecutorFailuresInTaskSet]
  ): Unit = {
    ...
    failuresByExec.foreach { case (exec, failuresInTaskSet) =>
      ...
      appFailuresOnExecutor.addFailures(
        stageId,
        stageAttemptId,
        failuresInTaskSet
      )
      ...
      if (
        newTotal >= MAX_FAILURES_PER_EXEC && !executorIdToExcludedStatus
          .contains(exec)
      ) {
        logInfo(
          log"Excluding executor id: ${MDC(EXECUTOR_ID, exec)} because it has " +
            log"${MDC(TOTAL, newTotal)} task failures in successful task sets"
        )
        val node = failuresInTaskSet.node
        executorIdToExcludedStatus.put(
          exec,
          ExcludedExecutor(node, expiryTimeForNewExcludes)
        )
        // post both to keep backwards compatibility
        ...
        val excludedExecsOnNode =
          nodeToExcludedExecs.getOrElseUpdate(node, HashSet[String]())
        excludedExecsOnNode += exec
        ...
        if (
          ...
          logInfo(
            log"Excluding node ${MDC(HOST, node)} because it has " +
          ...
          // post both to keep backwards compatibility
          listenerBus.post(
            SparkListenerNodeBlacklisted(now, node, excludedExecsOnNode.size)
          )
          listenerBus.post(
            SparkListenerNodeExcluded(now, node, excludedExecsOnNode.size)
          )
          _excludedNodeList.set(nodeIdToExcludedExpiryTime.keySet.toSet)
  ...
  private[scheduler] final class ExecutorFailureList extends Logging {
    ...
    def addFailures(
        stage: Int,
        stageAttempt: Int,
        failuresInTaskSet: ExecutorFailuresInTaskSet
    ): Unit = {
      failuresInTaskSet.taskToFailureCountAndFailureTime.foreach {
        case (taskIdx, (_, failureTime)) =>
          ...
          failuresAndExpiryTimes += (
            (
              TaskId(stage, stageAttempt, taskIdx),
              expiryTime
            )
          )
  ...
  def isExcludeOnFailureEnabled(conf: SparkConf): Boolean = {
    conf.get(config.EXCLUDE_ON_FAILURE_ENABLED) match {
      ...
      case None =>
        ...
        conf.get(config.EXCLUDE_ON_FAILURE_LEGACY_TIMEOUT_CONF).exists {
          legacyTimeout =>
            if (legacyTimeout == 0) {
              logWarning(
                log"Turning off excludeOnFailure due to legacy configuration: " +
                  log"${MDC(CONFIG, legacyKey)} == 0"
              )
              false
            } else {
              logWarning(
                log"Turning on excludeOnFailure due to legacy configuration: " +
                  log"${MDC(CONFIG, legacyKey)} > 0"
              )
              true
            }
  ...
  def validateExcludeOnFailureConfs(conf: SparkConf): Unit = {
    ...
    if (timeout <= 0) {
      ...
      conf.get(config.EXCLUDE_ON_FAILURE_TIMEOUT_CONF) match {
        case Some(t) =>
          mustBePos(
            config.EXCLUDE_ON_FAILURE_TIMEOUT_CONF.key,
            timeout.toString
          )
        case None =>
          mustBePos(
            config.EXCLUDE_ON_FAILURE_LEGACY_TIMEOUT_CONF.key,
            timeout.toString
          )
      }
    }
    ...
    if (maxNodeAttempts >= maxTaskFailures) {
      throw new IllegalArgumentException(
        s"${config.MAX_TASK_ATTEMPTS_PER_NODE.key} " +
