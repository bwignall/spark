...
import com.esotericsoftware.kryo.{
  Kryo,
  KryoException,
  Serializer => KryoClassSerializer
}
import com.esotericsoftware.kryo.io.{Input => KryoInput, Output => KryoOutput}
import com.esotericsoftware.kryo.io.{
  UnsafeInput => KryoUnsafeInput,
...
class KryoSerializer(conf: SparkConf)
  ...
  def newKryo(): Kryo = {
    ...
    kryo.register(
      Utils.classForName("scala.collection.immutable.ArraySeq$ofRef")
    )
    kryo.register(Utils.classForName("scala.collection.immutable.$colon$colon"))
    kryo.register(
      Utils.classForName("scala.collection.immutable.Map$EmptyMap$")
    )
...
private[spark] class KryoSerializationStream(
  ...
  private[this] var output: KryoOutput =
    if (useUnsafe) new KryoUnsafeOutput(outStream)
    else new KryoOutput(outStream)
...
private[spark] class KryoDeserializationStream(
  ...
  final override def asKeyValueIterator: Iterator[(Any, Any)] =
...
private[spark] class KryoSerializerInstance(
  ...
  /** A re-used [[Kryo]] instance. Methods will borrow this instance by calling `borrowKryo()`, do
    ...
    * not synchronized.
    */
  @Nullable private[this] var cachedKryo: Kryo =
    if (usePool) null else borrowKryo()
  ...
  private lazy val input =
    if (useUnsafe) new KryoUnsafeInput() else new KryoInput()
  ...
  private lazy val loadableSparkClasses: Seq[Class[_]] = {
    Seq(
      ...
      try {
        ...
        case _: NoClassDefFoundError if Utils.isTesting =>
          None // See SPARK-23422.
      }
    }
  }
}
...
private[spark] class KryoInputObjectInputBridge(kryo: Kryo, input: KryoInput)
  ...
  override def readUTF(): String =
    input.readString() // readString in kryo does utf8
...
private[spark] class KryoOutputObjectOutputBridge(
  ...
  override def writeUTF(s: String): Unit =
    output.writeString(s) // writeString in kryo does UTF8
...
private class JavaIterableWrapperSerializer
  ...
  override def read(
      kryo: Kryo,
      in: KryoInput,
      clz: Class[java.lang.Iterable[_]]
...
private object JavaIterableWrapperSerializer extends Logging {
  ...
  private val underlyingMethodOpt = {
    try Some(wrapperClass.getDeclaredMethod("underlying"))
      ...
      case e: Exception =>
        logError(
          log"Failed to find the underlying field in ${MDC(CLASS_NAME, wrapperClass)}",
          e
        )
        None
    }
