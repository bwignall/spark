...
/** Authority that decides whether tasks can commit output to HDFS. Uses a "first committer wins"
  ...
  */
private[spark] class OutputCommitCoordinator(conf: SparkConf, isDriver: Boolean)
    extends Logging {
  ...
  def canCommit(
    ...
    val msg =
      AskPermissionToCommitOutput(stage, stageAttempt, partition, attemptNumber)
    coordinatorRef match {
      case Some(endpointRef) =>
        ThreadUtils.awaitResult(
          endpointRef.ask[Boolean](msg),
    ...
    synchronized {
      stageStates.get(stage) match {
        case Some(state) =>
          ...
          logInfo(
            log"Reusing state from previous attempt of stage ${MDC(LogKeys.STAGE_ID, stage)}"
          )
  ...
  private[scheduler] def taskCompleted(
    ...
    reason match {
      ...
      case _: TaskCommitDenied =>
        logInfo(
          log"Task was denied committing, stage: ${MDC(LogKeys.STAGE_ID, stage)}." +
  ...
  private[scheduler] def handleAskPermissionToCommit(
    ...
    stageStates.get(stage) match {
      case Some(state)
        ...
        logInfo(
          log"Commit denied for stage=${MDC(LogKeys.STAGE_ID, stage)}." +
      ...
      case Some(state) =>
        ...
        if (existing == null) {
          logDebug(
            s"Commit allowed for stage=$stage.$stageAttempt, partition=$partition, " +
          ...
          logDebug(
            s"Commit denied for stage=$stage.$stageAttempt, partition=$partition: " +
