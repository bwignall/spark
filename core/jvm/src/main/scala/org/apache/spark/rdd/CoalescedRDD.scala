...
private[spark] case class CoalescedRDDPartition(
  ...
  var parents: Seq[Partition] =
    parentsIndices.map(rdd.partitions(_)).toImmutableArraySeq
...
private[spark] class CoalescedRDD[T: ClassTag](
  ...
  if (partitionCoalescer.isDefined) {
    require(
      partitionCoalescer.get.isInstanceOf[Serializable],
  ...
  override def getPartitions: Array[Partition] = {
    ...
    pc.coalesce(maxPartitions, prev).zipWithIndex.map { case (pg, i) =>
...
private class DefaultPartitionCoalescer(val balanceSlack: Double = 0.10)
  ...
  def setupGroups(targetLen: Int, partitionLocs: PartitionLocations): Unit = {
    ...
    val numPartsToLookAt =
      math.min(expectedCoupons2, partitionLocs.partsWithLocs.length)
  ...
  def getPartitions: Array[PartitionGroup] =
    groupArr.filter(pg => pg.numPartitions > 0).toArray
