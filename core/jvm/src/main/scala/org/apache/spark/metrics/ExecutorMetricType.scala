...
sealed trait SingleValueExecutorMetricType extends ExecutorMetricType {
  ...
  override private[spark] def getMetricValues(
...
private[spark] abstract class MemoryManagerExecutorMetricType(
    f: MemoryManager => Long
) extends SingleValueExecutorMetricType {
  override private[spark] def getMetricValue(
      memoryManager: MemoryManager
  ): Long = {
    f(memoryManager)
  }
...
case object GarbageCollectionMetrics extends ExecutorMetricType with Logging {
  ...
  private lazy val oldGenerationGarbageCollector: Seq[String] = {
    SparkEnv.get.conf.get(
      config.EVENT_LOG_GC_METRICS_OLD_GENERATION_GARBAGE_COLLECTORS
    )
  }
...
case object OnHeapStorageMemory
    extends MemoryManagerExecutorMetricType(_.onHeapStorageMemoryUsed)
...
case object OnHeapUnifiedMemory
    extends MemoryManagerExecutorMetricType(
