...
private[spark] abstract class Task[T](
    ...
    serializedTaskMetrics: Array[Byte] = SparkEnv.get.closureSerializer
      .newInstance()
      .serialize(TaskMetrics.registered)
      .array(),
  ...
  @transient lazy val metrics: TaskMetrics =
    SparkEnv.get.closureSerializer
      .newInstance()
      .deserialize(ByteBuffer.wrap(serializedTaskMetrics))
    ...
    try {
      ...
      try {
        Utils.tryLogNonFatalError {
          ...
          blockManager.memoryStore.releaseUnrollMemoryForThisTask(
            MemoryMode.ON_HEAP
          )
          blockManager.memoryStore.releaseUnrollMemoryForThisTask(
            MemoryMode.OFF_HEAP
