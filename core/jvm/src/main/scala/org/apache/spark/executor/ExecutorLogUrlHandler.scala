...
private[spark] class ExecutorLogUrlHandler(logUrlPattern: Option[String])
  ...
  private def doApplyPattern(
    ...
    val allAttributeKeysExceptLogFiles =
      allAttributeKeys.filter(_ != "LOG_FILES")
    ...
    if (
      ...
      allPatterns
        .contains("FILE_NAME") && !allAttributeKeys.contains("LOG_FILES")
    ) {
      logFailToRenewLogUrls(
      ...
      val updatedUrl = allPatternsExceptFileName.foldLeft(urlPattern) {
        case (orig, patt) =>
