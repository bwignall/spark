...
private[spark] class SecurityManager(
  ...
  private var adminAclsGroups: Set[String] =
    sparkConf.get(ADMIN_ACLS_GROUPS).toSet
  ...
  def checkModifyPermissions(user: String): Boolean = {
    logDebug(
      "user=" + user + " aclsEnabled=" + aclsEnabled() + " modifyAcls=" +
        modifyAcls.mkString(",") + " modifyAclsGroups=" + modifyAclsGroups
          .mkString(",")
    )
  ...
  def initializeAuth(): Unit = {
    ...
    if (
      sparkConf.get(AUTH_SECRET_FILE_DRIVER).isDefined !=
      ...
      sparkConf.getOption(SparkLauncher.SPARK_MASTER).map {
        case SparkMasterRegex.KUBERNETES_REGEX(_) =>
          ...
          val base64Key = Base64.getEncoder
            .encodeToString(Files.readAllBytes(secretFile.toPath))
          require(
            !base64Key.isEmpty,
