...
case class FetchFailed(
  ...
  override def toErrorString: String = {
    ...
    val mapIndexString =
      if (mapIndex == Int.MinValue) "Unknown" else mapIndex.toString
...
case class ExceptionFailure(
  ...
  def exception: Option[Throwable] =
    exceptionWrapper.flatMap(w => Option(w.exception))
...
case class TaskCommitDenied(jobID: Int, partitionID: Int, attemptNumber: Int)
  ...
  override def toErrorString
      : String = "TaskCommitDenied (Driver denied task commit)" +
