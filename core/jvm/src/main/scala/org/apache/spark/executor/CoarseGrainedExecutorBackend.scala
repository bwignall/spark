...
private[spark] class CoarseGrainedExecutorBackend(
  ...
  override def onStart(): Unit = {
    if (env.conf.get(DECOMMISSION_ENABLED)) {
      ...
      logInfo(
        log"Registering SIG${MDC(LogKeys.SIGNAL, signal)}" +
          log" handler to trigger decommissioning."
      )
      SignalUtils.register(
        signal,
        log"Failed to register SIG${MDC(LogKeys.SIGNAL, signal)} " +
    ...
    try {
      ...
      if (
        NettyUtils.preferDirectBufs(shuffleClientTransportConf) &&
        PlatformDependent
          .maxDirectMemory() < env.conf.get(MAX_REMOTE_BLOCK_SIZE_FETCH_TO_MEM)
      ) {
    ...
    rpcEnv
      ...
      .onComplete {
        ...
        case Failure(e) =>
          exitExecutor(
            1,
            s"Cannot register with driver: $driverUrl",
            e,
            notifyDriver = false
          )
      }(ThreadUtils.sameThread)
  ...
  def notifyDriverAboutPushCompletion(
      shuffleId: Int,
      shuffleMergeId: Int,
      mapIndex: Int
  ): Unit = {
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case UpdateDelegationTokens(tokenBytes) =>
      logInfo(
        log"Received tokens of ${MDC(LogKeys.NUM_BYTES, tokenBytes.length)} bytes"
      )
    ...
    case ExecutorDecommissionSigReceived =>
      ...
      try {
        driver.foreach { driverRef =>
          ...
          driverNotified =
            driverRef.askSync[Boolean](ExecutorDecommissioning(executorId))
        ...
        case e: Exception =>
          if (driverNotified) {
            logError(
              "Fail to decommission self (but driver has been notified).",
              e
            )
          } else {
            logError(
              "Fail to tell driver that we are starting decommissioning",
              e
            )
          }
  ...
  override def onDisconnected(remoteAddress: RpcAddress): Unit = {
    if (stopping.get()) {
      logInfo(
        log"Driver from ${MDC(LogKeys.RPC_ADDRESS, remoteAddress)}" +
      ...
      logWarning(
        log"An unknown (${MDC(LogKeys.REMOTE_ADDRESS, remoteAddress)} " +
  ...
  override def statusUpdate(
    ...
    driver match {
      ...
      case None =>
        logWarning(
          log"Drop ${MDC(LogKeys.MESSAGE, msg)} because has not yet connected to driver"
        )
    }
  }
  ...
  protected def exitExecutor(
    ...
    if (stopping.compareAndSet(false, true)) {
      val message =
        log"Executor self-exiting due to : ${MDC(LogKeys.REASON, reason)}"
      if (throwable != null) {
      ...
      logInfo(
        "Skip exiting executor since it's been already asked to exit before."
      )
    }
  }
  ...
  private def decommissionSelf(): Unit = {
    if (!env.conf.get(DECOMMISSION_ENABLED)) {
      logWarning(
        "Receive decommission request, but decommission feature is disabled."
      )
      return
    ...
    try {
      ...
      if (migrationEnabled) {
        ...
        logError(
          log"Storage decommissioning attempted but neither " +
          ...
          while (true) {
            ...
            if (executor == null || executor.numRunningTasks == 0) {
              if (migrationEnabled) {
                ...
                if (
                  allBlocksMigrated && (migrationTime > lastTaskFinishTime
                    .get())
                ) {
                ...
                logInfo(
                  "No running tasks, no block migration configured, stopping."
                )
                exitExecutor(
                  0,
                  ExecutorLossMessage.decommissionFinished,
...
private[spark] object CoarseGrainedExecutorBackend extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    run(
      parseArguments(args, this.getClass.getCanonicalName.stripSuffix("$")),
      createFn
    )
    System.exit(0)
  }
      ...
      arguments.workerUrl.foreach { url =>
        env.rpcEnv.setupEndpoint(
          ...
          new WorkerWatcher(
            env.rpcEnv,
            url,
            isChildProcessStopping = backend.stopping
          )
        )
      }
  ...
  def parseArguments(
    ...
    if (hostname == null) {
      ...
      log.info(
        s"Executor hostname is not provided, will use '$hostname' to advertise itself"
      )
    }

    if (
      driverUrl == null || executorId == null || cores <= 0 || appId == null
    ) {
      printUsageAndExit(classNameForEntry)
    }
