...
private[spark] class ExecutorAllocationManager(
  ...
  private[spark] val numExecutorsToAddPerResourceProfileId =
  ...
  private[spark] val numExecutorsTargetPerResourceProfileId =
    new mutable.HashMap[Int, Int]
  ...
  private var numLocalityAwareTasksPerResourceProfileId =
    new mutable.HashMap[Int, Int]
  ...
  private def validateSettings(): Unit = {
    ...
    if (maxNumExecutors == 0) {
      throw new SparkException(
        s"${DYN_ALLOCATION_MAX_EXECUTORS.key} cannot be 0!"
      )
    }
    if (minNumExecutors > maxNumExecutors) {
      throw new SparkException(
        s"${DYN_ALLOCATION_MIN_EXECUTORS.key} ($minNumExecutors) must " +
          s"be less than or equal to ${DYN_ALLOCATION_MAX_EXECUTORS.key} ($maxNumExecutors)!"
      )
    }
    if (schedulerBacklogTimeoutS <= 0) {
      throw new SparkException(
        s"${DYN_ALLOCATION_SCHEDULER_BACKLOG_TIMEOUT.key} must be > 0!"
      )
    }
    ...
    if (!conf.get(config.SHUFFLE_SERVICE_ENABLED) && !reliableShuffleStorage) {
      if (shuffleTrackingEnabled) {
        ...
        logInfo(
          "Shuffle data decommission is enabled without a shuffle service."
        )
      } else if (!testing) {
        throw new SparkException(
          "Dynamic allocation of executors requires one of the " +
    ...
    if (
      shuffleTrackingEnabled && (shuffleDecommissionEnabled || reliableShuffleStorage)
    ) {
      logWarning(
        "You are enabling both shuffle tracking and other DA supported mechanism, " +
  ...
  private[spark] def maxNumExecutorsNeededPerResourceProfile(rpId: Int): Int = {
    ...
    logDebug(
      s"max needed for rpId: $rpId numpending: $numRunningOrPendingTasks," +
        s" tasksperexecutor: $tasksPerExecutor"
    )
    val maxNeeded = math
      .ceil(
        numRunningOrPendingTasks * executorAllocationRatio /
          tasksPerExecutor
      )
      .toInt
    ...
    if (unschedulableTaskSets > 0) {
      ...
      val maxNeededForUnschedulables = math
        .ceil(
          unschedulableTaskSets * executorAllocationRatio /
            tasksPerExecutor
        )
        .toInt
      math.max(
        maxNeededWithSpeculationLocalityOffset,
        executorMonitor.executorCountWithResourceProfile(
          rpId
  ...
  private def updateAndSyncNumExecutorsTarget(now: Long): Int = synchronized {
    if (initializing) {
      ...
      val updatesNeeded =
        new mutable.HashMap[Int, ExecutorAllocationManager.TargetNumUpdates]

      // Update targets for all ResourceProfiles then do a single request to the cluster manager
      numExecutorsTargetPerResourceProfileId.foreach {
        case (rpId, targetExecs) =>
    ...
    if (updates.size > 0) {
      ...
      if (requestAcknowledged) {
        ...
        updates.foreach { case (rpId, targetNum) =>
          ...
          totalDelta += delta
          if (delta > 0) {
            val executorsString = log" new executor" + {
              if (delta > 1) log"s" else log""
            }
            logInfo(
            ...
            addTime =
              now + TimeUnit.SECONDS.toNanos(sustainedSchedulerBacklogTimeoutS)
          } else {
            logDebug(
              s"Lowering target number of executors to" +
        ...
        updates.foreach { case (rpId, targetNum) =>
          logWarning(
            "Unable to reach the cluster manager to request more executors!"
          )
          numExecutorsTargetPerResourceProfileId(rpId) =
            targetNum.oldNumExecutorsTarget
        }
        0
      }
    } else {
  ...
  private def addExecutors(maxNumExecutorsNeeded: Int, rpId: Int): Int = {
    ...
    if (oldNumExecutorsTarget >= maxNumExecutors) {
      logDebug(
        "Not adding executors because our current target total " +
    ...
    var numExecutorsTarget = math.max(
      numExecutorsTargetPerResourceProfileId(rpId),
  ...
  /** Request the cluster manager to remove the given executors.
    ...
    */
  private def removeExecutors(executors: Seq[(String, Int)]): Seq[String] =
    synchronized {
      ...
      executors.foreach { case (executorIdToBeRemoved, rpId) =>
        if (rpId == UNKNOWN_RESOURCE_PROFILE_ID) {
          if (testing) {
            throw new SparkException(
              "ResourceProfile Id was UNKNOWN, this is not expected"
            )
          }
          logWarning(
            log"Not removing executor ${MDC(EXECUTOR_IDS, executorIdToBeRemoved)} " +
          ...
          val newExecutorTotal = numExecutorsTotalPerRpId.getOrElseUpdate(
            rpId,
            (executorMonitor.executorCountWithResourceProfile(rpId) -
          ...
          if (newExecutorTotal - 1 < minNumExecutors) {
            logDebug(
              s"Not removing idle executor $executorIdToBeRemoved because there " +
      ...
      val executorsRemoved = if (testing) {
        ...
        if (decommissionEnabled) {
          val executorIdsWithoutHostLoss = executorIdsToBeRemoved
            .map(id => (id, ExecutorDecommissionInfo("spark scale down")))
            .toArray
          ...
          client.killExecutors(
            executorIdsToBeRemoved.toSeq,
            adjustTargetNumExecutors = false,
            countFailures = false,
            force = false
          )
      ...
      if (testing || executorsRemoved.nonEmpty) {
        ...
        logInfo(
          log"Executors ${MDC(EXECUTOR_IDS, executorsRemoved.mkString(","))}" +
            log"removed due to idle timeout."
        )
        executorsRemoved.toSeq
      } else {
        logWarning(
          log"Unable to reach the cluster manager to kill executor/s " +
  ...
  private def onSchedulerBacklogged(): Unit = synchronized {
    if (addTime == NOT_SET) {
      ...
      addTime =
        clock.nanoTime() + TimeUnit.SECONDS.toNanos(schedulerBacklogTimeoutS)
    }
  }
  ...
  private[spark] class ExecutorAllocationListener extends SparkListener {
    ...
    private val stageAttemptToNumRunningTask =
      new mutable.HashMap[StageAttempt, Int]
    private val stageAttemptToTaskIndices =
    ...
    override def onStageSubmitted(
      ...
      allocationManager.synchronized {
        ...
        logDebug(
          s"Stage resource profile id is: $profId with numTasks: $numTasks"
        )
        resourceProfileIdToStageAttempt.getOrElseUpdate(
          profId,
          new mutable.HashSet[StageAttempt]
        ) += stageAttempt
        ...
        stageAttemptToExecutorPlacementHints.put(
          stageAttempt,
        ...
        if (!numExecutorsTargetPerResourceProfileId.contains(profId)) {
          ...
          if (initialNumExecutors > 0) {
            logDebug(
              s"requesting executors, rpId: $profId, initial number is $initialNumExecutors"
            )
    ...
    override def onStageCompleted(
      ...
      allocationManager.synchronized {
        ...
        if (
          stageAttemptToNumTasks.isEmpty
    ...
    override def onTaskStart(taskStart: SparkListenerTaskStart): Unit = {
      ...
      allocationManager.synchronized {
        ...
        if (taskStart.taskInfo.speculative) {
          stageAttemptToSpeculativeTaskIndices.getOrElseUpdate(
            stageAttempt,
          ...
          stageAttemptToTaskIndices.getOrElseUpdate(
            stageAttempt,
            new mutable.HashSet[Int]
          ) += taskIndex
        }
    ...
    override def onSpeculativeTaskSubmitted(
        speculativeTask: SparkListenerSpeculativeTaskSubmitted
    ): Unit = {
      ...
      allocationManager.synchronized {
        stageAttemptToPendingSpeculativeTasks
          .getOrElseUpdate(stageAttempt, new mutable.HashSet[Int])
          .add(taskIndex)
        allocationManager.onSchedulerBacklogged()
      }
    }
    ...
    def removeStageFromResourceProfileIfUnused(
      ...
      if (
        ...
        if (rpForStage.size == 1) {
          ...
          logWarning(
            log"Should have exactly one resource profile for stage " +
    ...
    def pendingTasksPerResourceProfile(rpId: Int): Int = {
      val attempts =
        resourceProfileIdToStageAttempt.getOrElse(rpId, Set.empty).toSeq
    ...
    def pendingSpeculativeTasksPerResourceProfile(rp: Int): Int = {
      val attempts =
        resourceProfileIdToStageAttempt.getOrElse(rp, Set.empty).toSeq
    ...
    def updateExecutorPlacementHints(): Unit = {
      ...
      val rplocalityToCount =
        new mutable.HashMap[Int, mutable.HashMap[String, Int]]()
      stageAttemptToExecutorPlacementHints.values.foreach {
        case (numTasksPending, localities, rpId) =>
          ...
          localityAwareTasksPerResourceProfileId(rpId) =
            rpNumPending + numTasksPending
          localities.foreach { case (hostname, count) =>
            val rpBasedHostToCount =
              rplocalityToCount.getOrElseUpdate(
                rpId,
      ...
      allocationManager.rpIdToHostToLocalTaskCount = rplocalityToCount.map {
        case (k, v) => (k, v.toMap)
      }.toMap
    }
  }
...
private[spark] class ExecutorAllocationManagerSource(
  ...
  val gracefullyDecommissioned: Counter = getCounter(
    "numberExecutorsGracefullyDecommissioned"
  )
  val decommissionUnfinished: Counter = getCounter(
    "numberExecutorsDecommissionUnfinished"
  )
  val driverKilled: Counter = getCounter("numberExecutorsKilledByDriver")
  val exitedUnexpectedly: Counter = getCounter(
    "numberExecutorsExitedUnexpectedly"
  )

  // The metrics are going to return the sum for all the different ResourceProfiles.
  registerGauge(
    "numberExecutorsToAdd",
    executorAllocationManager.numExecutorsToAddPerResourceProfileId.values.sum,
    0
  )
  registerGauge(
    "numberExecutorsPendingToRemove",
    executorAllocationManager.executorMonitor.pendingRemovalCount,
    0
  )
  registerGauge(
    "numberAllExecutors",
    executorAllocationManager.executorMonitor.executorCount,
    0
  )
  registerGauge(
    "numberTargetExecutors",
    executorAllocationManager.numExecutorsTargetPerResourceProfileId.values.sum,
    0
  )
  registerGauge(
    "numberMaxNeededExecutors",
    executorAllocationManager.numExecutorsTargetPerResourceProfileId.keys
      .map(executorAllocationManager.maxNumExecutorsNeededPerResourceProfile(_))
      .sum,
    0
  )
  registerGauge(
    "numberDecommissioningExecutors",
    executorAllocationManager.executorMonitor.decommissioningCount,
    0
  )
}
...
private object ExecutorAllocationManager {
  ...
  private[spark] case class TargetNumUpdates(
      delta: Int,
