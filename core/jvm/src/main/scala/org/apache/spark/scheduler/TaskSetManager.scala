...
private[spark] class TaskSetManager(
  ...
  private[scheduler] val partitionToIndex = tasks.zipWithIndex.map {
    case (t, idx) => t.partitionId -> idx
  }.toMap
  ...
  val minFinishedForSpeculation =
    math.max((speculationQuantile * numTasks).floor.toInt, 1)
  ...
  private[scheduler] val taskSetExcludelistHelperOpt
    ...
    healthTracker.map { _ =>
      new TaskSetExcludelist(
        sched.sc.listenerBus,
        conf,
        stageId,
  ...
  private[scheduler] def isBarrier =
    taskSet.tasks.nonEmpty && taskSet.tasks(0).isBarrier
  ...
  private[scheduler] val barrierPendingLaunchTasks =
    new HashMap[Int, BarrierPendingLaunchTask]()
    ...
    val (_, duration) = Utils.timeTakenMs {
      ...
      racks.zip(indicesForHosts).foreach {
        case (Some(rack), indices) =>
          pendingTasks.forRack
            .getOrElseUpdate(rack, new ArrayBuffer) ++= indices
  ...
  private var currentLocalityIndex =
    0 // Index of our current locality level in validLocalityLevels
  private var lastLocalityWaitResetTime =
    clock.getTimeMillis() // Time we last reset locality wait
  ...
  private[spark] def addPendingTask(
    ...
    for (loc <- tasks(index).preferredLocations) {
        ...
        case e: HDFSCacheTaskLocation =>
          ...
          exe match {
            case Some(set) =>
              ...
              logInfo(
                log"Pending task ${MDC(INDEX, index)} has a cached location at " +
      ...
      pendingTaskSetToAddTo.forHost.getOrElseUpdate(
        loc.host,
        new ArrayBuffer
      ) += index
      ...
      if (resolveRacks) {
        sched.getRackForHost(loc.host).foreach { rack =>
          pendingTaskSetToAddTo.forRack
            .getOrElseUpdate(rack, new ArrayBuffer) += index
        }
      }
    }
  ...
  private def dequeueTaskFromList(
    ...
    while (indexOffset > 0) {
      ...
      if (
        ...
        if (!successful(index)) {
          if (
            copiesRunning(index) == 0 && !barrierPendingLaunchTasks.contains(
              index
            )
          ) {
            return Some(index)
  ...
  private def isTaskExcludededOnExecOrNode(
      index: Int,
      execId: String,
      host: String
  ): Boolean = {
  ...
  protected def dequeueTaskHelper(
    ...
    val pendingTaskSetToUse =
      if (speculative) pendingSpeculatableTasks else pendingTasks
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.NODE_LOCAL)) {
      dequeue(pendingTaskSetToUse.forHost.getOrElse(host, ArrayBuffer()))
        .foreach { index =>
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.RACK_LOCAL)) {
      for {
        ...
        index <- dequeue(
          pendingTaskSetToUse.forRack.getOrElse(rack, ArrayBuffer())
        )
      } {
  ...
  private[scheduler] def resetDelayScheduleTimer(
      minLocality: Option[TaskLocality.TaskLocality]
  ): Unit = {
      ...
      val taskDescription =
        dequeueTask(execId, host, allowedLocality)
          .map { case (index, taskLocality, speculative) =>
            ...
            if (
              legacyLocalityWaitReset && maxLocality != TaskLocality.NO_PREF
            ) {
              resetDelayScheduleTimer(Some(taskLocality))
            }
            if (isBarrier) {
              barrierPendingLaunchTasks(index) = BarrierPendingLaunchTask(
                execId,
                host,
                index,
                taskLocality,
  ...
  def prepareLaunchingTask(
    ...
    val serializedTask: ByteBuffer =
      try {
        ...
        case NonFatal(e) =>
          val msg =
            log"Failed to serialize task ${MDC(TASK_ID, taskId)}, not attempting to retry it."
          logError(msg, e)
    ...
    if (
      ...
      logWarning(
        log"Stage ${MDC(STAGE_ID, task.stageId)} contains a task of very large size " +
  ...
  private def maybeFinishTaskSet(): Unit = {
    if (isZombie && runningTasks == 0) {
      ...
      if (tasksSuccessful == numTasks) {
        healthTracker.foreach(
          _.updateExcludedForSuccessfulTaskSet(
            taskSet.stageId,
            taskSet.stageAttemptId,
  ...
  /** Get the level we can launch tasks according to delay scheduling, based on current wait time.
    */
  private def getAllowedLocalityLevel(
      curTime: Long
  ): TaskLocality.TaskLocality = {
    // Remove the scheduled or finished tasks lazily
    def tasksNeedToBeScheduledFrom(
        pendingTaskIds: ArrayBuffer[Int]
    ): Boolean = {
    ...
    def moreTasksToRunIn(
        pendingTasks: HashMap[String, ArrayBuffer[Int]]
    ): Boolean = {
    ...
    while (currentLocalityIndex < myLocalityLevels.length - 1) {
      ...
      if (!moreTasks) {
        ...
        curTime - lastLocalityWaitResetTime >= localityWaits(
          ...
          hostToExecutors.forall { case (host, execsOnHost) =>
            ...
            if (nodeExcluded) {
              ...
              execsOnHost.forall { exec =>
                ...
                taskSetExcludelist.isExecutorExcludedForTask(
                  exec,
                  indexInTaskSet
                )
              }
  ...
  def canFetchMoreResults(size: Long): Boolean = sched.synchronized {
    ...
    calculatedTasks += 1
    if (
      !isShuffleMapTasks && maxResultSize > 0 && totalResultSize > maxResultSize
    ) {
      val msg =
        log"Total size of serialized results of ${MDC(COUNT, calculatedTasks)} tasks " +
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]): Unit = {
    ...
    for (attemptInfo <- taskAttempts(index) if attemptInfo.running) {
      logInfo(
        log"Killing attempt ${MDC(NUM_ATTEMPT, attemptInfo.attemptNumber)} for " +
    ...
    if (!successful(index)) {
      ...
      logInfo(
        log"Ignoring task-finished event for " +
  ...
  private def emptyTaskInfoAccumulablesAndNotifyDagScheduler(
    ...
    if (dropTaskInfoAccumulablesOnTaskCompletion) {
      ...
      taskAttempts(index) = taskAttempts(index).map { i =>
        if (i eq taskInfoWithAccumulables) clonedTaskInfo else i
      }
  ...
  def handleFailedTask(
    ...
    val failureException: Option[Throwable] = reason match {
      case fetchFailed: FetchFailed =>
        ...
        if (fetchFailed.bmAddress != null) {
          healthTracker.foreach(
            _.updateExcludedForFetchFailure(
              fetchFailed.bmAddress.host,
      ...
      case ef: ExceptionFailure =>
        ...
        if (ef.className == classOf[NotSerializableException].getName) {
          ...
          logError(
            log"${MDC(TASK_NAME, task)} had a not serializable result: " +
        ...
        if (
          ef.className == classOf[TaskOutputFileAlreadyExistException].getName
        ) {
          ...
          abort(
            "Task %s in stage %s (TID %d) can not write to output file: %s"
      ...
      case e: ExecutorLostFailure if !e.exitCausedByApp =>
        logInfo(
          log"${MDC(TASK_NAME, taskName(tid))} failed because while it was being computed," +
    ...
    if (!isZombie && reason.countTowardsTaskFailures) {
      ...
      taskSetExcludelistHelperOpt.foreach(
        _.updateExcludedForFailedTask(
          info.host,
          info.executorId,
          index,
          failureReasonString
    ...
    if (successful(index)) {
      logInfo(
        log"${MDC(LogKeys.TASK_NAME, taskName(info.taskId))} failed, but the task will not" +
  ...
  override def executorLost(
      ...
      for ((tid, info) <- taskInfos if info.executorId == execId) {
        ...
        lazy val isShuffleMapOutputAvailable = reason match {
          case ExecutorDecommission(_, _) =>
            ...
            val locationOpt = env.mapOutputTracker
        ...
        if (
          successful(index) && !info.running && !killedByOtherAttempt.contains(
            tid
          ) &&
    ...
    for (
      (tid, info) <- taskInfos if info.running && info.executorId == execId
    ) {
  ...
  private def checkAndSubmitSpeculatableTasks(
      ...
      threshold: Double,
      customizedThreshold: Boolean = false
  ): Boolean = {
    ...
    for (tid <- runningTasksSet) {
      ...
      val index = info.index
      if (
        !successful(index) && copiesRunning(index) == 1 && !speculatableTasks
          .contains(index)
      ) {
        ...
        def taskProcessRateIsInefficient(): Boolean = {
          taskProcessRateCalculator.forall(calculator => {
            calculator.getRunningTasksProcessRate(tid) <
              calculator
                .getAvgTaskProcessRate() * efficientTaskProcessMultiplier
          })
        }
  ...
  override def checkSpeculatableTasks(minTimeToSpeculation: Long): Boolean = {
    ...
    if (
      isZombie || isBarrier || (numTasks == 1 && !isSpeculationThresholdSpecified)
    ) {
      return false
    }
    ...
    if (numSuccessfulTasks >= minFinishedForSpeculation) {
      ...
      isSpeculationThresholdSpecified && speculationTasksLessEqToSlots
    ) {
      val threshold = speculationTaskDurationThresOpt.get
      logDebug(
        s"Tasks taking longer time than provided speculation threshold: $threshold"
      )
    ...
    if (foundTasks) {
      ...
      if (elapsedMs > minTimeToSpeculation) {
        logWarning(
          log"Time to checkSpeculatableTasks ${MDC(TIME_UNITS, elapsedMs)}ms > " +
  ...
  private def computeValidLocalityLevels(): Array[TaskLocality.TaskLocality] = {
    ...
    levels += ANY
    logDebug(
      "Valid locality levels for " + taskSet + ": " + levels.mkString(", ")
    )
    levels.toArray
  }
  ...
  def recomputeLocality(): Unit = {
    ...
    if (currentLocalityIndex > previousLocalityIndex) {
      ...
      currentLocalityIndex = getLocalityIndex(
        myLocalityLevels.diff(previousMyLocalityLevels).head
      )
    }
  }
  ...
  private[TaskSetManager] class TaskProcessRateCalculator {
    ...
    private[TaskSetManager] def getRunningTasksProcessRate(
        taskId: Long
    ): Double = {
    ...
    private[TaskSetManager] def updateAvgTaskProcessRate(
      ...
      result.accumUpdates.foreach { a =>
        if (
          a.name == Some(shuffleRead.RECORDS_READ) ||
      ...
      if (totalRecordsRead > 0 && totalExecutorRunTime > 0) {
        avgTaskProcessRate =
          sched.getTaskProcessRate(totalRecordsRead, totalExecutorRunTime)
      }
