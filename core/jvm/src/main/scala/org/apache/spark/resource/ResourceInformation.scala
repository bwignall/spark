...
class ResourceInformation(val name: String, val addresses: Array[String])
  ...
  override def toString: String =
    s"[name: ${name}, addresses: ${addresses.mkString(",")}]"
  ...
  override def hashCode(): Int =
    Seq(name, addresses.toImmutableArraySeq).hashCode()
  ...
  final def toJson(): JValue =
    ResourceInformationJson(name, addresses.toImmutableArraySeq).toJValue
}

private[spark] object ResourceInformation {

  private lazy val exampleJson: String = compact(
    render(ResourceInformationJson("gpu", Seq("0", "1")).toJValue)
  )
    ...
    try {
      ...
      case NonFatal(e) =>
        throw new SparkException(
          s"Error parsing JSON into ResourceInformation:\n$json\n" +
            s"Here is a correct example: $exampleJson.",
          e
        )
    }
  }
