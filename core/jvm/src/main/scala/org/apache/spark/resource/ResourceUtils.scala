...
class ResourceID(val componentName: String, val resourceName: String) {
  ...
  private[spark] def discoveryScriptConf: String =
...
class ResourceRequest(
  ...
  override def hashCode(): Int =
    Seq(id, amount, discoveryScript, vendor).hashCode()
}
...
/** Case class representing allocated resource addresses for a specific resource.
  ...
  */
private[spark] case class ResourceAllocation(
    id: ResourceID,
...
private[spark] object ResourceUtils extends Logging {
    ...
    val amount = settings
      .getOrElse(
        ...
        throw new SparkException(
          s"You must specify an amount for ${resourceId.resourceName}"
        )
      )
      .toInt
    val discoveryScript =
      Optional.ofNullable(settings.get(DISCOVERY_SCRIPT).orNull)
  ...
  def listResourceIds(
    ...
    sparkConf
      .getAllWithPrefix(s"$componentName.$RESOURCE_PREFIX.")
      .map { case (key, _) =>
        val index = key.indexOf('.')
        if (index < 0) {
          throw new SparkException(
            s"You must specify an amount config for resource: $key " +
      ...
      .distinct
      .map(name => new ResourceID(componentName, name))
      .toImmutableArraySeq
  }
  ...
  def addTaskResourceRequests(
    ...
    val nonZeroTaskReqs = listResourceIds(sparkConf, SPARK_TASK_PREFIX)
      .map { resourceId =>
        val settings = sparkConf.getAllWithPrefix(resourceId.confPrefix).toMap
        val amountDouble = settings
          .getOrElse(
            AMOUNT,
            throw new SparkException(
              s"You must specify an amount for ${resourceId.resourceName}"
  ...
  def parseResourceRequirements(
    ...
    rnamesAndAmounts.filter { case (_, amount) => amount > 0 }.map {
      case (rName, amountDouble) =>
        val (amount, parts) =
          if (componentName.equalsIgnoreCase(SPARK_TASK_PREFIX)) {
  ...
  def executorResourceRequestToRequirement(
      resourceRequest: Seq[ExecutorResourceRequest]
  ): Seq[ResourceRequirement] = {
  ...
  def withResourcesJson[T](
      resourcesFile: String
  )(extract: String => Seq[T]): Seq[T] = {
    ...
    try {
      ...
      case NonFatal(e) =>
        throw new SparkException(
          s"Error parsing resources file $resourcesFile",
          e
        )
    }
  }

  def parseAllocatedFromJsonFile(
      resourcesFile: String
  ...
  def parseAllocated(
    ...
    resourcesFileOpt.toSeq
      .flatMap(parseAllocatedFromJsonFile)
  ...
  private def parseAllocatedOrDiscoverResources(
    ...
    val otherResourceIds =
      listResourceIds(sparkConf, componentName).diff(allocated.map(_.id))
  ...
  private def assertResourceAllocationMeetsRequest(
    ...
    require(
      allocation.id == request.id && allocation.addresses.size >= request.amount,
  ...
  private def assertAllResourceAllocationsMatchResourceProfile(
      ...
      execReqs: Map[String, ExecutorResourceRequest]
  ): Unit = {
    execReqs.foreach { case (rName, req) =>
      require(
        allocations.contains(rName) && allocations(
          rName
        ).addresses.length >= req.amount,
  ...
  def getOrDiscoverAllResources(
    ...
    val allocations = parseAllocatedOrDiscoverResources(
      sparkConf,
      componentName,
      resourcesFileOpt
    )
  ...
  def getOrDiscoverAllResourcesForResourceProfile(
    ...
    val fileAllocResMap =
      fileAllocated.map(a => (a.id.resourceName, a.toResourceInformation)).toMap
  ...
  private[spark] def discoverResource(
    ...
    val discoveryScriptPlugin =
      "org.apache.spark.resource.ResourceDiscoveryScriptPlugin"
    val pluginClasses =
      sparkConf.get(RESOURCES_DISCOVERY_PLUGIN) :+ discoveryScriptPlugin
    val resourcePlugins = Utils.loadExtensions(
      classOf[ResourceDiscoveryPlugin],
      pluginClasses,
      sparkConf
    )
    ...
    throw new SparkException(
      s"None of the discovery plugins returned ResourceInformation for " +
  ...
  def validateTaskCpusLargeEnough(
    ...
    if (execCores < taskCpus) {
      throw new SparkException(
        s"The number of cores per executor (=$execCores) has to be >= " +
  ...
  def warnOnWastedResources(
    ...
    if (
      limitingResource.nonEmpty && !limitingResource.equals(
        ResourceProfile.CPUS
      )
    ) {
      if ((taskCpus * maxTaskPerExec) < cores) {
        val resourceNumSlots = Math.floor(cores / taskCpus).toInt
        val message =
          log"The configuration of cores (exec = ${MDC(NUM_CORES, cores)} " +
