          ...
          val (result, size) =
            serializer.get().deserialize[TaskResult[_]](serializedData) match {
              case directResult: DirectTaskResult[_] =>
                if (
                  !taskSetManager
                    .canFetchMoreResults(directResult.valueByteBuffer.size)
                ) {
              ...
              case IndirectTaskResult(blockId, size) =>
                ...
                logDebug(
                  s"Fetching indirect task result for ${taskSetManager.taskName(tid)}"
                )
        ...
        Utils.logUncaughtExceptions {
          ...
          try {
            if (serializedData != null && serializedData.limit() > 0) {
              reason = serializer
                .get()
                .deserialize[TaskFailedReason](serializedData, loader)
            }
          } catch {
