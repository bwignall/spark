...
private[spark] object TestUtils extends SparkTestUtils {
  ...
  /** Run some code involving jobs submitted to the given context and assert that the jobs spilled.
    */
  def assertSpilled(sc: SparkContext, identifier: String)(
      body: => Unit
  ): Unit = {
  ...
  /** Run some code involving jobs submitted to the given context and assert that the jobs
    * did not spill.
    */
  def assertNotSpilled(sc: SparkContext, identifier: String)(
      body: => Unit
  ): Unit = {
    val listener = new SpillListener
    withListener(sc, listener) { _ =>
  ...
  def assertExceptionMsg[E <: Throwable: ClassTag](
    ...
    val (typeMsg, typeCheck) = if (classTag[E] == classTag[Nothing]) {
      ...
      (
        s"of type ${clazz.getName} ",
        (e: Throwable) => clazz.isAssignableFrom(e.getClass)
      )
    }
    ...
    def contain(e: Throwable, msg: String): Boolean = {
      if (ignoreCase) {
        e.getMessage
          ...
          .contains(msg.toLowerCase(Locale.ROOT))
      } else {
  ...
  def testCommandAvailable(command: String): Boolean =
    Utils.checkCommandAvailable(command)
  ...
  private def isPythonVersionAtLeast(
    ...
    val pythonSnippet =
      s"import sys; sys.exit(sys.version_info < ($major, $minor, $reversion))"
    Try(Process(cmdSeq :+ s"python3 -c '$pythonSnippet'").! == 0)
      .getOrElse(false)
  }
  ...
  def getAbsolutePathFromExecutable(executable: String): Option[String] = {
    ...
    if (
      command.split(File.separator, 2).length == 1 &&
      ...
      sys
        ...
        .map(path =>
          Paths.get(
            s"${StringUtils.strip(path, "\"")}${File.separator}$command"
          )
        )
  ...
  def httpResponseMessage(
    ...
    withHttpConnection(url, method, headers = headers) { connection =>
      Source
        .fromInputStream(connection.getInputStream, "utf-8")
        .getLines()
        .mkString("\n")
    }
  }
  ...
  def withHttpServer(resBaseDir: String = ".")(body: URL => Unit): Unit = {
    ...
    val server = new Server(
      new InetSocketAddress(Utils.localCanonicalHostName(), 0)
    )
  ...
  def configTestLog4j2(level: String): Unit = {
    ...
    builder.add(
      builder.newRootLogger(level).add(builder.newAppenderRef("console"))
    )
  ...
  def createTempScriptWithExpectedOutput(
      dir: File,
      prefix: String,
      output: String
  ): String = {
    ...
    JavaFiles.setPosixFilePermissions(
      file.toPath,
...
private class SpillListener extends SparkListener {
  private val stageIdToTaskMetrics =
    new mutable.HashMap[Int, ArrayBuffer[TaskMetrics]]
  ...
  override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = synchronized {
    stageIdToTaskMetrics.getOrElseUpdate(
      taskEnd.stageId,
      new ArrayBuffer[TaskMetrics]
    ) += taskEnd.taskMetrics
  }

  override def onStageCompleted(
      stageComplete: SparkListenerStageCompleted
  ): Unit = synchronized {
