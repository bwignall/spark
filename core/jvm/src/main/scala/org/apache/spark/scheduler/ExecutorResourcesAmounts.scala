...
private[spark] class ExecutorResourcesAmounts(
    private val resources: Map[String, Map[String, Long]]
  ...
  def acquire(assignedResource: Map[String, Map[String, Long]]): Unit = {
    assignedResource.foreach { case (rName, taskResAmounts) =>
      val availableResourceAmounts = internalResources.getOrElse(
        rName,
        throw new SparkException(
          s"Try to acquire an address from $rName that doesn't exist"
        )
      )
      taskResAmounts.foreach { case (address, amount) =>
        val prevInternalTotalAmount = availableResourceAmounts.getOrElse(
          address,
          throw new SparkException(
            s"Try to acquire an address that doesn't exist. $rName " +
  ...
  def release(assignedResource: Map[String, Map[String, Long]]): Unit = {
    assignedResource.foreach { case (rName, taskResAmounts) =>
      val availableResourceAmounts = internalResources.getOrElse(
        rName,
        throw new SparkException(
          s"Try to release an address from $rName that doesn't exist"
        )
      )
      taskResAmounts.foreach { case (address, amount) =>
        val prevInternalTotalAmount = availableResourceAmounts.getOrElse(
          address,
          throw new SparkException(
            s"Try to release an address that is not assigned. $rName " +
  ...
  /** Try to assign the addresses according to the task requirement. This function always goes
    ...
    */
  def assignAddressesCustomResources(
      taskSetProf: ResourceProfile
        ...
        case Some(addressesAmountMap) =>
          ...
          if (taskAmount >= 1.0) {
            for (address <- addresses if taskAmount > 0) {
              ...
              if (
                ResourceAmountUtils.isOneEntireResource(
                  addressesAmountMap(address)
                )
              ) {
                taskAmount -= 1.0
...
private[spark] object ExecutorResourcesAmounts {
  ...
  /** Converts executor infos to ExecutorResourcesAmounts
    */
  def apply(
      executorInfos: Map[String, ExecutorResourceInfo]
  ): ExecutorResourcesAmounts = {
    new ExecutorResourcesAmounts(
      executorInfos.map { case (rName, rInfo) =>
        rName -> rInfo.resourcesAmounts
      }
    )
  }
