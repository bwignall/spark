...
private[deploy] object DeployTestUtils {
  ...
  def createAppDesc(
    ...
    val cmd =
      new Command("mainClass", List("arg1", "arg2"), Map(), Seq(), Seq(), Seq())
  ...
  def createDefaultResourceProfile(
    ...
    val rp = createResourceProfile(
      Some(memoryPerExecutorMb),
      customResources,
      coresPerExecutor
    )
  ...
  def createDriverInfo(): DriverInfo = {
    val dDesc =
      createDriverDesc().copy(resourceReqs = createResourceRequirement)
