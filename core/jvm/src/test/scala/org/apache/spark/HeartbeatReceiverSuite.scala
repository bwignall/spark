...
class HeartbeatReceiverSuite
  ...
  override def beforeEach(): Unit = {
    ...
    when(scheduler.executorHeartbeatReceived(any(), any(), any(), any()))
      .thenReturn(true)
  }
  ...
  test(
    "SPARK-44726: Show spark.storage.blockManagerHeartbeatTimeoutMs error message"
  ) {
    sc.stop()
    val conf = new SparkConf()
      .setMaster("local[2]")
...
/** Dummy RPC endpoint to simulate executors.
  */
private class FakeExecutorEndpoint(override val rpcEnv: RpcEnv)
    extends RpcEndpoint {
...
private class FakeSchedulerBackend(
  ...
  protected override def doKillExecutors(
