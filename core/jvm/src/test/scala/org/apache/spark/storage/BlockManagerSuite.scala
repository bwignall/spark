...
import org.apache.spark.network.{
  BlockDataManager,
...
import org.apache.spark.network.netty.{
...
class BlockManagerSuite
  ...
  val mapOutputTracker =
    new MapOutputTrackerMaster(new SparkConf(false), bcastManager, true)
  ...
  private def makeSortShuffleManager(
  ...
  private def makeBlockManager(
    ...
    val serializerManager =
      new SerializerManager(serializer, bmConf, encryptionKey)
    ...
    val externalShuffleClient =
      if (bmConf.get(config.SHUFFLE_SERVICE_ENABLED)) {
  ...
  override def beforeEach(): Unit = {
    ...
    val blockManagerInfo =
      new mutable.HashMap[BlockManagerId, BlockManagerInfo]()
    ...
    val driverEndpoint = rpcEnv.setupEndpoint(
      ...
      new RpcEndpoint {
        ...
        override def receiveAndReply(
          ...
          case CoarseGrainedClusterMessages
                .RegisterExecutor(executorId, _, _, _, _, _, _, _) =>
  ...
  test("SPARK-36036: make sure temporary download files are deleted") {
    ...
    def createAndRegisterTempFileForDeletion(): String = {
      ...
      val tempDownloadFile =
        store.remoteBlockTempFileManager.createTempFile(transportConf)

      tempDownloadFile.openForWriting().close()
      assert(
        new File(tempDownloadFile.path()).exists(),
        "The file has been created"
      )

      val registered = store.remoteBlockTempFileManager.registerTempFileToClean(
        tempDownloadFile
      )
      assert(
        registered,
        "The file has been successfully registered for auto clean up"
      )
  ...
  test(
    "SPARK-32091: count failures from active executors when remove rdd/broadcast/shuffle"
  ) {
    setupBlockManagerMasterWithBlocks(false)
    // fail because bm2 will timeout and it's not lost anymore
    assert(
      intercept[Exception](master.removeRdd(0, true)).getCause
    ...
    assert(
      intercept[Exception](master.removeShuffle(0, true)).getCause
        .isInstanceOf[TimeoutException]
    )
  }

  test(
    "SPARK-32091: ignore failures from lost executors when remove rdd/broadcast/shuffle"
  ) {
  ...
  test(
    "SPARK-41360: Avoid block manager re-registration if the executor has been lost"
  ) {
  ...
  test("BlockManagerId object caching") {
    ...
    assert(
      id1_.eq(id1),
      "Deserialized id1 is not the same object as original id1"
    )
  ...
  test("master + 1 manager interaction") {
    ...
    assert(
      store.getSingleAndReleaseLock("a1") === None,
      "a1 not removed from store"
    )
    assert(
      store.getSingleAndReleaseLock("a2") === None,
      "a2 not removed from store"
    )
  ...
  test("master + 2 managers interaction") {
    ...
    assert(
      master.getLocations("a1").size === 2,
      "master did not report 2 locations for a1"
    )
    assert(
      master.getLocations("a2").size === 2,
      "master did not report 2 locations for a2"
    )
  }
  ...
  test("removing block") {
    ...
    assert(
      memStatus._1 == 40000L,
      "total memory " + memStatus._1 + " should equal 40000"
    )
    assert(
      memStatus._2 <= 32000L,
      "remaining memory " + memStatus._2 + " should <= 12000"
    )
    assert(
      store.getSingleAndReleaseLock("a1-to-remove").isDefined,
      "a1 was not in store"
    )
    assert(
      store.getSingleAndReleaseLock("a2-to-remove").isDefined,
      "a2 was not in store"
    )
    assert(
      store.getSingleAndReleaseLock("a3-to-remove").isDefined,
      "a3 was not in store"
    )

    // Checking whether master knows about the blocks or not
    assert(
      master.getLocations("a1-to-remove").size > 0,
      "master was not told about a1"
    )
    assert(
      master.getLocations("a2-to-remove").size > 0,
      "master was not told about a2"
    )
    assert(
      master.getLocations("a3-to-remove").size === 0,
  ...
  private def assertUpdateBlockInfoReportedForRemovingBlock(
    ...
    def assertSizeReported(
        captor: ArgumentCaptor[Long],
    ...
    val diskSizeCaptor =
      ArgumentCaptor.forClass(classOf[Long]).asInstanceOf[ArgumentCaptor[Long]]
    verify(master).updateBlockInfo(
      mc.eq(store.blockManagerId),
      mc.eq(blockId),
      mc.eq(StorageLevel.NONE),
  ...
  private def assertUpdateBlockInfoNotReported(
      store: BlockManager,
      blockId: BlockId
  ): Unit = {
  ...
  test("optimize a location order of blocks without topology information") {
    ...
    val locations =
      blockManager invokePrivate sortLocations(bmMaster.getLocations("test"))
  ...
  test("optimize a location order of blocks with topology information") {
    ...
    val blockManager = makeBlockManager(128, "exec", bmMaster)
    blockManager.blockManagerId = BlockManagerId(
  ...
  test(
    ...
    assert(
      store.getRemoteBytes("list1").isDefined,
      "list1Get expected to be fetched"
    )
  ...
  Seq(
    ...
    test(
      s"SPARK-27622: avoid the network when block requested from same host, $storageLevel"
    ) {
      conf.set("spark.shuffle.io.maxRetries", "0")
      ...
      val otherHostBm =
        makeBlockManager(8000, "otherHost", master, Some(otherHostTransferSrv))
      ...
      val bmToPutBlock =
        if (storageLevel.replication > 1) otherHostBm else sameHostBm
      val array = Array.fill(16)(Byte.MinValue to Byte.MaxValue).flatten
      val blockId = "list"
      bmToPutBlock.putIterator(
        blockId,
        List(array).iterator,
        storageLevel,
        tellMaster = true
      ...
      doAnswer { _ =>
        fail(
          "Fetching over network is not expected when the block is requested from same host"
        )
      }.when(sameHostTransferSrv)
        .fetchBlockSync(mc.any(), mc.any(), mc.any(), mc.any(), mc.any())
      val cleanBm =
        makeBlockManager(8000, "clean", master, Some(sameHostTransferSrv))

      // check getRemoteBytes
      val bytesViaStore1 = cleanBm.getRemoteBytes(blockId)
      assert(bytesViaStore1.isDefined)
      val expectedContent =
        sameHostBm.getLocalBlockData(blockId).nioByteBuffer().array()
  ...
  private def testWithFileDelAfterLocalDiskRead(
      level: StorageLevel,
      getValueOrBytes: Boolean
  ) = {
    val testedFunc =
      if (getValueOrBytes) "getRemoteValue()" else "getRemoteBytes()"
    val testNameSuffix = s"$level, $testedFunc"
    test(
      s"SPARK-27622: as file is removed fall back to network fetch, $testNameSuffix"
    ) {
      conf.set("spark.shuffle.io.maxRetries", "0")
      ...
      doAnswer { inv =>
        ...
        val res = store1.readDiskBlockFromSameHostExecutor(
          blockId,
          localDirs,
          blockSize
        )
        ...
        .readDiskBlockFromSameHostExecutor(mc.any(), mc.any(), mc.any())
  ...
  test("in-memory LRU for partitions of same RDD") {
    ...
    // from the same RDD
    assert(
      store.getSingleAndReleaseLock(rdd(0, 3)) === None,
      "rdd_0_3 was in store"
    )
    assert(
      store.getSingleAndReleaseLock(rdd(0, 2)).isDefined,
      "rdd_0_2 was not in store"
    )
    assert(
      store.getSingleAndReleaseLock(rdd(0, 1)).isDefined,
      "rdd_0_1 was not in store"
    )
  ...
  encryptionTest(
    "disk and memory storage with serialization and getLocalBytes"
  ) { _conf =>
    testDiskAndMemoryStorage(
      StorageLevel.MEMORY_AND_DISK_SER,
      getAsBytes = true,
  ...
  encryptionTest("disk and off-heap memory storage with getLocalBytes") {
    _conf =>
      testDiskAndMemoryStorage(
        StorageLevel.OFF_HEAP,
        getAsBytes = true,
  ...
  test("negative byte values in ByteBufferInputStream") {
    ...
    assert(
      stream.read(temp, 0, temp.length) === 2,
      "unexpected number of bytes read"
    )
    assert(stream.read() === -1, "end of stream not signalled")
    assert(
      stream.read(temp, 0, temp.length) === -1,
      "end of stream not signalled"
    )
  }
  ...
  test("block compression") {
    try {
      ...
      assert(
        store.memoryStore.getSize(rdd(0, 0)) <= 1000,
        "rdd_0_0 was not compressed"
      )
      stopBlockManager(store)
      ...
      assert(
        store.memoryStore.getSize(rdd(0, 0)) >= 10000,
        "rdd_0_0 was compressed"
      )
      stopBlockManager(store)
      ...
      assert(
        store.memoryStore.getSize("other_block") >= 10000,
        "other_block was compressed"
      )
      stopBlockManager(store)
    } finally {
  ...
  test("block store put failure") {
    ...
    failAfter(1.second) {
      assert(
        store.getSingleAndReleaseLock("a1").isEmpty,
        "a1 should not be in store"
      )
    }
  }
  ...
  def testPutBlockDataAsStream(
    ...
    assert(
      blockManager.getLocalBlockData(blockId).nioByteBuffer().array() === ser
    )
  }
  ...
  test("query block statuses") {
    ...
    assert(
      store.master
        .getBlockStatus("list1", askStorageEndpoints = false)
        .size === 0
    )
    assert(
      store.master
        .getBlockStatus("list2", askStorageEndpoints = false)
        .size === 1
    )
    assert(
      store.master
        .getBlockStatus("list3", askStorageEndpoints = false)
        .size === 1
    )
    assert(
      store.master
        .getBlockStatus("list1", askStorageEndpoints = true)
        .size === 0
    )
    assert(
      store.master
        .getBlockStatus("list2", askStorageEndpoints = true)
        .size === 1
    )
    assert(
      store.master
        .getBlockStatus("list3", askStorageEndpoints = true)
        .size === 1
    )
    ...
    assert(
      store.master
        .getBlockStatus("list4", askStorageEndpoints = false)
        .size === 0
    )
    assert(
      store.master
        .getBlockStatus("list5", askStorageEndpoints = false)
        .size === 0
    )
    assert(
      store.master
        .getBlockStatus("list6", askStorageEndpoints = false)
        .size === 0
    )
    assert(
      store.master
        .getBlockStatus("list4", askStorageEndpoints = true)
        .size === 0
    )
    assert(
      store.master
        .getBlockStatus("list5", askStorageEndpoints = true)
        .size === 1
    )
    assert(
      store.master
        .getBlockStatus("list6", askStorageEndpoints = true)
        .size === 1
    )
  }
  ...
  test("SPARK-1194 regression: fix the same-RDD rule for cache replacement") {
    ...
    assert(
      store.getSingleAndReleaseLock(rdd(1, 0)).isDefined,
      "rdd_1_0 was not in store"
    )
  ...
  test("safely unroll blocks through putIterator (disk)") {
    ...
    val result3 = memoryStore.putIteratorAsValues(
      "b3",
      smallIterator,
      MemoryMode.ON_HEAP,
      ClassTag.Any
    ...
    // b1, b2 and b4.
    val result4 = memoryStore.putIteratorAsValues(
      "b4",
      bigIterator,
      MemoryMode.ON_HEAP,
      ClassTag.Any
  ...
  private def testReadWithLossOfOnDiskFiles(
      storageLevel: StorageLevel,
      readMethod: BlockManager => Option[_]
  ): Unit = {
    val store = makeBlockManager(12000)
  ...
  test(
    ...
    testReadWithLossOfOnDiskFiles(
      StorageLevel.DISK_ONLY,
      _.getLocalBytes("blockId")
    )
    for (
      (readMethodName, readMethod) <- readMethods; storageLevel <- storageLevels
    ) {
  ...
  test(
    ...
    when(
      mockBlockManagerMaster.getLocationsAndStatus(
        mc.any[BlockId],
        mc.any[String]
      )
    ).thenReturn(
  ...
  test(
    ...
    val mockBlockTransferService =
      new MockBlockTransferService(maxFailures = 10) {
  ...
  test("SPARK-25888: serving of removed file not detected by shuffle service") {
    ...
    val emptyBlockFetcher = new MockBlockTransferService(0) {
      override def fetchBlockSync(
        ...
        val transConf =
          SparkTransportConf.fromSparkConf(conf, "shuffle", numUsableCores = 1)
        // empty ManagedBuffer
  ...
  test("test sorting of block locations") {
    ...
    val otherHost = "otherHost"
    val store = makeBlockManager(8000, "executor1")
    val externalShuffleServicePort =
    ...
    Utils.tryWithResource(new TransportContext(transConf, handler, true)) {
      transCtx =>
        ...
        def newShuffleServer(port: Int): (TransportServer, Int) = {
          (
            transCtx.createServer(
              port,
              Seq.empty[TransportServerBootstrap].asJava
            ),
            port
          )
        }
        ...
        val (server, shufflePort) = Utils.startServiceOnPort(
          candidatePort,
          newShuffleServer,
          conf,
          "ShuffleServer"
  ...
  test(
    ...
    assert(
      mockBlockTransferService.tempFileManager === store.remoteBlockTempFileManager
    )
  }
  ...
  test("query locations of blockIds") {
    val mockBlockManagerMaster = mock(classOf[BlockManagerMaster])
    val blockLocations =
      Seq(BlockManagerId("1", "host1", 100), BlockManagerId("2", "host2", 200))
    ...
    val locs =
      BlockManager.blockIdsToLocations(blockIds, env, mockBlockManagerMaster)
  ...
  test(
    ...
    assert(!result)
    verify(liveListenerBus, never()).post(
      SparkListenerBlockUpdated(BlockUpdatedInfo(updateInfo))
    )
  }
  ...
  test("test decommission block manager should not be part of peers") {
    ...
    assert(
      master.getPeers(store3.blockManagerId).map(_.executorId).toSet === Set(
        exec1,
        exec2
      )
    )
  ...
  test("test decommissionRddCacheBlocks should migrate all cached blocks") {
    ...
    assert(master.getLocations(blockId).size === 2)
    assert(
      master.getLocations(blockId).toSet === Set(
  ...
  test(
    "test decommissionRddCacheBlocks should keep the block if it is not able to migrate"
  ) {
  ...
  private def testShuffleBlockDecommissioning(
    ...
    Files.write(
      bm1.diskBlockManager.getFile(shuffleData).toPath(),
      shuffleDataBlockContent
    )
    Files.write(
      bm2.diskBlockManager.getFile(shuffleData2).toPath(),
      shuffleDataBlockContent
    )
    ...
    Files.write(
      bm1.diskBlockManager.getFile(shuffleIndex).toPath(),
      shuffleIndexBlockContent
    )
    Files.write(
      bm1.diskBlockManager.getFile(shuffleIndexOnly).toPath(),
      shuffleIndexBlockContent
    )
    Files.write(
      bm2.diskBlockManager.getFile(shuffleIndex2).toPath(),
      shuffleIndexBlockContent
    )
    ...
    try {
      ...
      mapOutputTracker.registerMapOutput(
        0,
        1,
        MapStatus(bm1.blockManagerId, Array(blockSize), 1)
      )
      assert(
        mapOutputTracker
          .shuffleStatuses(0)
          .mapStatuses(0)
          .location === bm1.blockManagerId
      )
      assert(
        mapOutputTracker
          .shuffleStatuses(0)
      ...
      if (willReject) {
        ...
        assert(
          Files.readAllBytes(
            bm2.diskBlockManager.getFile(shuffleIndex).toPath()
          )
            === shuffleIndexBlockContent
        )
      } else {
  ...
  test(
    "SPARK-32919: Prefer active executor locations for shuffle push mergers"
  ) {
    makeBlockManager(
      100,
      "execA",
    ...
    assert(
      master.getExecutorEndpointRef(SparkContext.DRIVER_IDENTIFIER).isEmpty
    )
    makeBlockManager(
      100,
      SparkContext.DRIVER_IDENTIFIER,
    ...
    assert(
      master.getExecutorEndpointRef(SparkContext.DRIVER_IDENTIFIER).isDefined
    )
  ...
  test("SPARK-33387 Support ordered shuffle block migration") {
    ...
    assert(
      sortedBlocks.sameElements(
        decomManager.shufflesToMigrate.asScala.map(_._1)
      )
    )
  }

  test(
    "SPARK-34193: Potential race condition during decommissioning with TorrentBroadcast"
  ) {
  ...
  test(
    "check KryoException when getting disk blocks and 'Input/output error' is occurred"
  ) {
    ...
    assert(
      kryoException.getMessage === "java.io.IOException: Input/output error"
    )
    assertUpdateBlockInfoReportedForRemovingBlock(
      store,
      "my-block-id",
  ...
  test(
    "SPARK-39647: Failure to register with ESS should prevent registering the BM"
  ) {
    val handler = new NoOpRpcHandler {
      override def receive(
          client: TransportClient,
  ...
  test(
    "SPARK-41497: getOrElseUpdateRDDBlock do compute based on cache visibility statue"
  ) {
    val store = makeBlockManager(8000, "executor1")
    ...
    assert(
      store.getStatus(blockId).isEmpty && !store.isRDDBlockVisible(blockId)
    )
    ...
    assert(
      store.getStatus(blockId).nonEmpty && !store.isRDDBlockVisible(blockId)
    )
  ...
  test(
    "SPARK-41497: master & manager interaction about rdd block visibility information"
  ) {
  ...
  test(
    "SPARK-41497: rdd block's visibility status should be cached once got from driver"
  ) {
    val store = makeBlockManager(8000, "executor1")
    val taskId = 0L
    val blockId = RDDBlockId(rddId = 1, splitIndex = 1)
  ...
  private def createKryoSerializerWithDiskCorruptedInputStream()
    ...
    class TestKryoSerializerInstance(
        ks: KryoSerializer,
        useUnsafe: Boolean,
        usePool: Boolean
    ) extends KryoSerializerInstance(ks, useUnsafe, usePool) {
      override def deserializeStream(s: InputStream): DeserializationStream = {
        new TestKryoDeserializationStream(
          this,
          new TestDiskCorruptedInputStream(),
          false
        )
      }
    }
    ...
    class TestKryoSerializer(conf: SparkConf) extends KryoSerializer(conf) {
      override def newInstance(): SerializerInstance = {
        new TestKryoSerializerInstance(
          this,
          conf.get(KRYO_USE_UNSAFE),
          conf.get(KRYO_USE_POOL)
        )
      }
    }
...
private object BlockManagerSuite {
  ...
  private implicit class BlockManagerTestUtils(store: BlockManager) {
    ...
    def dropFromMemoryIfExists(
        blockId: BlockId,
        data: () => Either[Array[Any], ChunkedByteBuffer]
    ): Unit = {
    ...
    val getLocalBytesAndReleaseLock: (BlockId) => Option[ChunkedByteBuffer] = {
      ...
      wrapGet { bid =>
        store.getLocalBytes(bid).map(_.toChunkedByteBuffer(allocator))
      }
    }
  }
