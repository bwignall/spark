  ...
  test("SPARK-33084: Add jar support Ivy URI -- test invalid ivy uri") {
    ...
    val e2 = intercept[IllegalArgumentException] {
      DependencyUtils.resolveMavenDependencies(
        URI.create("ivy://org.apache.test:test-test")
      )
    }.getMessage
    assert(
      e2.contains(
        "Invalid Ivy URI authority in uri ivy://org.apache.test:test-test: " +
    ...
    assert(
      e3.contains(
        "Invalid query string in Ivy URI " +
    ...
    assert(
      e5.contains(
        "Invalid exclude string in Ivy URI " +
