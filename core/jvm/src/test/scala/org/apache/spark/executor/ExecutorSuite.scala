...
class ExecutorSuite
  ...
  test(
    ...
    withExecutor("id", "localhost", env) { executor =>
      ...
      assert(
        executorSuiteHelper.testFailedReason.toErrorString === "TaskKilled (test)"
      )
  ...
  test(
    ...
    val conf =
      new SparkConf().setMaster("local").setAppName("executor suite test")
    ...
    val secondRDD = new FetchFailureHidingRDD(
      sc,
      inputRDD,
      throwOOM = false,
      interrupt = false
    ...
    val taskBinary =
      sc.broadcast(serializer.serialize((secondRDD, resultFunc)).array())
    val taskDescription =
      createResultTaskDescription(serializer, taskBinary, secondRDD, 1)
  ...
  test("SPARK-19276: OOMs correctly handled with a FetchFailure") {
    ...
    verify(uncaughtExceptionHandler).uncaughtException(
      any(),
  ...
  private def testFetchFailureHandling(
      oom: Boolean,
      poll: Boolean = false
  ): (TaskFailedReason, UncaughtExceptionHandler) = {
    ...
    val secondRDD =
      new FetchFailureHidingRDD(sc, inputRDD, throwOOM = oom, interrupt = !oom)
    val taskBinary =
      sc.broadcast(serializer.serialize((secondRDD, resultFunc)).array())
    val taskDescription =
      createResultTaskDescription(serializer, taskBinary, secondRDD, 1)

    runTaskGetFailReasonAndExceptionHandler(
      taskDescription,
      killTask = !oom,
      poll
    )
  ...
  test("Gracefully handle error in task deserialization") {
    ...
    val env = createMockEnv(conf, serializer)
    val serializedTask =
      serializer.newInstance().serialize(new NonDeserializableTask)
    val taskDescription = createFakeTaskDescription(serializedTask)
  ...
  test("SPARK-39696: Using accumulators should not cause heartbeat to fail") {
    ...
    val accums =
      (1 to 10).map(i => sc.longAccumulator(s"mapperRunAccumulator$i"))
  ...
  private def withHeartbeatExecutor(
      confs: (String, String)*
  ...
  private def heartbeatZeroAccumulatorUpdateTest(
      dropZeroMetrics: Boolean
  ): Unit = {
    val c =
      EXECUTOR_HEARTBEAT_DROP_ZERO_ACCUMULATOR_UPDATES.key -> dropZeroMetrics.toString
    withHeartbeatExecutor(c) { (executor, heartbeats) =>
      ...
      assert(
        heartbeats(0).accumUpdates.length == 0,
      ...
      assert(
        updates.length == 1 && updates(0)._1 == 6,
      ...
      if (dropZeroMetrics) {
        assert(
          accumsSent == metrics.accumulators().count(!_.isZero),
          "The number of accumulators sent should match the number of nonzero accumulators"
        )
      } else {
        assert(
          accumsSent == metrics.accumulators().length,
  ...
  test("Send task executor metrics in DirectTaskResult") {
    ...
    val taskBinary =
      sc.broadcast(serializer.serialize((rdd, resultFunc)).array())
    val taskDescription =
      createResultTaskDescription(serializer, taskBinary, rdd, 0)
  ...
  test("Send task executor metrics in ExceptionFailure") {
    ...
    val metrics =
      taskFailedReason.asInstanceOf[ExceptionFailure].metricPeaks.toArray
    val taskMetrics = new ExecutorMetrics(metrics)
    assert(taskMetrics.getMetricValue("JVMHeapMemory") > 0)
  }

  test(
    "SPARK-34949: do not re-register BlockManager when executor is shutting down"
  ) {
    ...
    withExecutor("id", "localhost", mockEnv) { executor =>
      withMockHeartbeatReceiverRef(executor) { mockReceiverRef =>
        ...
        executor.invokePrivate(reportHeartbeat())
        assert(
          reregisterInvoked.get(),
          "BlockManager.reregister should be invoked " +
  ...
  test(
    "SPARK-40235: updateDependencies is interruptible when waiting on lock"
  ) {
    val conf = new SparkConf
    val serializer = new JavaSerializer(conf)
    val env = createMockEnv(conf, serializer)
    withExecutor("id", "localhost", env) { executor =>
      ...
      eventually(timeout(10.seconds), interval(100.millis)) {
        ...
        assert(
          Set(Thread.State.BLOCKED, Thread.State.WAITING).contains(threadState)
        )
      }
  ...
  private def createFakeTaskDescription(
  ...
  private def runTaskGetFailReasonAndExceptionHandler(
      ...
      if (killTask) {
        val killingThread = new Thread("kill-task") {
          override def run(): Unit = {
            ...
            if (
              ExecutorSuiteHelper.latches.latch1.await(10, TimeUnit.SECONDS)
            ) {
              ...
              executor.killAllTasks(
                true,
                "Killed task, e.g. because of speculative execution"
              )
            } else {
              timedOut.set(true)
            }
    ...
    val failReason =
      SparkEnv.get.closureSerializer
        .newInstance()
        .deserialize[TaskFailedReason](failureData)
      ...
      case t: Throwable =>
        if (throwOOM) {
          ...
          throw new RuntimeException(
            "User Exception that hides the original exception",
            t
          )
        }
    }
