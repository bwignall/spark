...
class RadixSortSuite extends SparkFunSuite {
  ...
  val SORT_TYPES_TO_TEST = Seq(
    RadixSortType(
      "unsigned binary data asc nulls first",
    ...
    RadixSortType(
      "unsigned binary data asc nulls last",
    ...
    RadixSortType(
      "unsigned binary data desc nulls last",
    ...
    RadixSortType(
      "unsigned binary data desc nulls first",
    ...
    RadixSortType(
      "twos complement asc nulls first",
    ...
    RadixSortType(
      "twos complement asc nulls last",
    ...
    RadixSortType(
      "twos complement desc nulls last",
    ...
    RadixSortType(
      "twos complement desc nulls first",
  ...
  private def referenceKeyPrefixSort(
      buf: LongArray,
      lo: Long,
      hi: Long,
      refCmp: PrefixComparator
  ): Unit = {
    val sortBuffer = new LongArray(
      MemoryBlock.fromLongArray(new Array[Long](buf.size().toInt))
    )
  ...
  for (sortType <- SORT_TYPES_TO_TEST) {
    test("radix support for " + sortType.name) {
      val s = sortType.referenceComparator
        .asInstanceOf[PrefixComparators.RadixSortSupport]
    ...
    fuzzTest(s"fuzz test key prefix ${sortType.name} with random bitmasks") {
      seed =>
        val rand = new XORShiftRandom(seed)
        val mask = randomBitMask(rand)
