...
/** This's an end to end test suite used to test the host local shuffle reading.
  */
class HostLocalShuffleReadingSuite
  ...
  Seq(true, false).foreach { isESSEnabled =>
    ...
    test(
      s"host local shuffle reading with external shuffle service $essStatus"
    ) {
      conf
        .set(SHUFFLE_SERVICE_ENABLED, isESSEnabled)
        .set(STORAGE_LOCAL_DISK_BY_EXECUTORS_CACHE_SIZE, 5)
      sc = new SparkContext(
        "local-cluster[2,1,1024]",
        "test-host-local-shuffle-reading",
        conf
      )
      ...
      sc.env.blockManager.externalShuffleServiceEnabled should equal(
        isESSEnabled
      )
      ...
      val cachedExecutors = rdd
        .mapPartitions { _ =>
          SparkEnv.get.blockManager.hostLocalDirManager
            .map { localDirManager =>
      ...
      val (local, remote) = rdd
        .map { case (_, _) =>
          val shuffleReadMetrics =
            TaskContext.get().taskMetrics().shuffleReadMetrics
          (
            (
