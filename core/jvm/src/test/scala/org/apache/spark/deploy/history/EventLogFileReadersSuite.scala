...
abstract class EventLogFileReadersSuite
  ...
  protected val fileSystem =
    Utils.getHadoopFileSystem("/", SparkHadoopUtil.get.conf)
  ...
  test("Retrieve EventLogFileReader correctly") {
    def assertInstanceOfEventLogReader(
        ...
        actual: Option[EventLogFileReader]
    ): Unit = {
      if (expectedClazz.isEmpty) {
        assert(
          actual.isEmpty,
          s"Expected no EventLogFileReader instance but was " +
            s"${actual.map(_.getClass).getOrElse("<None>")}"
        )
      } else {
        assert(
          actual.isDefined,
          s"Expected an EventLogFileReader instance but was empty"
        )
        assert(
          expectedClazz.get.isAssignableFrom(actual.get.getClass),
    ...
    def testCreateEventLogReaderWithPath(
      ...
      val reader2 =
        EventLogFileReader(fileSystem, fileSystem.getFileStatus(path))
      assertInstanceOfEventLogReader(expectedClazz, reader2)
    }

    // path with no last index - single event log
    val reader1 =
      EventLogFileReader(fileSystem, new Path(testDirPath, "aaa"), None)
    assertInstanceOfEventLogReader(
      Some(classOf[SingleFileEventLogFileReader]),
      Some(reader1)
    )

    // path with last index - rolling event log
    val reader2 = EventLogFileReader(
      fileSystem,
      new Path(testDirPath, s"${EVENT_LOG_DIR_NAME_PREFIX}aaa"),
      Some(3)
    )
    assertInstanceOfEventLogReader(
    ...
    testCreateEventLogReaderWithPath(
      eventLogFile,
      isFile = true,
      Some(classOf[SingleFileEventLogFileReader])
    )
  ...
  allCodecs.foreach { codecShortName =>
    test(
      s"get information, list event log files, zip log files - with codec $codecShortName"
    ) {
      ...
      val writer =
        createWriter(appId, attemptId, testDirPath.toUri, conf, hadoopConf)
      writer.start()
      ...
      val logPathIncompleted =
        getCurrentLogPath(writer.logPath, isCompleted = false)
      val readerOpt =
        EventLogFileReader(fileSystem, new Path(logPathIncompleted))
      ...
      val logPathCompleted =
        getCurrentLogPath(writer.logPath, isCompleted = true)
      val readerOpt2 =
        EventLogFileReader(fileSystem, new Path(logPathCompleted))
      assertAppropriateReader(readerOpt2)
      val reader2 = readerOpt2.get

      verifyReader(
        reader2,
        new Path(logPathCompleted),
        codecShortName,
        isCompleted = true
      )
    }
  ...
  protected def assertAppropriateReader(
      actualReader: Option[EventLogFileReader]
  ): Unit
...
class SingleFileEventLogFileReaderSuite extends EventLogFileReadersSuite {
  ...
  override protected def assertAppropriateReader(
    ...
    assert(
      actualReader.isDefined,
      s"Expected an EventLogReader instance but was empty"
    )
    assert(
      actualReader.get.isInstanceOf[SingleFileEventLogFileReader],
      s"Expected SingleFileEventLogReader but was ${actualReader.get.getClass}"
    )
  }

  override protected def getCurrentLogPath(
      logPath: String,
      isCompleted: Boolean
  ): String = {
  ...
  override protected def verifyReader(
    ...
    assert(
      reader.listEventLogFiles.map(_.getPath.toUri.getPath) ===
    ...
    Utils.tryWithResource(
      ...
      val expected =
        Files.toString(new File(logPath.toString), StandardCharsets.UTF_8)
...
class RollingEventLogFilesReaderSuite extends EventLogFileReadersSuite {
  test("SPARK-46012: appStatus file should exist") {
    withTempDir { dir =>
      ...
      val appStatusFile = fileSystem
        .listStatus(new Path(logPathCompleted))
        .find(RollingEventLogFilesWriter.isAppStatusFile)
        .get
        .getPath
  ...
  override protected def assertAppropriateReader(
    ...
    assert(
      actualReader.get.isInstanceOf[RollingEventLogFilesFileReader],
  ...
  override protected def verifyReader(
    ...
    assert(reader.rootPath === fileSystem.makeQualified(logPath))
    assert(
      reader.lastIndex === Some(
        getEventLogFileIndex(lastEventFile.getPath.getName)
      )
    )
    ...
    assert(
      reader.listEventLogFiles.map(_.getPath) === eventFiles.map(_.getPath)
    )
    assert(reader.compressionCodec === compressionCodecShortName)
    ...
    Utils.tryWithResource(
      ...
      val allFileNames =
        fileSystem.listStatus(logPath).map(_.getPath.getName).toSet
