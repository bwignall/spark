...
class OutputCommitCoordinatorSuite extends SparkFunSuite with BeforeAndAfter {
  ...
  before {
    ...
    sc = new SparkContext(conf) {
      override private[spark] def createSparkEnv(
        ...
        outputCommitCoordinator = spy[OutputCommitCoordinator](
    ...
    val mockTaskScheduler =
      spy[TaskSchedulerImpl](sc.taskScheduler.asInstanceOf[TaskSchedulerImpl])
  ...
  test("Job should not complete if all commits are denied") {
    ...
    doReturn(false)
      ...
      .handleAskPermissionToCommit(any(), any(), any(), any())
...
private case class OutputCommitFunctions(tempDirPath: String) {
  ...
  private val jobId = new SerializableWritable(
    SparkHadoopWriterUtils.createJobID(new Date, 0)
  )
  ...
  private def runCommitWithProvidedCommitter(
    ...
    val attemptId = new TaskAttemptID(
      new TaskID(jobId.value, TaskType.MAP, ctx.partitionId()),
      taskAttemptId
    )
