...
class SparkContextSuite
  ...
  test("basic case for addFile and listFiles") {
    withTempDir { dir =>
      ...
      try {
        ...
        sc.parallelize(Array(1).toImmutableArraySeq, 1)
          .map(x => {
        ...
        sc.parallelize(Array(1).toImmutableArraySeq, 1)
          .map { x =>
            ...
            Seq(gotten1, gotten2, gotten3, gotten4, gotten5).foreach { gotten =>
              if (!gotten.exists()) {
                throw new SparkException(
                  s"The archive doesn't exist: ${gotten.getAbsolutePath}"
                )
              }
              if (!gotten.isDirectory) {
                throw new SparkException(
                  s"The archive was not unpacked: ${gotten.getAbsolutePath}"
                )
              }
            }
      ...
      try {
        ...
        sc.parallelize(Array(1).toImmutableArraySeq, 1)
          .map(x => {
            ...
            if (
              !new File(SparkFiles.get(neptune.getName + sep + alien1.getName))
                .exists()
            ) {
              throw new SparkException(
  ...
  test(
    ...
    withTempDir { dir =>
      try {
        val sep = File.separator
        val tmpDir =
          Utils.createTempDir(dir.getAbsolutePath + sep + "test space")
        ...
        assert(
          sc.listFiles()
            .head
            .contains(new Path(tmpConfFile1.getName).toUri.toString)
        )
      } finally {
        sc.stop()
      }
    ...
    withTempDir { dir =>
      try {
        ...
        assert(
          sc.listFiles()
            .head
            .contains(new Path(tmpConfFile2.getName).toUri.toString)
        )
      } finally {
        sc.stop()
      }
  ...
  for (
    ...
    test(
      s"$method can be called twice with same file in $schedulingMode (SPARK-16787)"
    ) {
  ...
  test("Cancelling stages/jobs with custom reasons.") {
    ...
    for (cancelWhat <- Seq("stage", "job")) {
      ...
      val ex = intercept[SparkException] {
        sc.range(0, 10000L, numSlices = 10)
          .mapPartitions { x =>
            x.synchronized {
              x.wait()
            }
      ...
      ex.getCause() match {
        ...
        case cause: Throwable =>
          fail(
            "Expected the cause to be SparkException, got " + cause
              .toString() + " instead."
          )
      }
      ...
      eventually(timeout(20.seconds)) {
        assert(
          sc.statusTracker.getExecutorInfos.map(_.numRunningTasks()).sum == 0
        )
      }
  ...
  def testCancellingTasks(desc: String)(blockFn: => Unit): Unit =
    test(s"Killing tasks $desc") {
      sc =
        new SparkContext(new SparkConf().setAppName("test").setMaster("local"))
      ...
      val listener = new SparkListener {
        ...
        override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = {
          if (
            taskEnd.taskInfo.attemptNumber == 1 && taskEnd.reason == Success
          ) {
  ...
  test(
    "SPARK-19446: DebugFilesystem.assertNoOpenStreams should report " +
  ...
  test("support barrier execution mode under local mode") {
    ...
    val rdd2 = rdd.barrier().mapPartitions { it =>
      ...
      if (context.getTaskInfos().length != 2) {
        throw new SparkException(
          "Expected taksInfos length is 2, actual length is " +
  ...
  test(s"Avoid setting ${CPUS_PER_TASK.key} unreasonably (SPARK-27192)") {
    ...
    Seq(
      ...
      executorCores.map(executorCores =>
        conf.set(EXECUTOR_CORES, executorCores)
      )
      val ex = intercept[SparkException] {
  ...
  test("Test parsing resources task configs with missing executor config") {
    ...
    assert(
      error.contains(
        "No executor resource configs were specified for the following " +
          "task configs: gpu"
      )
    )
  }
  ...
  test("Test parsing resources executor config < task requirements") {
    ...
    assert(
      error.contains(
        "The executor resource: gpu, amount: 1 needs to be >= the task " +
    ...
    sc.range(0, 1).foreach { _ =>
      new SparkContext(
        new SparkConf()
          .setAppName("test")
          .setMaster("local")
          .set(EXECUTOR_ALLOW_SPARK_CONTEXT, true)
      ).stop()
    }
  }
  ...
  test("SPARK-33084: Add jar support Ivy URI -- default transitive = true") {
    ...
    assert(
      sc.listJars()
        .exists(_.contains("org.apache.hive_hive-storage-api-2.7.0.jar"))
    )
    assert(
      sc.listJars().exists(_.contains("commons-lang_commons-lang-2.6.jar"))
    )
  }
  ...
  test(
    ...
    val logAppender =
      new LogAppender("transitive=true will download dependency jars")
    withLogAppender(logAppender) {
      ...
      eventually(timeout(10.seconds), interval(1.second)) {
        ...
        val existMsg = logAppender.loggingEvents
          .filter(
            _.getMessage.getFormattedMessage.contains(
              "The dependency jars of Ivy URI " +
                "ivy://org.apache.hive:hive-storage-api:2.7.0?transitive=true"
  ...
  test(
    "SPARK-34506: Add jar support Ivy URI -- transitive=false will not download " +
      "dependency jars"
  ) {
    sc = new SparkContext(
      new SparkConf().setAppName("test").setMaster("local-cluster[3, 1, 1024]")
  ...
  test(
    ...
    sc.addJar(
      "ivy://org.apache.hive:hive-storage-api:2.7.0?exclude=commons-lang:commons-lang"
    )
    assert(
      sc.listJars()
        .exists(_.contains("org.apache.hive_hive-storage-api-2.7.0.jar"))
  ...
  test(
    ...
    sc.addJar(
      "ivy://org.apache.hive:hive-storage-api:2.7.0" +
  ...
  test("SPARK-33084: Add jar support Ivy URI -- test different version") {
    ...
    assert(
      sc.listJars()
        .exists(_.contains("org.apache.hive_hive-storage-api-2.6.0.jar"))
    )
  }
  ...
  test(
    ...
    assert(
      sc.hadoopConfiguration.get(testKey) === "/tmp/hive_one",
      "hive configs have higher priority than hadoop ones "
    )
    assert(
      sc.hadoopConfiguration.get(bufferKey).toInt === 65536,
    ...
    sc = new SparkContext(sparkConf)
    assert(
      sc.hadoopConfiguration.get(testKey) === "/tmp/hive_two",
  ...
  test("SPARK-35383: Fill missing S3A magic committer configs if needed") {
    ...
    resetSparkContext()
    val c2 = c1.clone.set(
      "spark.hadoop.fs.s3a.bucket.mybucket.committer.magic.enabled",
      "false"
    )
    ...
    withTempDir { dir =>
      try {
        ...
        val tmpCanonicalDir =
          Utils.createTempDir(dir.getAbsolutePath + sep + "test space")
        val tmpAbsoluteDir =
          new File(tmpCanonicalDir.getAbsolutePath + sep + '.' + sep)
        ...
        assert(
          tmpAbsoluteDir.getAbsolutePath !== tmpAbsoluteDir.getCanonicalPath
        )
  ...
  test(
    "SPARK-39957: ExitCode HEARTBEAT_FAILURE should be counted as network failure"
  ) {
    ...
    val sc = new SparkContext(
      ...
      "test-exit-code-heartbeat",
      conf
    )
    val result = sc
      .parallelize(1 to 10, 1)
      .map { x =>
        val context = org.apache.spark.TaskContext.get()
  ...
  test(
    "SPARK-39957: ExitCode HEARTBEAT_FAILURE will be counted as task failure when" +
    ...
    val conf = new SparkConf()
      ...
      .set(EXECUTOR_REMOVE_DELAY.key, "0s")
    val sc = new SparkContext(
      "local-cluster[1, 1, 1024]",
      "test-exit-code-heartbeat",
      conf
  ...
  test(
    "SPARK-42689: ShuffleDataIO initialized after application id has been configured"
  ) {
    val conf = new SparkConf().setAppName("test").setMaster("local")
    // TestShuffleDataIO will validate if application id has been configured in its constructor
    conf.set(
      SHUFFLE_IO_PLUGIN_CLASS.key,
