...
/** This test suite covers all the cases that shall fail fast on job submitted that contains one
  ...
  */
class BarrierStageOnSubmittedSuite
  ...
  test("submit a barrier ResultStage that contains PartitionPruningRDD") {
    ...
    val prunedRdd =
      new PartitionPruningRDD(sc.parallelize(1 to 10, 4), index => index > 1)
    val rdd = prunedRdd
      .barrier()
  ...
  test(
    "submit a barrier ResultStage that requires more slots than current total under local " +
      "mode"
  ) {
    val conf = new SparkConf()
  ...
  test(
    "submit a barrier ShuffleMapStage that requires more slots than current total under " +
      "local mode"
  ) {
    val conf = new SparkConf()
      // Shorten the time interval between two failed checks to make the test fail faster.
  ...
  test(
    "submit a barrier ResultStage that requires more slots than current total under " +
      "local-cluster mode"
  ) {
    val conf = new SparkConf()
      .set(CPUS_PER_TASK, 2)
  ...
  test(
    "SPARK-32518: CoarseGrainedSchedulerBackend.maxNumConcurrentTasks should " +
