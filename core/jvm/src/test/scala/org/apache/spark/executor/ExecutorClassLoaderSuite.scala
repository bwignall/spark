...
class ExecutorClassLoaderSuite
  ...
  test("child over system classloader") {
    ...
    ToolProvider.getSystemJavaCompiler
      .getTask(
        ...
        null,
        null,
        Collections.singletonList(scalaOptionFile)
      )
      .call()
    ...
    assert(
      optionClass.getClassLoader == classLoader,
  ...
  test("child first") {
    ...
    val classLoader =
      new ExecutorClassLoader(new SparkConf(), null, url1, parentLoader, true)
    val fakeClass =
      classLoader.loadClass("ReplFakeClass2").getConstructor().newInstance()
  ...
  test("parent first") {
    ...
    val fakeClass =
      classLoader.loadClass("ReplFakeClass1").getConstructor().newInstance()
    val fakeClassVersion = fakeClass.toString
  ...
  test("child first can fall back") {
    ...
    val classLoader =
      new ExecutorClassLoader(new SparkConf(), null, url1, parentLoader, true)
    val fakeClass =
      classLoader.loadClass("ReplFakeClass3").getConstructor().newInstance()
    val fakeClassVersion = fakeClass.toString
    assert(fakeClassVersion === "2")
  }
  ...
  test("child first can fail") {
    ...
    intercept[java.lang.ClassNotFoundException] {
      classLoader
        .loadClass("ReplFakeClassDoesNotExist")
        .getConstructor()
        .newInstance()
    }
  }
  ...
  test("resource from parent") {
    ...
    val bufferedSource =
      Source.fromInputStream(is, StandardCharsets.UTF_8.name())
  ...
  test("resources from parent") {
    ...
    val bufferedSource =
      Source.fromInputStream(resources.nextElement().openStream())
    Utils.tryWithSafeFinally {
  ...
  test("fetch classes using Spark's RpcEnv") {
      ...
      (invocation: InvocationOnMock) => {
        ...
        if (path.toFile.exists()) {
          ...
          when(channel.read(any()))
            .thenThrow(
              new RuntimeException(s"Stream '${uri.getPath}' was not found.")
            )
          channel
        }
      }
    ...
    val fakeClassInputStream =
      classLoader.getResourceAsStream("ReplFakeClass2.class")
    try {
  ...
  test(
    "SPARK-20547 ExecutorClassLoader should not throw ClassNotFoundException without " +
    ...
    try {
      ...
      val sourceCodeOfClassA =
        """public class TestClassA implements java.io.Serializable {
          ...
          |}""".stripMargin
      val sourceFileA =
        new JavaSourceFromString("TestClassA", sourceCodeOfClassA)
      TestUtils.createCompiledClass(
        sourceFileA.name,
        tempDir,
        sourceFileA,
        Seq(tempDir.toURI.toURL)
      )
      ...
      val sourceCodeOfClassB =
        ...
        |}""".stripMargin
      val sourceFileB =
        new JavaSourceFromString("TestClassB", sourceCodeOfClassB)
      TestUtils.createCompiledClass(
        sourceFileB.name,
        tempDir,
        sourceFileB,
