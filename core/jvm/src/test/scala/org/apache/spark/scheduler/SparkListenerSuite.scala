...
class SparkListenerSuite
  ...
  private def numDroppedEvents(bus: LiveListenerBus): Long = {
    bus.metrics.metricRegistry
      .counter(s"queue.$SHARED_QUEUE.numDroppedEvents")
      .getCount
  }
  ...
  private def eventProcessingTimeCount(bus: LiveListenerBus): Long = {
    bus.metrics.metricRegistry
      .timer(s"queue.$SHARED_QUEUE.listenerProcessingTime")
      .getCount()
  }
  ...
  test("SPARK-46383: Track TaskInfo objects") {
    ...
    // `onTaskEnd` events.
    val conf =
      new SparkConf().set(DROP_TASK_INFO_ACCUMULABLES_ON_TASK_COMPLETION, true)
  ...
  test("local metrics") {
    ...
    listener.stageInfos.foreach { case (stageInfo, taskInfoMetrics) =>
      ...
      taskInfoMetrics.foreach { case (taskInfo, taskMetrics) =>
        ...
        if (
          stageInfo.rddInfos
            .exists(info => info.name == d2.name || info.name == d3.name)
        ) {
          assert(taskMetrics.shuffleWriteMetrics.bytesWritten > 0L)
        }
        if (stageInfo.rddInfos.exists(_.name == d4.name)) {
          assert(
            taskMetrics.shuffleReadMetrics.totalBlocksFetched == 2 * numSlices
          )
          assert(
            taskMetrics.shuffleReadMetrics.localBlocksFetched == 2 * numSlices
          )
  ...
  test(
    ...
    listener.synchronized {
      var remainingWait = finishTime - System.currentTimeMillis
      while (
        listener.endedTasks.size < listener.startedTasks.size && remainingWait > 0
      ) {
  ...
  Seq(true, false).foreach { throwInterruptedException =>
    val suffix =
      if (throwInterruptedException) "throw interrupt"
      else "set Thread interrupted"
    test(s"interrupt within listener is handled correctly: $suffix") {
      ...
      val counter1 = new BasicJobCounter()
      val counter2 = new BasicJobCounter()
      val interruptingListener1 =
        new InterruptingListener(throwInterruptedException)
      val interruptingListener2 =
        new InterruptingListener(throwInterruptedException)
      bus.addToSharedQueue(counter1)
  ...
  Seq(true, false).foreach { throwInterruptedException =>
    ...
    test(
      s"SPARK-30285: Fix deadlock in AsyncEventQueue.removeListenerOnError: $suffix"
    ) {
      val LISTENER_BUS_STOP_WAITING_TIMEOUT_MILLIS = 10 * 1000L // 10 seconds
      val bus = new LiveListenerBus(new SparkConf(false))
      val counter1 = new BasicJobCounter()
      val counter2 = new BasicJobCounter()
      val interruptingListener =
        new DelayInterruptingJobCounter(throwInterruptedException, 3)
      bus.addToSharedQueue(counter1)
  ...
  test("event queue size can be configured through spark conf") {
    ...
    val conf = new SparkConf(false)
      .set(LISTENER_BUS_EVENT_QUEUE_CAPACITY, 5)
      .set(
        s"spark.scheduler.listenerbus.eventqueue.${SHARED_QUEUE}.capacity",
        "1"
      )
      .set(
        s"spark.scheduler.listenerbus.eventqueue.${EVENT_LOG_QUEUE}.capacity",
        "2"
      )
    ...
    assert(
      bus
        .activeQueues() === Set(SHARED_QUEUE, APP_STATUS_QUEUE, EVENT_LOG_QUEUE)
    )
  ...
  test(
    "SPARK-39973: Suppress error logs when the number of timers is set to 0"
  ) {
    sc = new SparkContext(
      "local",
      "SparkListenerSuite",
      new SparkConf()
        .set(LISTENER_BUS_METRICS_MAX_LISTENER_CLASSES_TIMED.key, 0.toString)
    )
