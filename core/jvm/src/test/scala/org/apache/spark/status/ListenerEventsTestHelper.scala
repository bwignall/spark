...
object ListenerEventsTestHelper {
  ...
  def createExecutorRemovedEvent(
      executorId: Int
  ): SparkListenerExecutorRemoved = {
  ...
  def createExecutorMetricsUpdateEvent(
    ...
    val executorUpdates = Map(
      (stageId, 0) -> new ExecutorMetrics(executorMetrics)
    )
    SparkListenerExecutorMetricsUpdate(
      executorId.toString,
  ...
  def pushJobEventsWithoutJobEnd(
    ...
    listener.onJobStart(
      SparkListenerJobStart(jobId, time, Seq(stage), jobProps)
    )
    ...
    JobInfo(
      Seq(stage.stageId),
      Map(stage.stageId -> s1Tasks.map(_.taskId)),
