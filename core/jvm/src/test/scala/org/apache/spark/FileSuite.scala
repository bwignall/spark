...
class FileSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("text files") {
    ...
    Utils.tryWithSafeFinally {
      ...
      assert(
        sc.textFile(outputDir).collect().toList === List("1", "2", "3", "4")
      )
    } {
      bufferSrc.close()
    }
  ...
  test("text files (compressed)") {
    ...
    val compressedFile =
      new File(compressedOutputDir, "part-00000" + codec.getDefaultExtension)
  ...
  test("text files do not allow null rows") {
    ...
    assert(
      Utils
        ...
        .contains("text files do not allow null rows")
    )
  }
  ...
  def runSequenceFileCodecTest(
    ...
    test(s"SequenceFile (compressed) - $codecName") {
      ...
      val compressedContent =
        sc.sequenceFile[String, String](compressedOutputDir).collect()
  ...
  private val codecs =
    Seq((new DefaultCodec(), "default"), (new BZip2Codec(), "bzip2")) ++ {
      try {
  ...
  test("object files of classes from a JAR") {
    ...
    Utils.withContextClassLoader(loader) {
      ...
      val objs = sc.makeRDD(1 to 3).map { _ =>
        Utils
          .classForName[AnyRef](className, noSparkClassLoader = true)
          .getConstructor()
          .newInstance()
      }
      val outputDir = new File(tempDir, "output").getAbsolutePath
      ...
      // Try reading the output back as an object file
      val ct = reflect.ClassTag[Any](
        Utils.classForName(className, noSparkClassLoader = true)
      )
  ...
  test("read SequenceFile using new Hadoop API") {
    ...
    nums.saveAsSequenceFile(outputDir)
    val output =
      sc.newAPIHadoopFile[IntWritable, Text, SequenceFileInputFormat[
        IntWritable,
        Text
      ]](outputDir)
    assert(
  ...
  private def writeBinaryData(
      testOutput: Array[Byte],
      testOutputCopies: Int
  ): File = {
  ...
  test("portabledatastream persist disk storage") {
    ...
    val outFile = writeBinaryData(testOutput, 1)
    val inRdd =
      sc.binaryFiles(outFile.getAbsolutePath).persist(StorageLevel.DISK_ONLY)
    inRdd.foreach(_._2.toArray()) // force the file to read
    assert(inRdd.values.collect().head.toArray() === testOutput)
  }
  ...
  test("portabledatastream flatmap tests") {
    ...
    val numOfCopies = 3
    val copyRdd =
      inRdd.flatMap(curData => (0 until numOfCopies).map(_ => curData._2))
  ...
  test("SPARK-22357 test binaryFiles minPartitions") {
    ...
    withTempDir { tempDir =>
      ...
      for (p <- Seq(1, 2, 8)) {
        assert(
          sc.binaryFiles(tempDirPath, minPartitions = p).getNumPartitions === p
        )
      }
    }
  }
  ...
  test("prevent user from overwriting the empty directory (old Hadoop API)") {
    sc = new SparkContext("local", "test")
    val randomRDD =
      sc.parallelize(Seq((1, "a"), (1, "a"), (2, "b"), (3, "c")), 1)
  ...
  test("prevent user from overwriting the empty directory (new Hadoop API)") {
    ...
    val randomRDD = sc.parallelize(
      Seq(("key1", "a"), ("key2", "a"), ("key3", "b"), ("key4", "c")),
      1
    )
    intercept[FileAlreadyExistsException] {
  ...
  test("save Hadoop Dataset through old Hadoop API") {
    ...
    job.set(
      ...
      tempDir.getPath + "/outputDataset_old"
    )
  ...
  test("spark.hadoopRDD.ignoreEmptySplits work correctly (new Hadoop API)") {
    ...
    def testIgnoreEmptySplits(
      ...
      val output = new File(tempDir, "output")
      sc.parallelize(data, actualPartitionNum)
        .saveAsNewAPIHadoopFile[NewTextOutputFormat[String, String]](
          output.getPath
        )
      for (i <- 0 until actualPartitionNum) {
  ...
  test(
    ...
    def collectRDDAndDeleteFileBeforeCompute(newApi: Boolean): Array[_] = {
      ...
      val writer = new OutputStreamWriter(
        new FileOutputStream(new File(dataPath.toString))
      )
      writer.write("hello\n")
      writer.write("world\n")
      writer.close()
  ...
  test("SPARK-25100: Support commit tasks when Kyro registration is required") {
    ...
    assert(
      sc.textFile(outputFilePath1).collect() === Array(
        "(1,1)",
        "(2,2)",
        "(3,3)"
      )
    ...
    assert(
      sc.textFile(outputFilePath2).collect() === Array("1\t1", "2\t2", "3\t3")
    )
  }
}
