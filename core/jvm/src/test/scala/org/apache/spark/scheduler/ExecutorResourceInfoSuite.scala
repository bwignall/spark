...
class ExecutorResourceInfoSuite
    extends SparkFunSuite
    with ExecutorResourceUtils {

  implicit def convertMapLongToDouble(
  ...
  implicit def convertMapDoubleToLong(
  ...
  test("Don't allow acquire address that is not available") {
    ...
    assert(
      e.getMessage.contains(
        "Try to acquire gpu address 1 amount: 1.0, but only 0.0 left."
      )
    )
  }
  ...
  test("Don't allow acquire address that doesn't exist") {
    ...
    assert(
      e.getMessage.contains("Try to acquire an address that doesn't exist.")
    )
  }
  ...
  test("Don't allow release address that is not assigned") {
    ...
    assert(
      e.getMessage.contains(
        "Try to release gpu address 2 amount: 1.0. " +
  ...
  test("Don't allow release address that doesn't exist") {
    ...
    assert(
      e.getMessage.contains("Try to release an address that doesn't exist.")
    )
  }
  ...
  test("assign/release resource for different task requirements") {
    ...
    def testAllocation(
        taskAddressAmount: Map[String, Double],
    ...
    testAllocation(
      taskAddressAmount = Map("1" -> 1.0, "2" -> 1.0),
    ...
    testAllocation(
      taskAddressAmount = Map("0" -> 1.0, "1" -> 1.0),
    ...
    testAllocation(
      taskAddressAmount = Map("0" -> 1.0, "1" -> 1.0, "2" -> 1.0, "3" -> 1.0),
