...
private[spark] class Benchmark(
  ...
  /** Adds a case with manual timing control. When the function is run, timing does not start
    ...
    * @param numIters if non-zero, forces exactly this many iterations to be run
    */
  def addTimerCase(name: String, numIters: Int = 0)(
      f: Benchmark.Timer => Unit
  ): Unit = {
  ...
  def run(): Unit = {
    ...
    val nameLen =
      Math.max(40, Math.max(name.length, benchmarks.map(_.name.length).max))
    out.printf(
  ...
  def measure(num: Long, overrideNumIters: Int)(f: Timer => Unit): Result = {
    ...
    while (i < minIters || totalTime < minDuration) {
      ...
      if (outputPerIteration) {
        // scalastyle:off
        println(
          s"Iteration $i took ${NANOSECONDS.toMicros(runTime)} microseconds"
        )
        // scalastyle:on
      }
      i += 1
    ...
    // scalastyle:off
    println(
      s"  Stopped after $i iterations, ${NANOSECONDS.toMillis(runTimes.sum)} ms"
    )
    // scalastyle:on
...
private[spark] object Benchmark {
  ...
  def getProcessorName(): String = {
    val cpu = if (SystemUtils.IS_OS_MAC_OSX) {
      ...
      Try {
        val grepPath =
          Utils.executeAndGetOutput(Seq("which", "grep")).stripLineEnd
        Utils
          .executeAndGetOutput(
