...
class SparkHadoopUtilSuite extends SparkFunSuite {
  ...
  test("SPARK-40640: spark.hive propagation") {
    ...
    new SparkHadoopUtil()
      .appendS3AndSparkHadoopHiveConfigurations(sc, hadoopConf)
    assertConfigMatches(
      hadoopConf,
      "hive.hiveoption",
      "value",
  ...
  test(
    "SPARK-19739: S3 session token propagation requires access and secret keys"
  ) {
    val hadoopConf = new Configuration(false)
    SparkHadoopUtil.appendS3CredentialsFromEnvironment(
      hadoopConf,
  ...
  test(
    "SPARK-45404: aws endpoint propagation requires access and secret keys"
  ) {
    val hadoopConf = new Configuration(false)
    SparkHadoopUtil.appendS3CredentialsFromEnvironment(
      hadoopConf,
  ...
  test("substituteHadoopVariables") {
    ...
    val result1 =
      new SparkHadoopUtil().substituteHadoopVariables(text1, hadoopConf)
    assert(result1 == "yyy")

    val text2 = "${hadoopconf-xxx"
    val result2 =
      new SparkHadoopUtil().substituteHadoopVariables(text2, hadoopConf)
    assert(result2 == "${hadoopconf-xxx")

    val text3 = "${hadoopconf-xxx}zzz"
    val result3 =
      new SparkHadoopUtil().substituteHadoopVariables(text3, hadoopConf)
    assert(result3 == "yyyzzz")

    val text4 = "www${hadoopconf-xxx}zzz"
    val result4 =
      new SparkHadoopUtil().substituteHadoopVariables(text4, hadoopConf)
    assert(result4 == "wwwyyyzzz")

    val text5 = "www${hadoopconf-xxx}"
    val result5 =
      new SparkHadoopUtil().substituteHadoopVariables(text5, hadoopConf)
    assert(result5 == "wwwyyy")

    val text6 = "www${hadoopconf-xxx"
    val result6 =
      new SparkHadoopUtil().substituteHadoopVariables(text6, hadoopConf)
    assert(result6 == "www${hadoopconf-xxx")

    val text7 = "www$hadoopconf-xxx}"
    val result7 =
      new SparkHadoopUtil().substituteHadoopVariables(text7, hadoopConf)
    assert(result7 == "www$hadoopconf-xxx}")

    val text8 = "www{hadoopconf-xxx}"
    val result8 =
      new SparkHadoopUtil().substituteHadoopVariables(text8, hadoopConf)
