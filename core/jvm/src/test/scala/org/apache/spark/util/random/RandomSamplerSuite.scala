...
class RandomSamplerSuite extends SparkFunSuite with Matchers {
  ...
  def replacementSampling(
      data: Iterator[Int],
      sampler: PoissonSampler[Int]
  ): Iterator[Int] = {
    data.flatMap { item =>
  ...
  test("bernoulli sampling") {
    ...
    d = medianKSD(
      gaps(sampler.sample(Iterator.from(0))),
      gaps(sample(Iterator.from(0), 0.5))
    )
    d should be < D
  ...
  test("bernoulli sampling without iterator") {
    ...
    sampler.setSeed(rngSeed.nextLong)
    d = medianKSD(
      gaps(data.filter(_ => sampler.sample() > 0)),
      gaps(sample(Iterator.from(0), 0.5))
    )
    d should be < D
  ...
  test("bernoulli data types") {
    ...
    d = medianKSD(
      gaps(
        sampler.sample(
          ArrayBuffer(Iterator.from(0).take(20 * sampleSize).toSeq: _*).iterator
        )
      ),
      gaps(sample(Iterator.from(0), 0.1))
    )
    ...
    d = medianKSD(
      gaps(
        sampler.sample(Iterator.from(0).take(20 * sampleSize).toList.iterator)
      ),
      gaps(sample(Iterator.from(0), 0.1))
    )
    d should be < D
  ...
  test("bernoulli set seed") {
    ...
    sampler2.setSeed(73)
    d = medianKSD(
      gaps(sampler1.sample(Iterator.from(0))),
      gaps(sampler2.sample(Iterator.from(0)))
    )
    d should be(0.0)
  ...
  test("replacement sampling without iterator") {
    ...
    sampler.setSeed(rngSeed.nextLong)
    d = medianKSD(
      gaps(replacementSampling(data, sampler)),
      gaps(sampleWR(Iterator.from(0), 0.5))
    )
    d should be < D
