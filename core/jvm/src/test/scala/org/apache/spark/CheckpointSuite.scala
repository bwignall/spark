...
trait RDDCheckpointTester { self: SparkFunSuite =>
  ...
  protected def testRDD[U: ClassTag](
    ...
    val (rddSizeAfterCheckpoint, partitionSizeAfterCheckpoint) =
      getSerializedSizes(operatedRDD)
    logInfo(
      "RDD after checkpoint: " + operatedRDD + "\n" + operatedRDD.toDebugString
    )
...
class CheckpointSuite
  ...
  runTest("checkpointing partitioners", skipLocalCheckpoint = true) {
    _: Boolean =>
  ...
  runTest("ParallelCollectionRDD") { reliableCheckpoint: Boolean =>
    ...
    assert(
      parCollection.partitions.toList ===
  ...
  private def testCheckpointAllMarkedAncestors(
      reliableCheckpoint: Boolean,
      checkpointAllMarkedAncestors: Boolean
  ): Unit = {
    sc.setLocalProperty(
...
class FatPairRDD(parent: RDD[Int], _partitioner: Partitioner)
...
class CheckpointStorageSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("checkpoint compression") {
    withTempDir { checkpointDir =>
      ...
      val compressedInputStream = CompressionCodec
        .createCodec(conf)
  ...
  test("cache checkpoint preferred location") {
    withTempDir { checkpointDir =>
      ...
      assert(
        !checkpointedRDD.cachedPreferredLocations.asMap.containsKey(partition)
      )
      ...
      assert(
        preferredLoc == checkpointedRDD.cachedPreferredLocations.get(partition)
      )
    }
  }
  ...
  test("SPARK-31484: checkpoint should not fail in retry") {
    withTempDir { checkpointDir =>
      ...
      sc.setCheckpointDir(checkpointDir.toString)
      val rdd =
        sc.makeRDD(1 to 200, numSlices = 4).repartition(1).mapPartitions {
          iter =>
            iter.map { i =>
