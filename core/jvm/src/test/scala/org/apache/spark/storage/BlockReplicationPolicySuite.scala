...
class RandomBlockReplicationPolicyBehavior
  ...
  val replicationPolicy: BlockReplicationPolicy =
    new RandomBlockReplicationPolicy
  ...
  test("block replication - random block replication policy") {
    ...
    val storeSize = 1000
    val blockManagers =
      generateBlockManagerIds(numBlockManagers, Seq("/Rack-1"))
    val candidateBlockManager =
      BlockManagerId("test-store", "localhost", 1000, None)
  ...
  /** Returns a sequence of [[BlockManagerId]], whose rack is randomly picked from the given `racks`.
    ...
    */
  protected def generateBlockManagerIds(
      count: Int,
      racks: Seq[String]
  ): Seq[BlockManagerId] = {
    val randomizedRacks: Seq[String] = Random.shuffle(
      racks ++ racks.length
        .until(count)
        .map(_ => racks(Random.nextInt(racks.length)))
    )
    ...
    (0 until count).map { i =>
      BlockManagerId(
        ...
        s"Host-$i",
        10000 + i,
        Some(randomizedRacks(i))
      )
    }
  }
}

class TopologyAwareBlockReplicationPolicyBehavior
    extends RandomBlockReplicationPolicyBehavior {
  ...
  test("All peers in the same rack") {
    ...
    (1 to 10).foreach { numReplicas =>
      ...
      val blockManager =
        BlockManagerId("Driver", "Host-driver", 10001, Some(racks.head))
  ...
  test("Peers in 2 racks") {
    ...
    (1 to 10).foreach { numReplicas =>
      ...
      val blockManager =
        BlockManagerId("Driver", "Host-driver", 9001, Some(racks.head))
