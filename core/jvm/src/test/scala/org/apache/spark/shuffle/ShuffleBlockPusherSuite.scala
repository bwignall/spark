...
class ShuffleBlockPusherSuite extends SparkFunSuite {
  ...
  private def verifyBlockPushCompleted(
      blockPusher: ShuffleBlockPusher
  ): Unit = {
  ...
  test("A batch of blocks is limited by maxBlocksBatchSize") {
    ...
    val mergerLocs = dependency.getMergerLocs.map(loc =>
      BlockManagerId("", loc.host, loc.port)
    )
    val largeBlockSize = 2 * 1024 * 1024
    blockPusher.initiateBlockPush(
      mock(classOf[File]),
      Array.fill(dependency.partitioner.numPartitions) { 5 },
      dependency,
      0
    )
  ...
  test(
    ...
    when(
      shuffleClient.pushBlocks(
        ArgumentMatchers.eq("slow-client"),
        any(),
        any(),
        any(),
        any()
      ...
      .thenAnswer((invocation: InvocationOnMock) => {
        ...
        blocks.foreach { blockId =>
          blockPushListener.onBlockPushSuccess(
            blockId,
    ...
    when(
      shuffleClient.pushBlocks(
        ArgumentMatchers.eq("test-client"),
        any(),
        any(),
        any(),
        any()
  ...
  test(
    ...
    listener.onBlockPushSuccess(
      blockPendingResponse,
      mock(classOf[ManagedBuffer])
    )
    pusher.runPendingTasks()
  ...
  test("Connect exceptions remove all the push requests for that host") {
    when(dependency.getMergerLocs).thenReturn(
      Seq(
        BlockManagerId("client1", "client1", 1),
        BlockManagerId("client2", "client2", 2)
      )
    )
    conf.set("spark.reducer.maxBlocksInFlightPerAddress", "2")
    when(shuffleClient.pushBlocks(any(), any(), any(), any(), any()))
  ...
  private class TestShuffleBlockPusher(conf: SparkConf)
      extends ShuffleBlockPusher(conf) {
    ...
    override protected def createRequestBuffer(
      ...
      when(managedBuffer.nioByteBuffer())
        .thenReturn(ByteBuffer.wrap(byteBuffer))
      managedBuffer
    }
  }
  ...
  private class ConcurrentTestBlockPusher(conf: SparkConf, semaphore: Semaphore)
    ...
    val blockPusher =
      ThreadUtils.newDaemonFixedThreadPool(1, "test-block-pusher")
