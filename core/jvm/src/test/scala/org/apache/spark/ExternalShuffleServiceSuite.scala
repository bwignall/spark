...
/** This suite creates an external shuffle server and routes all shuffle fetches through it.
  ...
  */
class ExternalShuffleServiceSuite
    extends ShuffleSuite
  ...
  test("using external shuffle service") {
    ...
    sc.getConf.get(config.SHUFFLE_HOST_LOCAL_DISK_READING_ENABLED) should equal(
      false
    )
  ...
  test(
    ...
    try {
            ...
            res.foreach { case (eid, dirs) =>
              val file = new File(
                ExecutorDiskUtils.getFilePath(
                  dirs,
                  sc.env.blockManager.subDirsPerLocalDir,
                  blockId.name
                )
              )
      ...
      val filesToCheck =
        promises.map(p => ThreadUtils.awaitResult(p, Duration(2, "sec")))
  ...
  test(
    ...
    for (enabled <- Seq(true, false)) {
      ...
      try {
        ...
        val mapOutputs =
          sc.env.mapOutputTracker.getMapSizesByExecutorId(0, 0).toSeq
        ...
        val filesToCheck =
          promises.flatMap(p => ThreadUtils.awaitResult(p, Duration(2, "sec")))
