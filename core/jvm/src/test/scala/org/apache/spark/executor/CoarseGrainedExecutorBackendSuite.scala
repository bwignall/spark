...
class CoarseGrainedExecutorBackendSuite
  ...
  test("parsing one resource") {
    ...
    withTempDir { tmpDir =>
      ...
      assert(
        parsedResources.get(GPU).get.addresses.sameElements(Array("0", "1"))
      )
    }
  }
  ...
  test("error checking parsing resources and executor and task configs") {
    ...
    withTempDir { tmpDir =>
      ...
      assert(
        error.contains(
          "Resource: gpu, with addresses: 0 is less than what the " +
            "user requested: 2"
        )
      )
    }
    ...
    withTempDir { tmpDir =>
      ...
      assert(
        error.contains(
          "User is expecting to use resource: gpu, but didn't " +
  ...
  private def testExecutorResourceFoundLessThanRequired(
    ...
    withTempDir { tmpDir =>
      ...
      assert(
        error.contains(
          "Resource: gpu, with addresses: 0,1 is less than what the " +
            "user requested: 4"
        )
      )
    }
  ...
  test(
    ...
    withTempDir { dir =>
      ...
      val rp = rpBuilder.require(ereqs).build()
      allocatedFileAndConfigsResourceDiscoveryTestFpga(
        dir,
        createSparkConf(),
        rp
      )
  ...
  test("track allocated resources by taskId") {
    ...
    try {
      ...
      backend.self.send(
        LaunchTask(new SerializableBuffer(serializedTaskDescription))
      )
  ...
  test(s"Tasks launched should always be cancelled.") {
    ...
    try {
      ...
      val otherTasks =
        taskDescriptions.slice(1, numTasks).map(getFakeTaskRunner(_)).toArray
      assert(otherTasks.length == numTasks - 1)
      // Workaround for compilation issue around Mockito.doReturn
      doReturn(firstLaunchTask, otherTasks: _*)
        .when(executor)
        .createTaskRunner(any(), any())
      ...
      taskDescriptions.foreach { taskDescription =>
        val buffer =
          new SerializableBuffer(TaskDescription.encode(taskDescription))
        backend.self.send(LaunchTask(buffer))
        Thread.sleep(1)
        backend.self.send(
          KillTask(taskDescription.taskId, "exec1", false, "test")
        )
      }
      ...
      logInfo(
        s"Task executed ${tasksExecuted.size}, task killed ${tasksKilled.size}"
      )
    } finally {
      if (backend != null) {
        backend.rpcEnv.shutdown()
