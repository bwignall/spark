...
class SortingSuite extends SparkFunSuite with SharedSparkContext with Matchers {
  ...
  test("sort descending") {
    ...
    val pairs = sc.parallelize(pairArr.toImmutableArraySeq, 2)
    assert(
      pairs.sortByKey(false).collect() === pairArr.sortWith((x, y) =>
        x._1 > y._1
      )
    )
  }
  ...
  test("sort descending with one split") {
    ...
    assert(
      pairs.sortByKey(false, 1).collect() === pairArr.sortWith((x, y) =>
        x._1 > y._1
      )
    )
  }
  ...
  test("sort descending with many partitions") {
    ...
    val pairs = sc.parallelize(pairArr.toImmutableArraySeq, 2)
    assert(
      pairs.sortByKey(false, 20).collect() === pairArr.sortWith((x, y) =>
        x._1 > y._1
      )
    )
  }
  ...
  test(
    ...
    val sorted =
      sc.parallelize(pairArr.toImmutableArraySeq, 10).sortByKey(false)
