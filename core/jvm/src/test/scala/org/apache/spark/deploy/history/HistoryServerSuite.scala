...
abstract class HistoryServerSuite
    ...
    if (json.indexOf("lastUpdated") >= 0) {
      ...
      for (i <- subStrings.indices) {
        if (subStrings(i).indexOf("lastUpdatedEpoch") >= 0) {
          ...
          subStrings(i) =
            regex.replaceAllIn(subStrings(i), "\"lastUpdated\" : \"\"")
        }
      }
      subStrings.mkString(",")
    } else {
  ...
  test("automatically retrieve uiRoot from request through Knox") {
    ...
    when(requestThroughKnox.getHeader("X-Forwarded-Context"))
      .thenReturn(knoxBaseUrl)
    val responseThroughKnox = page.render(requestThroughKnox)

    val urlsThroughKnox = responseThroughKnox \\ "@href" map (_.toString)
    val siteRelativeLinksThroughKnox =
      urlsThroughKnox filter (_.startsWith("/"))
  ...
  test("incomplete apps get refreshed") {
    ...
    def listDir(dir: Path): Seq[FileStatus] = {
      ...
      statuses
        .flatMap(stat =>
          if (stat.isDirectory) listDir(stat.getPath)
          else Seq(stat)
        )
        .toImmutableArraySeq
    }
    ...
    assert(
      metrics.lookupCount.getCount > 0,
  ...
  test("ui and api authorization checks") {
    ...
    tests.foreach { case (user, expectedCode) =>
      testUrls.foreach { url =>
        val headers =
          if (user != null) Seq(FakeAuthFilter.FAKE_HTTP_USER -> user) else Nil
        val sc = TestUtils.httpResponseCode(new URL(url), headers = headers)
        assert(
          sc === expectedCode,
          s"Unexpected status code $sc for $url (user = $user)"
        )
      }
    }
  }
...
object HistoryServerSuite {
  ...
  def connectAndGetInputStream(
      url: URL
  ): (Int, Option[InputStream], Option[String]) = {
    val connection = url.openConnection().asInstanceOf[HttpURLConnection]
    connection.setRequestMethod("GET")
    connection.connect()
    val code = connection.getResponseCode()
