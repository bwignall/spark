...
class AppClientSuite
    ...
    Utils.tryWithResource(new AppClientInst(masterRpcEnv.address.toSparkURL)) {
      ci =>
        ...
        eventually(timeout(10.seconds), interval(10.millis)) {
          ...
          assert(
            app.getTargetExecutorNumForRPId(DEFAULT_RESOURCE_PROFILE_ID) === 1
          )
  ...
  private class AppClientCollector
      extends StandaloneAppClientListener
      with Logging {
    ...
    val execDecommissionedMap =
      new ConcurrentHashMap[String, ExecutorDecommissionInfo]()
    ...
    def executorDecommissioned(
        id: String,
        decommissionInfo: ExecutorDecommissionInfo
    ): Unit = {
      val previousDecommissionInfo =
        execDecommissionedMap.putIfAbsent(id, decommissionInfo)
      assert(
        previousDecommissionInfo === null,
        s"Expected no previous decommission info for $id"
      )
    }
  ...
  private class AppClientInst(masterUrl: String) extends Closeable {
    ...
    val client = new StandaloneAppClient(
      rpcEnv,
      Array(masterUrl),
      desc,
      listener,
