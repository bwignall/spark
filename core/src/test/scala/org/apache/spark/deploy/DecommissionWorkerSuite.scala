...
class DecommissionWorkerSuite
  ...
  test("decommission workers should not result in job failure") {
    ...
    sc = createSparkContext(
      config.TASK_MAX_FAILURES.key -> maxTaskFailures.toString
    )
    ...
    val listener = new RootStageAwareListener {
      override def handleRootTaskStart(
    ...
    listener.rootTasksEnded.asScala.foreach { taskInfo =>
      assert(
        taskInfo.index === 0,
        s"Expected task index ${taskInfo.index} to be 0"
      )
  ...
  test(
    ...
    try {
          ...
          if (taskInfo.index == 0) {
            if (workerForTask0Decommissioned.compareAndSet(false, true)) {
              ...
              decommissionWorkerOnMaster(
                workerInfo,
                "Kill early done map worker"
              )
              killWorkerAfterTimeout(workerInfo, 0)
              logInfo(
                s"Killed the node ${workerInfo.hostPort} that was running the early task"
              )
            }
          }
        }
      ...
      listener.rootTasksStarted.asScala.foreach { taskInfo =>
        ...
        assert(
          taskInfo.successful,
          s"Task ${taskInfo.index} should be successful"
        )
      }
      val tasksEnded = listener.rootTasksEnded.asScala
      tasksEnded.filter(_.index != 0).foreach { taskInfo =>
        assert(
          taskInfo.attemptNumber === 0,
          "2nd task should succeed on 1st attempt"
        )
      }
      val firstTaskAttempts = tasksEnded.filter(_.index == 0)
      assert(
        firstTaskAttempts.size > 1,
        s"Task 0 should have multiple attempts"
      )
    } finally {
      ss.close()
    }
  ...
  def testFetchFailures(initialSleepMillis: Int): Unit = {
    ...
    logInfo(
      s"Will try to decommission the task running on executor $executorToDecom"
    )
    val listener = new RootStageAwareListener {
      override def handleRootTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = {
        val taskInfo = taskEnd.taskInfo
        if (
          taskInfo.executorId == executorToDecom && taskInfo.attemptNumber == 0 &&
      ...
      val jobResult = sc
        ...
        .mapPartitionsWithIndex(
          (_, _) => {
            ...
            if (
              context.attemptNumber() == 0 && context.stageAttemptNumber() == 0
            ) {
              val sleepTimeSeconds =
                if (executorId == executorToDecom) 10 else 1
              Thread.sleep(sleepTimeSeconds * 1000L)
            }
            List(1).iterator
          },
  ...
  private abstract class RootStageAwareListener extends SparkListener {
    ...
    override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = {
      ...
      val taskSignature =
        getSignature(taskEnd.taskInfo, taskEnd.stageId, taskEnd.stageAttemptId)
  ...
  private def getExecutorToWorkerAssignments: Map[String, WorkerInfo] = {
    ...
    master.workers.foreach { wi =>
      assert(
        wi.executors.size <= 1,
        "There should be at most one executor per worker"
      )
  ...
  private def createWorkers(
    ...
    workerRpcEnvs.foreach { rpcEnv =>
      ...
      assert(
        oldRpcEnv.isEmpty,
        s"Detected duplicate rpcEnv ${oldRpcEnv} for ${rpcEnv.address}"
      )
    }
    workerIdToRpcEnvs.clear()
    ...
    eventually(timeout(1.minute), interval(1.seconds)) {
      ...
      logInfo(
        s"Waiting for $numWorkers workers to come up: So far $numWorkersCurrently"
      )
  ...
  def decommissionWorkerOnMaster(
      workerInfo: WorkerInfo,
      reason: String
  ): Unit = {
    logInfo(
      s"Trying to decommission worker ${workerInfo.id} for reason `$reason`"
    )
    master.self.send(DecommissionWorkers(Seq(workerInfo.id)))
  }

  def killWorkerAfterTimeout(
      workerInfo: WorkerInfo,
      secondsToWait: Int
  ): Unit = {
