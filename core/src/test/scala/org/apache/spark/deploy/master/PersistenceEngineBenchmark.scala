...
object PersistenceEngineBenchmark extends BenchmarkBase {
  ...
  override def runBenchmarkSuite(mainArgs: Array[String]): Unit = {
    ...
    runBenchmark("PersistenceEngineBenchmark") {
      val benchmark =
        new Benchmark(s"$numWorkers Workers", numWorkers, output = output)

      serializers.foreach { serializer =>
        val serializerName = serializer.getClass.getSimpleName
        benchmark.addCase(
          s"ZooKeeperPersistenceEngine with $serializerName",
          numIters
        ) { _ =>
      ...
      serializers.foreach { serializer =>
        ...
        CompressionCodec.ALL_COMPRESSION_CODECS.foreach { c =>
          ...
          val name =
            s"FileSystemPersistenceEngine with $serializerName ($shortCodecName)"
          benchmark.addCase(name, numIters) { _ =>
            val dir = Utils.createTempDir().getAbsolutePath
            val engine =
              new FileSystemPersistenceEngine(dir, serializer, Some(codec))
            writeAndRead(engine)
            engine.close()
          }
        }
  ...
  private def createWorkerInfo(id: Int): WorkerInfo = {
    ...
    val workerInfo = new WorkerInfo(
      s"worker-20231201000000-255.255.255.255-$id",
      "host",
      8080,
      4,
      1234,
  ...
  private def createDriverInfo(id: Int): DriverInfo = {
    ...
    new DriverInfo(
      now,
      f"driver-20240101000000-$id%04d",
      DriverDescription(
  ...
  private def createApplicationInfo(id: Int): ApplicationInfo = {
    ...
    val customResources = Map(GPU -> 3, FPGA -> 3)
    ...
    appInfo.endTime =
      now + 1000 // Elapsed 1s from submitDate in order to give different values
    appInfo
  }
