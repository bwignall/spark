...
trait TestPrematureExit {
  ...
  private[spark] def testPrematureExit(
      input: Array[String],
      searchString: String,
      mainObject: CommandLineUtils = SparkSubmit
  ): Unit = {
...
class SparkSubmitSuite
  ...
  test(
    ...
    val clArgs1 = Seq(
      ...
      "0",
      "--conf",
      s"${DYN_ALLOCATION_ENABLED.key}=true",
      "thejar.jar"
    )
    ...
    val clArgs2 = Seq(
      ...
      "0",
      "--conf",
      s"${DYN_ALLOCATION_ENABLED.key}=false",
      "thejar.jar"
    )

    val e = intercept[SparkException](new SparkSubmitArguments(clArgs2))
    assert(
      e.getMessage.contains("Number of executors must be a positive number")
    )
  }
  ...
  test("handles YARN cluster mode") {
    ...
    ) should include regex (".*one.jar,.*two.jar,.*three.jar")
    conf.get(
      "spark.yarn.dist.files"
    ) should include regex (".*file1.txt,.*file2.txt")
    conf.get(
      "spark.yarn.dist.archives"
    ) should include regex (".*archive1.txt,.*archive2.txt")
  ...
  test("SPARK-33530: handles standalone mode with archives") {
    ...
    conf.get(
      "spark.archives"
    ) should include regex (".*archive1.zip,.*archive2.jar")
    conf.get("spark.app.name") should be("beauty")
    conf.get(UI_ENABLED) should be(false)
    sys.props("SPARK_SUBMIT") should be("true")
  }
  ...
  test("handles k8s cluster mode") {
    val clArgs = Seq(
      ...
      "--conf",
      "spark.kubernetes.driver.container.image=bar",
      "/home/thejar.jar",
      "arg1"
    )
    val appArgs = new SparkSubmitArguments(clArgs)
  ...
  test(
    "SPARK-35084: include jars of the --packages" +
    ...
    IvyTestUtils.withRepository(main, Some(dep.toString), None) { repo =>
      val clArgs = Seq(
        "--class",
        JarCreationTest.getClass.getName.stripSuffix("$"),
        "--name",
        "testApp",
        "--deploy-mode",
        "client",
  ...
  test(
    "SPARK-47495: Not to add primary resource to jars again" +
      " in k8s client mode & driver runs inside a POD"
  ) {
    val clArgs = Seq(
      "--deploy-mode",
      ...
      "spark.kubernetes.submitInDriver=true",
      "--jars",
      "src/test/resources/TestUDTF.jar",
  ...
  test(
    "SPARK-47475: Avoid jars download if scheme matches " +
    ...
    withTempDir { tmpDir =>
      ...
      val clArgs = Seq(
        ...
        "spark.kubernetes.submitInDriver=true",
        "--conf",
        "spark.kubernetes.jars.avoidDownloadSchemes=s3a",
        "--conf",
        ...
        "src/test/resources/log4j2.properties",
        "--jars",
        s"src/test/resources/TestUDTF.jar,$remoteJarFile",
        "/home/jarToIgnore.jar",
        "arg1"
      )
      val appArgs = new SparkSubmitArguments(clArgs)
      val (_, _, conf, _) =
        submit.prepareSubmitEnvironment(appArgs, Some(hadoopConf))
      conf.get("spark.master") should be("k8s://https://host:port")
  ...
  test("SPARK-43014: Set `spark.app.submitTime` if missing ") {
    ...
    val clArgs2 = Seq(
      ...
      "org.SomeClass",
      "--conf",
      "spark.app.submitTime=" + submitTime,
      "/home/thejar.jar"
    )
    val appArgs2 = new SparkSubmitArguments(clArgs2)
    val (_, _, conf2, _) = submit.prepareSubmitEnvironment(appArgs2)
  ...
  test(
    "SPARK-43014: Do not overwrite `spark.app.submitTime` in k8s cluster mode driver " +
    ...
    val clArgs = Seq(
      ...
      "spark.kubernetes.submitInDriver=true",
      "--conf",
      "spark.kubernetes.setSubmitTimeInDriver=false",
      "/home/thejar.jar"
    )
    val appArgs = new SparkSubmitArguments(clArgs)
    val (_, _, conf, _) = submit.prepareSubmitEnvironment(appArgs)
  ...
  test(
    ...
    withTempDir { tempDir =>
      val (jarFile, (childArgs, classpaths, _, mainClass_)) =
        testResolveMainClassOnRemoteJar(
          tempDir,
          "client"
        )
  ...
  test("launch simple application with spark-submit") {
    ...
    val args = Seq(
      "--class",
      SimpleApplicationTest.getClass.getName.stripSuffix("$"),
      "--name",
      "testApp",
      "--master",
      "local",
  ...
  test("launch simple application with spark-submit with redaction") {
    ...
    withTempDir { testDir =>
      ...
      val args = Seq(
        ...
        "--conf",
        "spark.eventLog.testing=true",
        "--conf",
        s"spark.eventLog.dir=${testDirPath.toUri.toString}",
        "--conf",
      ...
      val logData =
        EventLogFileReader.openEventLog(listStatus.last.getPath, fileSystem)
  ...
  test(
    ...
    IvyTestUtils.withRepository(main, Some(dep.toString), None) { repo =>
      val args = Seq(
        ...
        "local-cluster[2,1,1024]",
        "--conf",
        "spark.jars.packages=my.great.lib:mylib:0.1,my.great.dep:mylib:0.1",
        "--conf",
        s"spark.jars.repositories=$repo",
        "--conf",
        "spark.ui.enabled=false",
        "--conf",
        "spark.master.rest.enabled=false",
  ...
  ignore("correctly builds R packages included in a jar with --packages") {
    ...
    val rScriptDir =
      Seq(sparkHome, "R", "pkg", "tests", "fulltests", "packageInAJarTest.R")
  ...
  test("include an external JAR in SparkR") {
    ...
    assume(RUtils.isSparkRInstalled, "SparkR is not installed in this build.")
    val rScriptDir =
      Seq(sparkHome, "R", "pkg", "tests", "fulltests", "jarTest.R").mkString(
        File.separator
      )
    assert(new File(rScriptDir).exists)
    ...
    withTempDir { tempDir =>
      ...
      val excSource = new JavaSourceFromString(
        new File(srcDir, "DummyClass").toURI.getPath,
        """package sparkrtest;
        |
      ...
      val jarFile = new File(
        tempDir,
        "sparkRTestJar-%s.jar".format(System.currentTimeMillis())
      )
      val jarURL = TestUtils.createJar(
        Seq(excFile),
        jarFile,
        directoryPrefix = Some("sparkrtest")
      )
  ...
  test("resolves command line argument paths correctly") {
    withTempDir { dir =>
      ...
      conf.get(JARS) should be(
        Utils.resolveURIs(jars + ",thejar.jar").split(",").toSeq
      )
      ...
      val clArgs3 = Seq(
        ...
        pyFiles,
        "--conf",
        "spark.pyspark.driver.python=python3.4",
        "--conf",
        "spark.pyspark.python=python3.5",
        "mister.py"
      )
  ...
  test("ambiguous archive mapping results in error message") {
    withTempDir { dir =>
      ...
      testPrematureExit(
        clArgs2.toArray,
        "resolves ambiguously to multiple files"
      )
    }
  }
  ...
  test("resolves config paths correctly") {
    ...
    withTempDir { tmpDir =>
      ...
      updateConfWithFakeS3Fs(hadoopConf)
      val f4 =
        File.createTempFile("test-submit-remote-python-files", "", tmpDir)
      ...
      val (_, _, conf4, _) =
        submit.prepareSubmitEnvironment(appArgs4, conf = Some(hadoopConf))
      // Should not format python path for yarn cluster mode
      conf4.get(SUBMIT_PYTHON_FILES) should be(
        Utils.resolveURIs(remotePyFiles).split(",")
      )
    }
  }
  ...
  test("user classpath first in driver") {
    ...
    val args = Seq(
      "--class",
      UserClasspathFirstTest.getClass.getName.stripSuffix("$"),
      "--name",
      "testApp",
      "--master",
      "local",
      "--conf",
      "spark.driver.extraClassPath=" + systemJar,
      "--conf",
  ...
  test("SPARK_CONF_DIR overrides spark-defaults.conf") {
    forConfDir(Map("spark.executor.memory" -> "3g")) { path =>
      ...
      val appArgs =
        new SparkSubmitArguments(args, env = Map("SPARK_CONF_DIR" -> path))
      appArgs.executorMemory should be("3g")
    }
  }

  test(
    "SPARK-48392: load spark-defaults.conf when --load-spark-defaults is set"
  ) {
    forConfDir(
      Map("spark.executor.memory" -> "3g", "spark.driver.memory" -> "3g")
    ) { path =>
      withPropertyFile(
        "spark-conf.properties",
        Map("spark.executor.cores" -> "16", "spark.driver.memory" -> "4g")
      ) { propsFile =>
        val unusedJar = TestUtils.createJarWithClasses(Seq.empty)
        val args = Seq(
          "--class",
  ...
  test(
    "SPARK-48392: should skip spark-defaults.conf when --load-spark-defaults is not set"
  ) {
    forConfDir(
      Map("spark.executor.memory" -> "3g", "spark.driver.memory" -> "3g")
    ) { path =>
      ...
      withTempDir { tmpFileDir =>
        withTempDir { tmpPyFileDir =>
          withTempDir { tmpArchiveDir =>
            val jar1 = TestUtils.createJarWithFiles(
              Map("test.resource" -> "1"),
              tmpJarDir
            )
            val jar2 = TestUtils.createJarWithFiles(
              Map("test.resource" -> "USER"),
              tmpJarDir
            )
            ...
            val archive1 =
              File.createTempFile("archive1", ".zip", tmpArchiveDir)
            val archive2 =
              File.createTempFile("archive2", ".zip", tmpArchiveDir)
            ...
            val args = Seq(
              ...
              "client",
              "--jars",
              s"${tmpJarDir.getAbsolutePath}/*.jar",
              "--files",
              s"${tmpFileDir.getAbsolutePath}/tmpFile*",
              "--py-files",
              s"${tmpPyFileDir.getAbsolutePath}/tmpPy*",
              "--archives",
              s"${tmpArchiveDir.getAbsolutePath}/*.zip",
  ...
  test("SPARK-27575: yarn confs should merge new value with existing value") {
    ...
    val jar1 =
      TestUtils.createJarWithFiles(Map("test.resource" -> "1"), tmpJarDir)
    val jar2 =
      TestUtils.createJarWithFiles(Map("test.resource" -> "USER"), tmpJarDir)

    val tmpJarDirYarnOpt = Utils.createTempDir()
    val jar1YarnOpt = TestUtils.createJarWithFiles(
      Map("test.resource" -> "2"),
      tmpJarDirYarnOpt
    )
    val jar2YarnOpt = TestUtils.createJarWithFiles(
      Map("test.resource" -> "USER2"),
      tmpJarDirYarnOpt
    )
    ...
    val file1YarnOpt =
      File.createTempFile("tmpPy1YarnOpt", ".py", tmpFileDirYarnOpt)
    val file2YarnOpt =
      File.createTempFile("tmpPy2YarnOpt", ".egg", tmpFileDirYarnOpt)
    ...
    val pyFile1YarnOpt =
      File.createTempFile("tmpPy1YarnOpt", ".py", tmpPyFileDirYarnOpt)
    val pyFile2YarnOpt =
      File.createTempFile("tmpPy2YarnOpt", ".egg", tmpPyFileDirYarnOpt)
    ...
    val archive1YarnOpt =
      File.createTempFile("archive1YarnOpt", ".zip", tmpArchiveDirYarnOpt)
    val archive2YarnOpt =
      File.createTempFile("archive2YarnOpt", ".zip", tmpArchiveDirYarnOpt)
    ...
    assertEqualsWithFiles(
      Set(archive1, archive2, archive1YarnOpt, archive2YarnOpt),
  ...
  private def checkDownloadedFile(
      sourcePath: String,
      outputPath: String
  ): Unit = {
  ...
  test("remove copies of application jar from classpath") {
    ...
    val resolvedJars = DependencyUtils.resolveAndDownloadJars(
      jars,
      userJar,
      sparkConf,
      hadoopConf
  ...
  test("Avoid re-upload remote resources in yarn client mode") {
    ...
    withTempDir { tmpDir =>
      ...
      val tmpJar =
        TestUtils.createJarWithFiles(Map("test.resource" -> "USER"), tmpDir)
      ...
      val appArgs = new SparkSubmitArguments(args)
      val (_, _, conf, _) =
        submit.prepareSubmitEnvironment(appArgs, conf = Some(hadoopConf))
  ...
  test(
    ...
    val args = Seq(
      ...
      "--conf",
      "spark.plugins=TestSparkPlugin",
      "--conf",
      "spark.ui.enabled=false",
      "--jars",
      jarUrl.toString + "," + unusedJar.toString,
      "--files",
      tempFile.toString + "," + unusedFile.toString,
      unusedJar.toString
    )
    runSparkSubmit(args)
  }
  ...
  test(
    "SPARK-45762: The ShuffleManager plugin to use can be defined in a user jar"
  ) {
  ...
  """.stripMargin
    ...
    val argsBase = Seq(
      ...
      "local-cluster[1,1,1024]",
      "--conf",
      "spark.shuffle.manager=TestShuffleManager",
      "--conf",
      "spark.ui.enabled=false"
    )
    ...
    val argsSuccess =
      (argsBase ++ Seq("--jars", jarUrl.toString)) :+ unusedJar.toString
    // check process success exit code
    assertResult(0)(runSparkSubmit(argsSuccess, expectFailure = false))
  }

  private def testRemoteResources(
      enableHttpFs: Boolean,
      forceDownloadSchemes: Seq[String] = Nil
  ): Unit = {
    val hadoopConf = new Configuration()
    updateConfWithFakeS3Fs(hadoopConf)
    if (enableHttpFs) {
    ...
    val forceDownloadArgs = if (forceDownloadSchemes.nonEmpty) {
      Seq(
        "--conf",
        s"spark.yarn.dist.forceDownloadSchemes=${forceDownloadSchemes.mkString(",")}"
      )
    } else {
      Nil
    }
    ...
    def isSchemeForcedDownload(scheme: String) = {
      forceDownloadSchemes.contains("*") || forceDownloadSchemes.contains(
        scheme
      )
    }
  ...
  private def withPropertyFile(fileName: String, conf: Map[String, String])(
      f: String => Unit
  ) = {
    withTempDir { tmpDir =>
  ...
  test("start SparkApplication without modifying system properties") {
    val args = Array(
      "--class",
      classOf[TestSparkApplication].getName(),
      "--master",
      "local",
      "--conf",
      "spark.test.hello=world",
      "spark-internal",
      "hello"
    )
  ...
  test("support --py-files/spark.submit.pyFiles in non pyspark application") {
    ...
    withTempDir { tmpDir =>
      ...
      val args1 = Seq(
        ...
        "client",
        "--conf",
        s"spark.submit.pyFiles=s3a://${pyFile.getAbsolutePath}",
        "spark-internal"
      )

      val appArgs1 = new SparkSubmitArguments(args1)
      val (_, _, conf1, _) =
        submit.prepareSubmitEnvironment(appArgs1, conf = Some(hadoopConf))
  ...
  test(
    ...
    val leadingDelimKeyFromFile =
      s"${delimKey}leadingDelimKeyFromFile" -> s"${LF}blah"
    val trailingDelimKeyFromFile =
      s"${delimKey}trailingDelimKeyFromFile" -> s"blah${CR}"
    val infixDelimFromFile =
      s"${delimKey}infixDelimFromFile" -> s"${CR}blah${LF}"
    ...
    withPropertyFile("test-spark-conf.properties", testProps) { propsFile =>
      val clArgs = Seq(
        ...
        "org.SomeClass",
        "--conf",
        s"${lineFeedFromCommandLine._1}=${lineFeedFromCommandLine._2}",
        "--conf",
        "spark.master=yarn",
        "--properties-file",
        propsFile,
  ...
  test("get a Spark configuration from arguments") {
    ...
    val clArgs = Seq(
      "--conf",
      s"${testConf._1}=${testConf._2}",
      "--conf",
      s"${masterConf._1}=${masterConf._2}",
      "--class",
      "Foo",
      "app.jar"
    )
    ...
    for (config <- configs) {
      ...
      if (executorValues.length != 1) {
        throw new SparkException(
          s"Inconsistent values for $config: " +
