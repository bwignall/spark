...
abstract class EventLogFileWritersSuite
  ...
  test("Use the default value of spark.eventLog.compression.codec") {
    ...
    val writer =
      createWriter(appId, appAttemptId, testDirPath.toUri, conf, hadoopConf)
    assert(
      writer.compressionCodecName === EVENT_LOG_COMPRESSION_CODEC.defaultValue
    )
  }

  protected def readLinesFromEventLogFile(
      log: Path,
      fs: FileSystem
  ): List[String] = {
...
class SingleEventLogFileWriterSuite extends EventLogFileWritersSuite {
  ...
  test("Log overwriting") {
    ...
    val appAttemptId = None
    val logUri =
      SingleEventLogFileWriter.getLogPath(testDir.toURI, appId, appAttemptId)

    val conf = getLoggingConf(testDirPath)
    val hadoopConf = SparkHadoopUtil.get.newConfiguration(conf)
    val writer =
      createWriter(appId, appAttemptId, testDir.toURI, conf, hadoopConf)
  ...
  test("Event log name") {
    ...
    // without compression
    assert(
      s"${baseDirUri.toString}/app1" === SingleEventLogFileWriter.getLogPath(
        baseDirUri,
        "app1",
        None,
        None
    ...
    assert(
      s"${baseDirUri.toString}/app1.lzf" ===
        SingleEventLogFileWriter.getLogPath(
          baseDirUri,
          "app1",
          None,
    ...
    assert(
      s"${baseDirUri.toString}/a-fine-mind_dollar_bills__1" ===
        SingleEventLogFileWriter.getLogPath(
          baseDirUri,
    ...
    assert(
      s"${baseDirUri.toString}/a-fine-mind_dollar_bills__1.lz4" ===
        SingleEventLogFileWriter.getLogPath(
          baseDirUri,
          "a fine:mind$dollar{bills}.1",
          None,
  ...
  override protected def verifyWriteEventLogFile(
    ...
    assert(
      fileSystem
        .exists(finalLogPath) && fileSystem.getFileStatus(finalLogPath).isFile
    )
    assert(
      expectedLines === readLinesFromEventLogFile(finalLogPath, fileSystem)
    )
  }
}
...
class RollingEventLogFilesWriterSuite extends EventLogFileWritersSuite {
  ...
  test("Event log names") {
    ...
    val logDir =
      RollingEventLogFilesWriter.getAppEventLogDirPath(baseDirUri, appId, None)
    assert(
      s"${baseDirUri.toString}/${EVENT_LOG_DIR_NAME_PREFIX}${appId}" === logDir.toString
    )
  ...
  test("Log overwriting") {
    val appId = "test"
    val appAttemptId = None
    val logDirPath = RollingEventLogFilesWriter.getAppEventLogDirPath(
      testDir.toURI,
      appId,
      appAttemptId
    )
  ...
  allCodecs.foreach { codecShortName =>
    test(s"rolling event log files - codec $codecShortName") {
      ...
      val logDirPath =
        getAppEventLogDirPath(testDirPath.toUri, appId, attemptId)
  ...
  test(
    s"rolling event log files - the max size of event log file size less than lower limit"
  ) {
    val appId = getUniqueApplicationId
    val attemptId = None
  ...
  override protected def verifyWriteEventLogFile(
    ...
    assert(
      fileSystem
        .exists(logDirPath) && fileSystem.getFileStatus(logDirPath).isDirectory
    )
    ...
    assert(
      fileSystem
        .exists(appStatusFile) && fileSystem.getFileStatus(appStatusFile).isFile
    )
