...
class EventLogFileCompactorSuite extends SparkFunSuite {
  ...
  test(
    ...
    withTempDir { dir =>
      ...
      val compactedPath = new Path(
        fileToCompact.getParent,
        fileToCompact.getName + EventLogFileWriter.COMPACTED
      )
      assert(fs.rename(fileToCompact, compactedPath))

      val newFileStatuses =
        Seq(fs.getFileStatus(compactedPath)) ++ fileStatuses.drop(1)
      val compactor = new EventLogFileCompactor(
        sparkConf,
        hadoopConf,
        fs,
  ...
  test("events for finished job are dropped in new compact file") {
    withTempDir { dir =>
      ...
      Utils.tryWithResource(
        EventLogFileReader.openEventLog(compactFilePath, fs)
      ) { is =>
        val lines = Source.fromInputStream(is)(Codec.UTF8).getLines().toList
        assert(
          lines.length === 2,
          "Compacted file should have only two events being accepted"
        )
        lines.foreach { line =>
  ...
  test("Don't compact file if score is lower than threshold") {
    withTempDir { dir =>
      ...
      val tasks =
        createTasks(4, Array("exec1"), 0L).map(createTaskStartEvent(_, 1, 0))

      // job 2 having 4 tasks
      val rddsForStage2 = createRddsWithId(3 to 4)
      val stage2 = createStage(2, rddsForStage2, Nil)
      val tasks2 =
        createTasks(4, Array("exec1"), 0L).map(createTaskStartEvent(_, 2, 0))
      ...
      val fileStatuses = writeEventsToRollingWriter(
        ...
        hadoopConf,
        Seq(
          SparkListenerJobStart(1, 0, Seq(stage1)),
          SparkListenerStageSubmitted(stage1)
        ),
        tasks,
        Seq(
          SparkListenerJobStart(2, 0, Seq(stage2)),
          SparkListenerStageSubmitted(stage2)
        ),
        tasks2,
  ...
  test("rewrite files with test filters") {
    ...
    withTempDir { tempDir =>
      ...
      val writer = new SingleEventLogFileWriter(
        "app",
        None,
        tempDir.toURI,
        sparkConf,
      ...
      writeEventToWriter(
        writer,
        SparkListenerApplicationStart("app", None, 0, "user", None)
      )

      // filterNodeExcluded: None & Some(true) => filter in
      expectedLines += writeEventToWriter(
        writer,
        SparkListenerNodeExcluded(0, "host1", 1)
      )
      ...
      assert(
        new Path(
          newPath
        ).getName === logPath.getName + EventLogFileWriter.COMPACTED
      )

      Utils.tryWithResource(
        EventLogFileReader.openEventLog(new Path(newPath), fs)
      ) { is =>
  ...
  private def assertCompaction(
      ...
      expectedNumOfFilesCompacted: Int
  ): Unit = {
    ...
    val expectCompactFileBasePath =
      originalFiles.take(expectedNumOfFilesCompacted).last.getPath
    val expectCompactFileIndex =
      RollingEventLogFilesWriter.getEventLogFileIndex(
  ...
  private def assertNoCompaction(
      ...
      originalFiles: Seq[FileStatus],
      compactRet: CompactionResult,
      expectedCompactRet: CompactionResultCode.Value
  ): Unit = {
