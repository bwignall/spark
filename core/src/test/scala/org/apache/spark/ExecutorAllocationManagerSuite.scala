...
class ExecutorAllocationManagerSuite extends SparkFunSuite {
  ...
  test("add executors multiple profiles") {
    val manager = createManager(createConf(1, 10, 1))
    post(
      SparkListenerStageSubmitted(createStageInfo(0, 1000, rp = defaultProfile))
    )
  ...
  test("remove executors multiple profiles") {
    ...
    (1 to 10).map(_.toString).foreach { id =>
      onExecutorAdded(manager, id, rprof1)
    }
    (11 to 20).map(_.toString).foreach { id =>
      onExecutorAdded(manager, id, rprof2)
    }
    (21 to 30).map(_.toString).foreach { id =>
      onExecutorAdded(manager, id, defaultProfile)
    }
  ...
  private def speculativeTaskSubmitEventFromTaskIndex(
    ...
    val event = new SparkListenerSpeculativeTaskSubmitted(
      stageId,
      stageAttemptId,
      taskIndex = taskIndex,
  ...
  test("SPARK-31418: one stage being unschedulable") {
    ...
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 1
    )
    ...
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 2
    )
  ...
  test("SPARK-31418: multiple stages being unschedulable") {
    ...
    (0 to 2).foreach(execId =>
      onExecutorAddedDefaultProfile(manager, execId.toString)
    )
    ...
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 3
    )
    ...
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 4
    )
    ...
    assert(numExecutorsTarget(manager, defaultProfile.id) === 4)
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 5
    )
  }
  ...
  test("SPARK-31418: remove executors after unschedulable tasks end") {
    ...
    (0 to 4).foreach(execId =>
      onExecutorAddedDefaultProfile(manager, execId.toString)
    )
    (0 to 9)
      .map { i => createTaskInfo(i, i, executorId = s"${i / 2}") }
      .foreach { info =>
        post(SparkListenerTaskStart(0, 0, info))
      }
    ...
    (0 to 3).foreach { i =>
      assert(removeExecutorDefaultProfile(manager, i.toString))
    }
    ...
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 0
    )
  ...
  test(
    "SPARK-41192: remove executors when task finished before speculative task scheduled"
  ) {
    val clock = new ManualClock()
    ...
    (0 until 10).foreach(execId =>
      onExecutorAddedDefaultProfile(manager, execId.toString)
    )
    (0 until 40)
      .map { i => createTaskInfo(i, i, executorId = s"${i / 4}") }
      .foreach { info =>
        post(SparkListenerTaskStart(0, 0, info))
      }
    assert(numExecutorsTarget(manager, defaultProfile.id) === 10)
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 10
    )
  ...
  test("SPARK-30511 remove executors when speculative tasks end") {
    ...
    // 10 speculative tasks (30 - 39) launch for the remaining tasks
    (30 to 39).foreach { i =>
      post(speculativeTaskSubmitEventFromTaskIndex(0, taskIndex = i))
    }
    assert(addExecutorsToTargetForDefaultProfile(manager, updatesNeeded) === 1)
    doUpdateRequest(manager, updatesNeeded.toMap, clock.getTimeMillis())
    assert(addExecutorsToTargetForDefaultProfile(manager, updatesNeeded) === 1)
    ...
    post(
      SparkListenerTaskEnd(
        ...
        null,
        Success,
        createTaskInfo(36, 36, executorId = "9"),
        new ExecutorMetrics,
        null
      )
    )
    post(
      SparkListenerTaskEnd(
        ...
        null,
        TaskKilled("test"),
        createTaskInfo(46, 36, executorId = "11", speculative = true),
        new ExecutorMetrics,
        null
      )
    )
    ...
    post(
      SparkListenerTaskEnd(
        ...
        null,
        Success,
        createTaskInfo(37, 37, executorId = "9"),
        new ExecutorMetrics,
        null
      )
    )
    post(
      SparkListenerTaskEnd(
        ...
        null,
        Success,
        createTaskInfo(47, 37, executorId = "11", speculative = true),
        new ExecutorMetrics,
        null
      )
    )
    ...
    post(
      SparkListenerTaskEnd(
        ...
        null,
        UnknownReason,
        createTaskInfo(38, 38, executorId = "9"),
        new ExecutorMetrics,
        null
      )
    )
    post(
      SparkListenerTaskEnd(
        ...
        null,
        UnknownReason,
        createTaskInfo(49, 39, executorId = "12", speculative = true),
        new ExecutorMetrics,
        null
      )
    )
    ...
    post(
      SparkListenerTaskEnd(
        ...
        null,
        Success,
        createTaskInfo(39, 39, executorId = "9"),
        new ExecutorMetrics,
        null
      )
    )
    post(
      SparkListenerTaskEnd(
        ...
        null,
        Success,
        createTaskInfo(48, 38, executorId = "12", speculative = true),
        new ExecutorMetrics,
        null
      )
    )
  ...
  test("remove executors") {
    val manager = createManager(createConf(5, 10, 5))
    (1 to 10).map(_.toString).foreach { id =>
      onExecutorAddedDefaultProfile(manager, id)
    }
    ...
    assert(removeExecutorDefaultProfile(manager, "5"))
    assert(
      !removeExecutorDefaultProfile(manager, "6")
    ) // reached the limit of 5
  ...
  test(
    "SPARK-33763: metrics to track dynamic allocation (decommissionEnabled=false)"
  ) {
    val manager = createManager(createConf(3, 5, 3))
    (1 to 5).map(_.toString).foreach { id =>
      onExecutorAddedDefaultProfile(manager, id)
    }

    assert(executorsPendingToRemove(manager).isEmpty)
    assert(
      removeExecutorsDefaultProfile(manager, Seq("1", "2")) === Seq("1", "2")
    )
    assert(executorsPendingToRemove(manager).contains("1"))
    assert(executorsPendingToRemove(manager).contains("2"))
    ...
    assert(
      manager.executorAllocationManagerSource.driverKilled.getCount() === 1
    )
    assert(
      manager.executorAllocationManagerSource.exitedUnexpectedly
        .getCount() === 0
    )

    onExecutorRemoved(manager, "2", "another driver requested exit")
    assert(
      manager.executorAllocationManagerSource.driverKilled.getCount() === 2
    )
    assert(
      manager.executorAllocationManagerSource.exitedUnexpectedly
        .getCount() === 0
  ...
  test(
    "SPARK-33763: metrics to track dynamic allocation (decommissionEnabled = true)"
  ) {
    val manager =
      createManager(createConf(3, 5, 3, decommissioningEnabled = true))
    (1 to 5).map(_.toString).foreach { id =>
      onExecutorAddedDefaultProfile(manager, id)
    }
    ...
    assert(
      manager.executorAllocationManagerSource.gracefullyDecommissioned
        .getCount() === 1
    )
    assert(
      manager.executorAllocationManagerSource.decommissionUnfinished
        .getCount() === 0
    )
    assert(
      manager.executorAllocationManagerSource.exitedUnexpectedly
  ...
  test("remove multiple executors") {
    ...
    assert(executorsPendingToRemove(manager).contains("1"))
    assert(
      removeExecutorsDefaultProfile(manager, Seq("2", "3")) === Seq("2", "3")
    )
    assert(executorsPendingToRemove(manager).size === 3)
    assert(executorsPendingToRemove(manager).contains("2"))
    assert(executorsPendingToRemove(manager).contains("3"))
  ...
  test(
    "Removing with various numExecutorsTargetForDefaultProfileId condition"
  ) {
    ...
    assert(addExecutorsToTargetForDefaultProfile(manager, updatesNeeded) === 2)
    doUpdateRequest(manager, updatesNeeded.toMap, clock.getTimeMillis())
    (1 to 8).foreach(execId =>
      onExecutorAddedDefaultProfile(manager, execId.toString)
    )
    (1 to 8).map { i => createTaskInfo(i, i, s"$i") }.foreach { info =>
      post(SparkListenerTaskStart(0, 0, info))
    }
    assert(manager.executorMonitor.executorCount === 8)
    assert(numExecutorsTargetForDefaultProfileId(manager) === 8)
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) == 8
    )
    ...
    assert(
      addExecutorsToTargetForDefaultProfile(manager, updatesNeeded) === 0
    ) // upper limit
  }
  ...
  test("interleaving add and remove") {
    ...
    assert(removeExecutorDefaultProfile(manager, "1"))
    assert(
      removeExecutorsDefaultProfile(manager, Seq("2", "3", "4")) === Seq(
        "2",
        "3",
        "4"
      )
    ...
    assert(
      removeExecutorsDefaultProfile(manager, Seq("5", "6", "7")) === Seq(
        "5",
        "6",
        "7"
      )
    ...
    onExecutorRemoved(manager, "10")
    assert(
      addExecutorsToTargetForDefaultProfile(manager, updatesNeeded) === 4
    ) // at upper limit
    doUpdateRequest(manager, updatesNeeded.toMap, clock.getTimeMillis())
  ...
  test("starting/canceling add timer") {
    ...
    assert(
      firstAddTime === clock.nanoTime() + TimeUnit.SECONDS.toNanos(
    ...
    assert(
      secondAddTime === clock.nanoTime() + TimeUnit.SECONDS.toNanos(
        schedulerBacklogTimeout
      )
    )
    clock.advance(100L)
  ...
  test("mock polling loop add behavior") {
    ...
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 0
    ) // timer not exceeded yet
    clock.advance(schedulerBacklogTimeout * 1000)
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 1
    ) // first timer exceeded
    clock.advance(sustainedSchedulerBacklogTimeout * 1000 / 2)
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 1
    ) // second timer not exceeded yet
    clock.advance(sustainedSchedulerBacklogTimeout * 1000)
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 1 + 2
    ) // second timer exceeded
    clock.advance(sustainedSchedulerBacklogTimeout * 1000)
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 1 + 2 + 4
    ) // third timer exceeded
    ...
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 7
    ) // timer is canceled
    clock.advance(sustainedSchedulerBacklogTimeout * 1000)
    schedule(manager)
    ...
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 7 + 1
    ) // timer restarted
    clock.advance(sustainedSchedulerBacklogTimeout * 1000)
    schedule(manager)
    ...
    schedule(manager)
    assert(
      numExecutorsTargetForDefaultProfileId(manager) === 20
    ) // limit reached
  }
  ...
  test("mock polling loop remove with decommissioning") {
    ...
    schedule(manager)
    assert(
      executorsPendingToRemove(manager).isEmpty
    ) // limit reached (1 executor remaining)
    assert(
      executorsDecommissioning(manager).size === 2
    ) // limit reached (1 executor remaining)
    ...
    schedule(manager)
    assert(
      executorsDecommissioning(manager).size === 6
    ) // limit reached (1 executor remaining)
  }
  ...
  test("listeners trigger add executors correctly") {
    ...
    val taskInfos = (0 to numTasks - 1).map { i =>
      createTaskInfo(i, i, "executor-1")
    }
  ...
  test(
    "SPARK-8366: maxNumExecutorsNeededPerResourceProfile should properly handle failed tasks"
  ) {
    val manager = createManager(createConf())
    assert(
      maxNumExecutorsNeededPerResourceProfile(manager, defaultProfile) === 0
  ...
  test("reset the state of allocation manager") {
    ...
    assert(
      executorsPendingToRemove(manager) === Set("first", "second", "third")
    )
    assert(manager.executorMonitor.executorCount === 8)
  ...
  test(
    ...
    val clock = new ManualClock()
    val manager = createManager(createConf(1, 2, 1), clock = clock)
    ...
    assert(
      manager.executorMonitor.executorsPendingToRemove() === Set("executor-1")
    )
  }
  ...
  test("SPARK-26758 check executor target number after idle time out ") {
    ...
    post(
      SparkListenerExecutorAdded(
        clock.getTimeMillis(),
        "executor-3",
        new ExecutorInfo("host1", 3, Map.empty)
      )
    )
  ...
  private def createConf(
    ...
    val sparkConf = new SparkConf()
      ...
      .set(
        config.DYN_ALLOCATION_SCHEDULER_BACKLOG_TIMEOUT.key,
        s"${schedulerBacklogTimeout.toString}s"
      )
      .set(
        config.DYN_ALLOCATION_SUSTAINED_SCHEDULER_BACKLOG_TIMEOUT.key,
        s"${sustainedSchedulerBacklogTimeout.toString}s"
      )
      .set(
        config.DYN_ALLOCATION_EXECUTOR_IDLE_TIMEOUT.key,
  ...
  private val execInfo = new ExecutorInfo(
    "host1",
    1,
    Map.empty,
    Map.empty,
  ...
  private def onExecutorAdded(
    ...
    val execInfo =
      new ExecutorInfo("host1", cores, Map.empty, Map.empty, Map.empty, rp.id)
    post(SparkListenerExecutorAdded(0L, id, execInfo))
  }
  ...
  private def removeExecutorDefaultProfile(
    ...
    val executorsRemoved =
      removeExecutorsDefaultProfile(manager, Seq(executorId))
  ...
  private def executorsDecommissioning(
      manager: ExecutorAllocationManager
  ): Set[String] = {
...
private object ExecutorAllocationManagerSuite extends PrivateMethodTester {
  ...
  private val _addExecutorsToTarget =
    PrivateMethod[Int](Symbol("addExecutorsToTarget"))
  private val _removeExecutors =
    PrivateMethod[Seq[String]](Symbol("removeExecutors"))
  private val _onSchedulerBacklogged =
    PrivateMethod[Unit](Symbol("onSchedulerBacklogged"))
  private val _onSchedulerQueueEmpty =
    PrivateMethod[Unit](Symbol("onSchedulerQueueEmpty"))
  private val _localityAwareTasksPerResourceProfileId =
    PrivateMethod[mutable.HashMap[Int, Int]](
      Symbol("numLocalityAwareTasksPerResourceProfileId")
    )
  private val _rpIdToHostToLocalTaskCount =
    PrivateMethod[Map[Int, Map[String, Int]]](
      Symbol("rpIdToHostToLocalTaskCount")
    )
  ...
  private val defaultProfile =
    ResourceProfile.getOrCreateDefaultProfile(new SparkConf)

  private def numExecutorsToAddForDefaultProfile(
      manager: ExecutorAllocationManager
  ): Int = {
  ...
  private def numExecutorsTargetForDefaultProfileId(
      manager: ExecutorAllocationManager
  ): Int = {
  ...
  private def onSpeculativeTaskSubmitted(
      manager: ExecutorAllocationManager,
      id: String
  ): Unit = {
  ...
  private def localityAwareTasksForDefaultProfile(
      manager: ExecutorAllocationManager
  ): Int = {
    val localMap =
      manager invokePrivate _localityAwareTasksPerResourceProfileId()
    localMap(defaultProfile.id)
  }

  private def totalRunningTasksPerResourceProfile(
      manager: ExecutorAllocationManager
  ): Int = {
    manager invokePrivate _totalRunningTasksPerResourceProfile(
      defaultProfile.id
    )
  }
  ...
  private def getResourceProfileIdOfExecutor(
      manager: ExecutorAllocationManager
  ): Int = {
    defaultProfile.id
  }
