...
private[spark] class SparkUICssErrorHandler extends DefaultCssErrorHandler {
  ...
  /** Some libraries have warn/error messages that are too noisy for the tests; exclude them from
    ...
    */
  private val cssExcludeList =
    List("bootstrap.min.css", "vis-timeline-graph2d.min.css")

  private def isInExcludeList(uri: String): Boolean =
    cssExcludeList.exists(uri.endsWith)
...
class UISeleniumSuite extends SparkFunSuite with WebBrowser with Matchers {
    ...
    withSpark(newSparkContext()) { sc =>
      ...
      eventually(timeout(5.seconds), interval(50.milliseconds)) {
        ...
        val tableRowText = findAll(
          cssSelector("#rdd-storage-by-block-table td")
        ).map(_.text).toSeq
        tableRowText should contain(StorageLevels.DISK_ONLY.description)
      }
      ...
      (updatedStorageJson.children.head \ "storageLevel")
  ...
  test("attaching and detaching a new tab") {
    withSpark(newSparkContext()) { sc =>
      ...
      eventually(timeout(10.seconds), interval(50.milliseconds)) {
        ...
        find(
          cssSelector("""ul li a[href*="environment"]""")
        ) should not be (None)
  ...
  test("kill job POST/GET response is correct") {
    withSpark(newSparkContext(killEnabled = true)) { sc =>
      ...
      eventually(timeout(5.seconds), interval(50.milliseconds)) {
        val url =
          new URL(sc.ui.get.webUrl.stripSuffix("/") + "/jobs/job/kill/?id=0")
        // SPARK-6846: should be POST only but YARN AM doesn't proxy POST
        TestUtils.httpResponseCode(url, "GET") should be(200)
        TestUtils.httpResponseCode(url, "POST") should be(200)
      }
  ...
  test("live UI json application list") {
    withSpark(newSparkContext()) { sc =>
      val appListRawJson = HistoryServerSuite.getUrl(
        new URL(sc.ui.get.webUrl + "/api/v1/applications")
      )
  ...
  test("job stages should have expected dotfile under DAG visualization") {
    withSpark(newSparkContext()) { sc =>
      ...
      eventually(timeout(5.seconds), interval(100.milliseconds)) {
        val stage0 = Utils.tryWithResource(
          Source.fromURL(
            sc.ui.get.webUrl +
        ...
        val stage1 = Utils.tryWithResource(
          Source.fromURL(
            sc.ui.get.webUrl +
        ...
        val stage2 = Utils.tryWithResource(
          Source.fromURL(
            sc.ui.get.webUrl +
    ...
    withSpark(
      newSparkContext(
        ...
        additionalConfs = Map(
          ...
          LIVE_ENTITY_UPDATE_MIN_FLUSH_PERIOD.key -> "10ms"
        )
      )
    ) { sc =>
    ...
    Seq(true, false).foreach { timelineEnabled =>
      withSpark(newSparkContext(timelineEnabled = timelineEnabled)) { sc =>
        ...
        eventually(timeout(10.seconds), interval(50.milliseconds)) {
          ...
          assert(
            findAll(
              className("expand-application-timeline")
            ).nonEmpty === timelineEnabled
          )

          goToUi(sc, "/jobs/job/?id=0")
          assert(
            findAll(
              className("expand-job-timeline")
            ).nonEmpty === timelineEnabled
          )
          ...
          assert(
            findAll(
              className("expand-task-assignment-timeline")
            ).nonEmpty === timelineEnabled
          )
        }
      }
  ...
  test("SPARK-41365: Stage page can be accessed if URI was encoded twice") {
    withSpark(newSparkContext()) { sc =>
      ...
      eventually(timeout(5.seconds), interval(50.milliseconds)) {
        ...
        encodeParams.add(
          "order%255B0%255D%255Bcolumn%255D",
          "Locality%2520Level"
        )
        ...
        assert(
          decodeParams.getFirst("order[0][column]").equals("Locality Level")
        )
        assert(decodeParams.get("order[0][column]").size() == 2)
        assert(decodeParams.getFirst("search[value]").equals(""))

        val decodeQuery =
          "draw=2&order[0][column]=4&order[0][dir]=asc&start=0&length=20" +
            "&search[value]=&search[regex]=false&numTasks=10&columnIndexToSort=4" +
            "&columnNameToSort=Locality Level"
        val encodeOnceQuery =
          "draw=2&order%5B0%5D%5Bcolumn%5D=4&start=0&length=20" +
            "&search%5Bvalue%5D=&search%5Bregex%5D=false&numTasks=10&columnIndexToSort=4" +
            "&columnNameToSort=Locality%20Level"
        val encodeTwiceQuery =
          "draw=2&order%255B0%255D%255Bcolumn%255D=4&start=0&length=20" +
            "&search%255Bvalue%255D=&search%255Bregex%255D=false&numTasks=10&columnIndexToSort=4" +
            "&columnNameToSort=Locality%2520Level"
        val encodeOnceRes = Utils.tryWithResource(
          Source.fromURL(
            apiUrl(sc.ui.get, "stages/0/0/taskTable?" + encodeOnceQuery)
          )
        )(_.mkString)
        val encodeTwiceRes = Utils.tryWithResource(
          Source.fromURL(
            apiUrl(sc.ui.get, "stages/0/0/taskTable?" + encodeTwiceQuery)
          )
        )(_.mkString)
  ...
  test("SPARK-44895: Add 'daemon', 'priority' for ThreadStackTrace") {
    withSpark(newSparkContext()) { sc =>
      val uiThreads = getJson(sc.ui.get, "executors/driver/threads").children
  ...
  def apiUrl(ui: SparkUI, path: String): URL = {
    new URL(
      ui.webUrl + "/api/v1/applications/" + ui.sc.get.applicationId + "/" + path
    )
  }
}
