...
class CoarseGrainedSchedulerBackendSuite
  ...
  test(
    "SPARK-47458 compute max number of concurrent tasks with resources limiting"
  ) {
    withTempDir { dir =>
  ...
  Seq(
    ...
    test(
      s"SPARK-47663 end to end test validating if task cpus:${taskCpus} and " +
      ...
      withTempDir { dir =>
        ...
        val ret = sc
          .parallelize(1 to 20, numPartitions)
          .mapPartitions { _ =>
  ...
  test("custom log url for Spark UI is applied") {
    ...
    val listener = new SparkListener() {
      override def onExecutorAdded(
        ...
        assert(
          executorAdded.executorInfo.logUrlMap === Seq("stdout", "stderr").map {
            file =>
              file -> (baseUrl + s"/$file")
          }.toMap
        )
  ...
  test("extra resources from executor") {
    ...
    val conf = new SparkConf()
      ...
      .set(
        SCHEDULER_REVIVE_INTERVAL.key,
        "1m"
      ) // don't let it auto revive during test
      .set(
        EXECUTOR_INSTANCES,
        0
      ) // avoid errors about duplicate executor registrations
      .setMaster(
    ...
    val mockEndpointRef = mock[RpcEndpointRef]
    val mockAddress = mock[RpcAddress]
    when(mockEndpointRef.send(LaunchTask)).thenAnswer(
      (_: InvocationOnMock) => {}
    )

    val resources =
      Map(GPU -> new ResourceInformation(GPU, Array("0", "1", "3")))
    ...
    val taskCpus = 1
    val taskDescs: Seq[Seq[TaskDescription]] = Seq(
      Seq(
        new TaskDescription(
          1,
          0,
    ...
    when(ts.resourceOffers(any[IndexedSeq[WorkerOffer]], any[Boolean]))
      .thenReturn(taskDescs)
    ...
    infos.foreach { info =>
      assert(
        info.requestTime.get > 0,
        "Exec allocation and request times don't make sense"
      )
      assert(
        info.requestTime.get > testStartTime,
        "Exec allocation and request times don't make sense"
      )
      assert(
        info.registrationTime.get >= info.requestTime.get,
        "Exec allocation and request times don't make sense"
      )
    }
  }
  ...
  test("exec alloc decrease.") {
    ...
    infos.foreach { info =>
      info.requestTime.map { t =>
        assert(t > 0, "Exec request times don't make sense")
        assert(
          t >= testStartTime,
          "Exec allocation and request times don't make sense"
        )
        assert(
          t <= info.registrationTime.get,
          "Exec allocation and request times don't make sense"
        )
      }
    }
    assert(
      infos.filter(_.requestTime.isEmpty).length === 1,
  ...
  test(
    "SPARK-41766: New registered executor should receive decommission request" +
  ...
  override def createSchedulerBackend(
    ...
    masterURL match {
      case MOCK_REGEX(backendClassName) =>
        ...
        ctor
          .newInstance(scheduler, sc.env.rpcEnv)
          .asInstanceOf[SchedulerBackend]
    }
  }
...
private[spark] class TestCoarseGrainedSchedulerBackend(
...
private[spark] class MockExecutorRpcEndpointRef(conf: SparkConf)
