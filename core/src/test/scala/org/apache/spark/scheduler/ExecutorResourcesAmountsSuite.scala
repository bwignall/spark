...
class ExecutorResourcesAmountsSuite
    extends SparkFunSuite
    with ExecutorResourceUtils {

  implicit def toFractionalResource(
      resources: Map[String, Long]
  ): Map[String, Double] =
    resources.map { case (k, v) =>
      k -> ResourceAmountUtils.toFractionalResource(v)
    }

  implicit def toInternalResource(
  ...
  implicit def toInternalResourceMap(
      resources: Map[String, Map[String, Double]]
  ...
  test("assign to rp without task resources requirement") {
    ...
    assert(
      availableExecResAmounts.resourceAddressAmount === Map(
        "gpu" -> 3,
        "fpga" -> 2
      )
    )
  ...
  test("Convert ExecutorResourceInfos to ExecutorResourcesAmounts") {
      ...
      if (rName == "gpu") {
        assert(
          compareMaps(
            ...
            Map("2" -> 1.0, "4" -> 1.0, "6" -> 1.0)
          )
        )
      } else {
        assert(
          compareMaps(rInfo.resourcesAmounts, Map("aa" -> 1.0, "bb" -> 1.0))
        )
      }
    }
    ...
    availableRes.foreach { case (rName, addressesAmount) =>
      if (rName == "gpu") {
        assert(
          compareMaps(addressesAmount, Map("2" -> 1.0, "4" -> 1.0, "6" -> 1.0))
        )
      } else {
    ...
    executorsInfo.foreach { case (rName, rInfo) =>
      if (rName == "gpu") {
        assert(
          compareMaps(
            rInfo.resourcesAmounts,
            Map("2" -> 0.6, "4" -> 1.0, "6" -> 0.4)
          )
        )
      } else {
        assert(
          compareMaps(rInfo.resourcesAmounts, Map("aa" -> 0.8, "bb" -> 0.3))
        )
      }
    }
    ...
    assert(
      availableExecResAmounts1.resourceAddressAmount === Map(
        "gpu" -> 3,
        "fpga" -> 2
      )
    )
    ...
    availableRes1.foreach { case (rName, addressesAmount) =>
      if (rName == "gpu") {
        assert(
          compareMaps(addressesAmount, Map("2" -> 0.6, "4" -> 1.0, "6" -> 0.4))
        )
      } else {
  ...
  test("the total amount after release should be <= 1.0") {
    ...
    val e = intercept[SparkException] {
      availableExecResAmounts.release(
        toInternalResourceMap(Map("gpu" -> Map("2" -> 0.7)))
      )
    }
    assert(
      e.getMessage.contains("after releasing gpu address 2 should be <= 1.0")
    )

    availableExecResAmounts.release(
      toInternalResourceMap(Map("gpu" -> Map("2" -> 0.6)))
    )
    assert(
      compareMaps(
        availableExecResAmounts.availableResources("gpu"),
        Map("2" -> 1.0)
      )
    )
  }
  ...
  test("the total amount after acquire should be >= 0") {
    ...
    assert(
      e.getMessage.contains("after acquiring gpu address 2 should be >= 0")
    )

    availableExecResAmounts.acquire(
      toInternalResourceMap(Map("gpu" -> Map("2" -> 0.4)))
    )
    assert(
      compareMaps(
        availableExecResAmounts.availableResources("gpu"),
    ...
    availableRes.foreach { case (rName, addressesAmount) =>
      if (rName == "gpu") {
        ...
        assert(
          compareMaps(
            addressesAmount,
            Map("aa" -> (1.0 - fpgaTaskAmount), "bb" -> 1.0)
          )
        )
      }
    }
  ...
  test("assign/release resource for different task requirements") {
    ...
    def testAllocation(
      ...
      assigned.foreach { resources =>
        assert(
          resources("gpu").values.toArray.sorted
            .map(ResourceAmountUtils.toFractionalResource)
    ...
    testRelease(
      releasedRes = Map("0" -> 0.80002, "1" -> 1.0, "2" -> 0.4, "3" -> 1.0),
    ...
    testRelease(
      releasedRes = Map("0" -> 1.0, "1" -> 1.0, "2" -> 1.0, "3" -> 1.0),
      expectedLeftRes = Map("0" -> 1.0, "1" -> 1.0, "2" -> 1.0, "3" -> 1.0)
    )
  }
  ...
  test("Don't allow acquire resource or address that is not available") {
    ...
    val e = intercept[SparkException] {
      availableExecResAmounts.acquire(
        toInternalResourceMap(Map("fpga" -> Map("1" -> 1.0)))
      )
    }
    assert(
      e.getMessage.contains(
        "Try to acquire an address from fpga that doesn't exist"
      )
    )
    ...
    val e1 = intercept[SparkException] {
      availableExecResAmounts.acquire(
        toInternalResourceMap(Map("gpu" -> Map("6" -> 1.0)))
      )
    }
    assert(
      e1.getMessage.contains("Try to acquire an address that doesn't exist")
    )
  }
  ...
  test("Don't allow release resource or address that is not available") {
    // Init Executor Resource.
    val totalRes =
      Map("gpu" -> Map("0" -> 0.5, "1" -> 0.5, "2" -> 0.5, "3" -> 0.5))
    val availableExecResAmounts = new ExecutorResourcesAmounts(totalRes)

    // Acquire an address from a resource that doesn't exist
    val e = intercept[SparkException] {
      availableExecResAmounts.release(
        toInternalResourceMap(Map("fpga" -> Map("1" -> 0.1)))
      )
    }
    assert(
      e.getMessage.contains(
        "Try to release an address from fpga that doesn't exist"
      )
    )
    ...
    val e1 = intercept[SparkException] {
      availableExecResAmounts.release(
        toInternalResourceMap(Map("gpu" -> Map("6" -> 0.1)))
      )
    }
    assert(
      e1.getMessage.contains("Try to release an address that is not assigned")
    )
  }
