...
class FakeTaskScheduler(
  ...
  def addExecutor(execId: String, host: String): Unit = {
    ...
    val executorsOnHost =
      hostToExecutors.getOrElseUpdate(host, new mutable.HashSet[String])
    executorsOnHost += execId
...
/** A Task implementation that results in a large serialized task.
  */
class LargeTask(stageId: Int)
    extends Task[Array[Byte]](
      stageId,
      0,
      0,
      1,
  ...
  val randomBuffer =
    new Array[Byte](TaskSetManager.TASK_SIZE_TO_WARN_KIB * 1024)
...
class TaskSetManagerSuite
  ...
  test("SPARK-46383: TaskInfo accumulables are cleared upon task completion") {
    ...
    val manager =
      new TaskSetManager(sched, taskSet, MAX_TASK_FAILURES, clock = clock)
    ...
    assert(
      manager.taskAttempts
        .flatMap(t => t.filter(!_.accumulables.isEmpty))
        .isEmpty
    )
    ...
    manager.handleSuccessfulTask(
      taskOption1.get.taskId,
      createTaskResult(1, accumUpdates)
    )
  ...
  test("basic delay scheduling") {
    ...
    assert(
      manager.resourceOffer("exec1", "host1", NODE_LOCAL)._1.get.index == 2
    )

    // Offer host2, exec2, at NODE_LOCAL level: we should choose task 2
    assert(
      manager.resourceOffer("exec2", "host2", NODE_LOCAL)._1.get.index == 1
    )
  ...
  test(
    ...
    // First offer host1, exec1: first task should be chosen
    assert(
      manager.resourceOffer("exec1", "host1", PROCESS_LOCAL)._1.get.index === 0
    )
    assert(
      manager.resourceOffer("exec3", "host2", PROCESS_LOCAL)._1.get.index === 1
    )
  ...
  test("delay scheduling with fallback") {
    ...
    val (taskDescription, delayReject, _) =
      manager.resourceOffer("exec2", "host2", ANY)
    ...
    val (noPendingTask, noPendingReject, _) =
      manager.resourceOffer("exec2", "host2", ANY)
  ...
  test(
    "executors should be excluded after task failure, in spite of locality preferences"
  ) {
    ...
    val healthTrackerOpt =
      Some(new HealthTracker(mockListenerBus, conf, None, clock))
    val manager = new TaskSetManager(sched, taskSet, 4, healthTrackerOpt, clock)

    {
      val offerResult =
        manager.resourceOffer("exec1", "host1", PROCESS_LOCAL)._1
  ...
  test("Executors exit for reason unrelated to currently running tasks") {
    ...
    manager.executorLost(
      "execC",
      "host2",
      ExecutorExited(1, true, "Terminated due to issue with running tasks")
    )
    assert(sched.taskSetsFailed.contains(taskSet.id))
  }

  test(
    "SPARK-31837: Shift to the new highest locality level if there is when recomputeLocality"
  ) {
    sc = new SparkContext("local", "test")
    sched = new FakeTaskScheduler(sc)
    val taskSet = FakeTask.createTaskSet(
  ...
  test(
    "SPARK-41469: task doesn't need to rerun on executor lost if shuffle data has migrated"
  ) {
    sc = new SparkContext("local", "test")
    sched = new FakeTaskScheduler(sc)
    ...
    val taskSet = FakeTask.createShuffleMapTaskSet(
      2,
      0,
      0,
    ...
    mapOutputTracker.updateMapOutput(
      0,
      taskId,
      BlockManagerId("exec1", "host1", 8848)
    )
  ...
  test(
    "SPARK-32653: Decommissioned host should not be used to calculate locality levels"
  ) {
    sc = new SparkContext("local", "test")
    sched = new FakeTaskScheduler(sc)
    val backend = mock(classOf[SchedulerBackend])
    ...
    sched.executorDecommission(
      exec0,
      ExecutorDecommissionInfo("test", Some(host0))
    )
    sched.executorDecommission(
      exec1,
      ExecutorDecommissionInfo("test", Some(host0))
  ...
  test(
    "SPARK-32653: Decommissioned executor should not be used to calculate locality levels"
  ) {
    sc = new SparkContext("local", "test")
    sched = new FakeTaskScheduler(sc)
    val backend = mock(classOf[SchedulerBackend])
    ...
    val taskSet =
      FakeTask.createTaskSet(1, Seq(ExecutorCacheTaskLocation(host0, exec0)))
    sched.submitTasks(taskSet)
    val manager = sched.taskSetManagerForAttempt(0, 0).get
  ...
  test("test RACK_LOCAL tasks") {
    ...
    assert(
      manager.resourceOffer("execB", "host2", RACK_LOCAL)._1.get.index === 1
    )
  }
  ...
  test("abort the job if total size of results is too large") {
    ...
    assert(
      thrown2.getMessage().contains("bigger than spark.driver.maxResultSize")
    )
  }
  ...
  test(
    ...
    val task1 =
      manager.resourceOffer("execA", "host1", TaskLocality.PROCESS_LOCAL)._1.get
  ...
  test(
    ...
    for (id <- Set(0, 1)) {
      manager.handleSuccessfulTask(
        id,
        createTaskResult(id, accumUpdatesByTask(id))
      )
  ...
  test("speculative and noPref task should be scheduled after node-local") {
    ...
    assert(
      manager.resourceOffer("execA", "host1", PROCESS_LOCAL)._1.get.index === 0
    )
  ...
  test(
    ...
    assert(
      manager.resourceOffer("execA", "host1", NODE_LOCAL)._1.get.index === 0
    )
    assert(
      manager.resourceOffer("execA", "host2", NODE_LOCAL)._1.get.index === 1
    )
    assert(
      manager.resourceOffer("execA", "host3", NODE_LOCAL)._1.get.index === 3
    )
  ...
  test(
    ...
    assert(
      manager.resourceOffer("execA", "host1", NODE_LOCAL)._1.get.index === 2
    )
    assert(
      manager.resourceOffer("execB", "host2", NODE_LOCAL)._1.get.index === 3
    )
    // node-local tasks are scheduled without delay
    assert(
      manager.resourceOffer("execA", "host1", NODE_LOCAL)._1.get.index === 0
    ...
    assert(
      manager.resourceOffer("execB", "host2", NODE_LOCAL)._1.get.index === 1
    )
    assert(manager.resourceOffer("execA", "host1", NODE_LOCAL)._1 === None)
  ...
  test(
    ...
    // process-local tasks are scheduled first
    assert(
      manager.resourceOffer("execA", "host1", PROCESS_LOCAL)._1.get.index === 1
    )
    assert(
      manager.resourceOffer("execB", "host2", PROCESS_LOCAL)._1.get.index === 2
    )
  ...
  test("Test TaskLocation for different host type.") {
    ...
    assert(
      TaskLocation("executor_some.host1_executor_task_3") ===
  ...
  test(
      ...
      tasks
        ...
        .getOrElse {
          throw new RuntimeException(
            s"couldn't find index $index in " +
    ...
    val originalTasks = Seq(3, 4).map { index =>
      index -> runningTaskForIndex(index)
    }.toMap
  ...
  test(
    ...
    when(mockDAGScheduler.taskEnded(any(), any(), any(), any(), any(), any()))
  ...
  test(
    ...
    val taskSet2 =
      FakeTask.createTaskSet(numTasks = 1, stageId = 0, stageAttemptId = 1)
    val manager2 = new TaskSetManager(
      sched,
      taskSet2,
      MAX_TASK_FAILURES,
      clock = new ManualClock
    ...
    val taskSet3 =
      FakeTask.createTaskSet(numTasks = 1, stageId = 1, stageAttemptId = 1)
    val manager3 = new TaskSetManager(
      sched,
      taskSet3,
      MAX_TASK_FAILURES,
      clock = new ManualClock
  ...
  test(
    "don't update excludelist for shuffle-fetch failures, preemption, denied commits, " +
      "or killed tasks"
  ) {
    // Setup a taskset, and fail some tasks for a fetch failure, preemption, denied commit,
    // and killed task.
    val conf = new SparkConf().set(config.EXCLUDE_ON_FAILURE_ENABLED, true)
    sc = new SparkContext("local", "test", conf)
    sched = new FakeTaskScheduler(sc, ("exec1", "host1"), ("exec2", "host2"))
    val taskSet = FakeTask.createTaskSet(4)
    ...
    val taskDescs = Seq(
      ...
      (0 until 2).flatMap { _ =>
        tsmSpy.resourceOffer(exec, host, TaskLocality.ANY)._1
      }
    }
    assert(taskDescs.size === 4)

    // now fail those tasks
    tsmSpy.handleFailedTask(
      taskDescs(0).taskId,
      TaskState.FAILED,
      FetchFailed(
        BlockManagerId(taskDescs(0).executorId, "host1", 12345),
        0,
        0L,
        0,
        0,
  ...
  test("update application healthTracker for shuffle-fetch") {
    ...
    val taskDescs = Seq(
      ...
      // offer each executor twice (simulating 2 cores per executor)
      (0 until 2).flatMap { _ =>
        tsm.resourceOffer(exec, host, TaskLocality.ANY)._1
      }
    }
    assert(taskDescs.size === 4)
  ...
  test(
    "update healthTracker before adding pending task to avoid race condition"
  ) {
    ...
    val taskSetManager =
      new TaskSetManager(sched, taskSet, 1, Some(healthTracker))
    val taskSetManagerSpy = spy[TaskSetManager](taskSetManager)

    val taskDesc =
      taskSetManagerSpy.resourceOffer(exec, host, TaskLocality.ANY)._1

    // Assert the task has been black listed on the executor it was last executed on.
    when(taskSetManagerSpy.addPendingTask(anyInt(), anyBoolean(), anyBoolean()))
      .thenAnswer((invocationOnMock: InvocationOnMock) => {
        val task: Int = invocationOnMock.getArgument(0)
        assert(
          taskSetManager.taskSetExcludelistHelperOpt.get
  ...
  test("SPARK-21563 context's added jars shouldn't change mid-TaskSet") {
    ...
    val manager1 = new TaskSetManager(
      sched,
      taskSet1,
      MAX_TASK_FAILURES,
      clock = new ManualClock
  ...
  test("SPARK-24677: Avoid NoSuchElementException from MedianHeap") {
    ...
    sched.submitTasks(taskSet)
    sched.resourceOffers((0 until 8).map { idx =>
  ...
  test("SPARK-13704 Rack Resolution is done with a batch of de-duped hosts") {
    ...
    assert(
      manager
        .resourceOffer("otherExec", "otherHost", TaskLocality.RACK_LOCAL)
        ._1
        .isEmpty
    )
  ...
  test("TaskSetManager passes task resource along") {
    ...
    val taskResourceAssignments =
      Map(GPU -> Map("0" -> ONE_ENTIRE_RESOURCE, "1" -> ONE_ENTIRE_RESOURCE))
    val taskOption =
      manager
        .resourceOffer("exec1", "host1", NO_PREF, 2, taskResourceAssignments)
        ._1
    assert(taskOption.isDefined)
    ...
    assert(
      allocatedResources(GPU).keys.toArray.sorted sameElements Array("0", "1")
    )
  ...
  private def testSpeculationDurationSetup(
      ...
      numCoresPerTask: Int,
      speculationMinimumThreshold: Option[String]
  ): (TaskSetManager, ManualClock) = {
    val conf = new SparkConf()
    ...
    if (speculationThresholdOpt.isDefined) {
      conf.set(
        config.SPECULATION_TASK_DURATION_THRESHOLD.key,
        speculationThresholdOpt.get
      )
    }
    if (speculationMinimumThreshold.isDefined) {
      conf.set(
        config.SPECULATION_MIN_THRESHOLD.key,
  ...
  Seq(1, 2).foreach { numTasks =>
    test(
      "SPARK-29976 when a speculation time threshold is provided, should speculative " +
    ...
    test(
      "SPARK-29976: when the speculation time threshold is not provided," +
  ...
  test(
    "SPARK-29976 when a speculation time threshold is provided, should not speculative " +
  ...
  test(
    "SPARK-21040: Check speculative tasks are launched when an executor is decommissioned" +
    ...
    assert(
      sched.getExecutorDecommissionState("exec2").map(_.startTime) ===
  ...
  test(
    "SPARK-29976 Regular speculation configs should still take effect even when a " +
  ...
  test(
    "SPARK-30417 when spark.task.cpus is greater than spark.executor.cores due to " +
  ...
  test(
    "SPARK-30359: don't clean executorsPendingToRemove " +
    ...
    val tasks = Array.tabulate[Task[_]](2)(partition =>
      new FakeLongTasks(stageId = 0, partition)
    )
    val taskSet: TaskSet = new TaskSet(
      tasks,
      stageId = 0,
      stageAttemptId = 0,
      priority = 0,
    ...
    eventually(timeout(10.seconds), interval(100.milliseconds)) {
      assert(manager.taskInfos(taskId0).running)
      assert(
        manager.taskInfos(taskId1).running && !manager
          .taskInfos(taskId1)
          .launching
      )
    }
  ...
  test(
    "SPARK-33741 Test minimum amount of time a task runs " +
  ...
  private def createTaskMetrics(
      ...
      efficientMultiplier: Double = 0.6
  ): Array[TaskMetrics] = {
  ...
  private def updateAndGetTaskMetrics(
      ...
      efficientMultiplier: Double
  ): TaskMetrics = {
    taskMetrics.inputMetrics.setRecordsRead(
      (efficientMultiplier * RECORDS_NUM).toLong
    )
    taskMetrics.shuffleReadMetrics.setRecordsRead(
      (efficientMultiplier * RECORDS_NUM).toLong
    )
  ...
  test(
    "SPARK-32170: test SPECULATION_MIN_THRESHOLD for speculating inefficient tasks"
  ) {
    ...
    (speculativeAllDurations ++ speculativeInefficientDurations
      ...
      val manager =
        new TaskSetManager(sched, taskSet, MAX_TASK_FAILURES, clock = clock)
      val taskMetricsByTask =
        createTaskMetrics(taskSet, Set(3), efficientMultiplier = 0.4)
      ...
      val task3Metrics: TaskMetrics =
        ser.deserialize(
          ByteBuffer.wrap(ser.serialize(taskMetricsByTask(3)).array())
        )
      sched.executorHeartbeatReceived(
        "exec1",
        Array((3, task3Metrics.internalAccums)),
        blockManagerId,
        mutable.Map.empty[(Int, Int), ExecutorMetrics]
      )

      updateAndGetTaskMetrics(taskMetricsByTask(4), efficientMultiplier = 5)
      val task4Metrics: TaskMetrics =
        ser.deserialize(
          ByteBuffer.wrap(ser.serialize(taskMetricsByTask(4)).array())
        )
      sched.executorHeartbeatReceived(
        "exec1",
        Array((4, task4Metrics.internalAccums)),
        blockManagerId,
        mutable.Map.empty[(Int, Int), ExecutorMetrics]
      )
      for (id <- Set(0, 1, 2)) {
        val resultBytes = ser.serialize(
          createTaskResult(id, taskMetricsByTask(id).internalAccums)
        )
        sched.statusUpdate(
          tid = id,
  ...
  test(
    "SPARK-32170: test SPECULATION_EFFICIENCY_TASK_PROCESS_RATE_MULTIPLIER for " +
    ...
    Seq(0.5, 0.8).foreach(processMultiplier => {
      sc.conf.set(
        config.SPECULATION_EFFICIENCY_TASK_PROCESS_RATE_MULTIPLIER,
        processMultiplier
      )
      sched = new FakeTaskScheduler(sc, ("exec1", "host1"), ("exec2", "host2"))
      val taskSet = FakeTask.createTaskSet(4)
      ...
      val manager =
        new TaskSetManager(sched, taskSet, MAX_TASK_FAILURES, clock = clock)
      val taskMetricsByTask =
        createTaskMetrics(taskSet, Set(3), efficientMultiplier = 0.6)
      val blockManagerId = BlockManagerId("exec1", "localhost", 12345)
      // offer resources for 4 tasks to start
      for (
        (k, v) <- List(
      ...
      sched.executorHeartbeatReceived(
        "exec1",
        Array((3, taskMetrics.internalAccums)),
        blockManagerId,
        mutable.Map.empty[(Int, Int), ExecutorMetrics]
      )
      for (id <- Set(0, 1, 2)) {
  ...
  test(
    "SPARK-32170: test SPECULATION_EFFICIENCY_TASK_DURATION_FACTOR for " +
      "speculating tasks"
  ) {
    // set the speculation multiplier to be 0, so speculative tasks are launched based on
    // minTimeToSpeculation parameter to checkSpeculatableTasks
  ...
  test("SPARK-37580: Reset numFailures when one of task attempts succeeds") {
    ...
    val tasks = new ArrayBuffer[TaskDescription]()
    for (
      (k, v) <- List("exec1" -> "host1", "exec2" -> "host2", "exec3" -> "host3")
    ) {
      val taskOption = manager.resourceOffer(k, v, NO_PREF)._1
      assert(taskOption.isDefined)
      val task = taskOption.get
    ...
    val numFailuresField =
      classOf[TaskSetManager].getDeclaredField("numFailures")
  ...
  test(
    "SPARK-40094: Send TaskEnd if task failed with " +
    ...
    val executorTracker1 = executorMonitor.ensureExecutorIsTracked(
      "exec1",
      ResourceProfile.DEFAULT_RESOURCE_PROFILE_ID
    )
    executorTracker1.updateRunningTasks(1)
    val executorTracker2 = executorMonitor.ensureExecutorIsTracked(
      "exec2",
      ResourceProfile.DEFAULT_RESOURCE_PROFILE_ID
    )
    ...
    manager1.handleFailedTask(
      offerResult1.get.taskId,
      TaskState.FAILED,
      reason1
    )
    ...
    manager2.handleFailedTask(
      offerResult2.get.taskId,
      TaskState.FAILED,
      reason2
    )
...
class FakeLongTasks(stageId: Int, partitionId: Int)
