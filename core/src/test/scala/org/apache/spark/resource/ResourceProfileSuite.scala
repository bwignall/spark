...
class ResourceProfileSuite extends SparkFunSuite with MockitoSugar {
  ...
  test("Default ResourceProfile") {
    ...
    assert(
      rprof.executorResources(ResourceProfile.CORES).amount === 1,
      "Executor resources should have 1 core"
    )
    assert(
      rprof.getExecutorCores.get === 1,
      "Executor resources should have 1 core"
    )
    assert(
      rprof.executorResources(ResourceProfile.MEMORY).amount === 1024,
      "Executor resources should have 1024 memory"
    )
    assert(
      rprof.executorResources.get(ResourceProfile.PYSPARK_MEM) == None,
      "pyspark memory empty if not specified"
    )
    assert(
      rprof.executorResources.get(ResourceProfile.OVERHEAD_MEM) == None,
      "overhead memory empty if not specified"
    )
    assert(
      rprof.executorResources(ResourceProfile.OFFHEAP_MEM).amount === 0,
      "Executor resources should have 0 offheap memory"
    )
    assert(
      rprof.taskResources.size === 1,
      "Task resources should just contain cpus by default"
    )
    assert(
      rprof.taskResources(ResourceProfile.CPUS).amount === 1,
  ...
  test("Default ResourceProfile with app level resources specified") {
    ...
    assert(
      rprof.executorResources(ResourceProfile.CORES).amount === 4,
      "Executor resources should have 4 core"
    )
    assert(
      rprof.getExecutorCores.get === 4,
      "Executor resources should have 4 core"
    )
    assert(
      rprof.executorResources(ResourceProfile.MEMORY).amount === 4096,
      "Executor resources should have 1024 memory"
    )
    assert(
      rprof.executorResources(ResourceProfile.PYSPARK_MEM).amount == 2048,
      "pyspark memory empty if not specified"
    )
    assert(
      rprof.executorResources(ResourceProfile.OVERHEAD_MEM).amount == 1024,
      "overhead memory empty if not specified"
    )
    assert(
      rprof.executorResources(ResourceProfile.OFFHEAP_MEM).amount == 3,
      "Executor resources should have 3 offHeap memory"
    )
    assert(
      rprof.taskResources.size === 2,
      "Task resources should just contain cpus and gpu"
    )
    assert(
      rprof.taskResources.contains("gpu"),
      "Task resources should have gpu"
    )
  }
  ...
  test("maxTasksPerExecutor cpus") {
    ...
    val immrprof =
      new ResourceProfile(rprof.executorResources, rprof.taskResources)
  ...
  test("maxTasksPerExecutor/limiting executor cores") {
    val sparkConf =
      new SparkConf().setMaster("spark://testing").set(EXECUTOR_CORES, 2)
    val rprof = new ResourceProfileBuilder()
    val taskReq = new TaskResourceRequests().resource("gpu", 1)
    val execReq =
      new ExecutorResourceRequests().resource("gpu", 2, "myscript", "nvidia")
  ...
  test("Create ResourceProfile") {
    ...
    assert(
      rprof.executorResources.contains("gpu"),
      "Executor resources should have gpu"
    )
    assert(
      rprof.executorResources.get("gpu").get.vendor === "nvidia",
      "gpu vendor should be nvidia"
    )
    assert(
      rprof.executorResources.get("gpu").get.discoveryScript === "myscript",
      "discoveryScript should be myscript"
    )
    assert(
      rprof.executorResources.get("gpu").get.amount === 2,
    ...
    assert(
      rprof.taskResources.get("gpu").get.amount === 1,
    ...
    assert(
      rprof.executorResources(ResourceProfile.CORES).amount === 2,
    ...
    assert(
      rprof.executorResources(ResourceProfile.OVERHEAD_MEM).amount === 2048,
      "Executor resources should have 2048 overhead memory"
    )
    assert(
      rprof.executorResources(ResourceProfile.PYSPARK_MEM).amount === 1024,
      "Executor resources should have 1024 pyspark memory"
    )
    assert(
      rprof.executorResources(ResourceProfile.OFFHEAP_MEM).amount === 3072,
      "Executor resources should have 3072 offHeap memory"
    )

    assert(rprof.taskResources.size === 2)
    assert(
      rprof.taskResources("cpus").amount === 1,
      "Task resources should have cpu"
    )
  }
  ...
  test("test ResourceProfiles equal") {
    ...
    assert(
      rprof.resourcesEqual(rprof2),
      "resource profile resourcesEqual not working"
    )
  }
  ...
  test("test TaskResourceProfiles equal") {
    ...
    assert(
      rprof.resourcesEqual(rprof2),
      "task resource profile resourcesEqual not working"
    )
  }
  ...
  test("Test ExecutorResourceRequests memory helpers") {
    ...
    assert(
      rprof.executorResources(ResourceProfile.OVERHEAD_MEM).amount === 2000,
      "Executor resources should have 2000 overhead memory"
    )
    assert(
      rprof.executorResources(ResourceProfile.PYSPARK_MEM).amount === 500,
      "Executor resources should have 512 pyspark memory"
    )
    assert(
      rprof.executorResources(ResourceProfile.OFFHEAP_MEM).amount === 1024,
  ...
  test("Test TaskResourceRequest fractional") {
    ...
    assert(
      rprof.taskResources.contains("fpga"),
      "Task resources should have gpu"
    )
    assert(
    ...
    assert(
      taskError.contains(
        "The resource amount 1.5 must be either <= 1.0, or a whole number."
      )
    )
  ...
  test("SPARK-45527 fractional TaskResourceRequests in ResourceProfile") {
    ...
    assert(
      msg.contains(
        "The task resource amount 0.7 must be either <= 0.5, or a whole number"
      )
    )
  }
