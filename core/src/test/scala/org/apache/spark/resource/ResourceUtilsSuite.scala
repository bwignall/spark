...
class ResourceUtilsSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("Resource discoverer multiple resource types") {
    ...
    withTempDir { dir =>
      ...
      val fpgaDiscovery = createTempScriptWithExpectedOutput(
        dir,
        "fpgDiscoverScript",
      ...
      val fooDiscovery = createTempScriptWithExpectedOutput(
        dir,
        "fooDiscoverScript",
      ...
      assert(
        gpuValue.get.addresses.sameElements(Array("0", "1")),
        "should have 0,1 entries"
      )
      ...
      assert(
        fpgaValue.get.addresses.sameElements(Array("f1", "f2", "f3")),
  ...
  test("get from resources file and discover the remaining") {
    ...
    withTempDir { dir =>
      ...
      val resourcesFile = createTempJsonFile(
        dir,
        "resources",
        Extraction.decompose(Seq(fpgaAllocation))
      )
  ...
  test("list resource ids") {
    ...
    val resourcesMap = listResourceIds(conf, SPARK_DRIVER_PREFIX).map { rId =>
      (rId.resourceName, 1)
    }.toMap
  ...
  test("parse resource request") {
    ...
    assert(
      request.discoveryScript === Optional.empty(),
    ...
    assert(request.amount === 2, "GPU count should be 2")
    assert(
      request.discoveryScript.get === discoveryScript,
      "should get discovery script"
    )
  ...
  test("Resource discoverer script returns mismatched name") {
    ...
    withTempDir { dir =>
      ...
      assert(
        error.contains(
          s"Error running the resource discovery script $gpuDiscovery: " +
  ...
  test("gpu's specified but not a discovery script") {
    val request = new ResourceRequest(
      EXECUTOR_GPU_ID,
      2,
      Optional.empty[String],
      Optional.empty[String]
    ...
    assert(
      error.contains(
        "User is expecting to use resource: gpu, but " +
  ...
  test("SPARK-45527 warnOnWastedResources for ResourceProfile") {
    ...
    Seq(
      (
        ...
        0.1,
        5
      ), // cpu limiting task number=10/2=5, gpu limiting task number = 1/0.1 = 10
      (
        10,
        3,
        1,
        0.1,
        3
      ), // cpu limiting task number=10/3=3, gpu limiting task number = 1/0.1 = 10
      (
        10,
        4,
        1,
        0.1,
        2
      ), // cpu limiting task number=10/4=2, gpu limiting task number = 1/0.1 = 10
      (
        10,
        5,
        1,
        0.1,
        2
      ), // cpu limiting task number=10/5=2, gpu limiting task number = 1/0.1 = 10
      (
        10,
        6,
        1,
        0.1,
        1
      ), // cpu limiting task number=10/6=1, gpu limiting task number = 1/0.1 = 10
      (
        10,
        10,
        1,
      ...
      (
        ...
        0.1,
        2
      ), // cpu limiting task number=20/7=3, gpu limiting task number = 1/0.1 = 10
      (
        30,
        7,
        1,
        0.1,
        4
      ), // cpu limiting task number=30/7=4, gpu limiting task number = 1/0.1 = 10
      (
        50,
        14,
        1,
        0.1,
        3
      ) // cpu limiting task number=50/14=3, gpu limiting task number = 1/0.1=10
    ).foreach {
      case (
            executorCores,
            taskCpus: Int,
        ...
        ereqs = new ExecutorResourceRequests()
          .cores(executorCores)
          .resource("gpu", executorGpus)
        treqs =
          new TaskResourceRequests().cpus(taskCpus).resource("gpu", taskGpus)
        rp = new ResourceProfileBuilder().require(ereqs).require(treqs).build()
        ...
        assert(
          msg.contains(
            "The configuration of resource: gpu (exec = 1, task = 0.1/10, runnable " +
    ...
    Seq(
      (
        ...
        1.0 / 9,
        9
      ), // cpu limiting task number=10, gpu limiting task number = 9
      (
        10,
        1,
        1,
        1.0 / 8,
        8
      ), // cpu limiting task number=10, gpu limiting task number = 8
      (
        10,
        1,
        1,
        1.0 / 7,
        7
      ), // cpu limiting task number=10, gpu limiting task number = 7
      (
        10,
        1,
        1,
        1.0 / 6,
        6
      ), // cpu limiting task number=10, gpu limiting task number = 6
      (
        10,
        1,
        1,
        1.0 / 5,
        5
      ), // cpu limiting task number=10, gpu limiting task number = 5
      (
        10,
        1,
        1,
        1.0 / 4,
        4
      ), // cpu limiting task number=10, gpu limiting task number = 4
      (
        10,
        1,
        1,
        1.0 / 3,
        3
      ), // cpu limiting task number=10, gpu limiting task number = 3
      (
        10,
        1,
        1,
        1.0 / 2,
        2
      ), // cpu limiting task number=10, gpu limiting task number = 2
      (
        10,
        1,
        1,
        1.0,
        1
      ), // cpu limiting task number=10, gpu limiting task number = 1
      (
        30,
        1,
        2,
        1.0 / 9,
        2 * 9
      ), // cpu limiting task number=30, gpu limiting task number = 2*9
      (
        30,
        1,
        2,
        1.0 / 8,
        2 * 8
      ), // cpu limiting task number=30, gpu limiting task number = 2*8
      (
        30,
        1,
        2,
        1.0 / 7,
        2 * 7
      ), // cpu limiting task number=30, gpu limiting task number = 2*7
      (
        30,
        1,
        2,
        1.0 / 6,
        2 * 6
      ), // cpu limiting task number=30, gpu limiting task number = 2*6
      (
        30,
        1,
        2,
        1.0 / 5,
        2 * 5
      ), // cpu limiting task number=30, gpu limiting task number = 2*5
      (
        30,
        1,
        2,
        1.0 / 4,
        2 * 4
      ), // cpu limiting task number=30, gpu limiting task number = 2*4
      (
        30,
        1,
        2,
        1.0 / 3,
        2 * 3
      ), // cpu limiting task number=30, gpu limiting task number = 2*3
      (
        30,
        1,
        2,
        1.0 / 2,
        2 * 2
      ), // cpu limiting task number=30, gpu limiting task number = 2*2
      (
        30,
        1,
        2,
        1.0,
        2 * 1
      ), // cpu limiting task number=30, gpu limiting task number = 2*1
      (
        30,
        1,
        2,
        2.0,
        1
      ), // cpu limiting task number=30, gpu limiting task number = 1
      (
        70,
        2,
        7,
        0.5,
        7 * 2
      ), // cpu limiting task number=30, gpu limiting task number = 7*1/0.5=
      (
        80,
        3,
        9,
        2.0,
        9 / 2
      ) // cpu limiting task number=30, gpu limiting task number = 9/2
    ).foreach {
      case (
            executorCores,
            taskCpus: Int,
        ...
        assert(
          msg.contains(
            s"The configuration of cores (exec = ${executorCores} task = " +
  ...
  test(
    "SPARK-45527 warnOnWastedResources for TaskResourceProfile when executor number = 1"
  ) {
    val conf = new SparkConf()
    conf.set("spark.executor.cores", "10")
    conf.set("spark.task.cpus", "1")
    ...
    Seq(
      ...
      (
        ...
        0.1,
        1
      ), // cpu limiting task number = 10/7 = 1, gpu limiting task number = 1/0.1 = 10
      (
        10,
        0.1,
        1
      ) // cpu limiting task number = 10/10 = 1, gpu limiting task number = 1/0.1 = 10
    ...
    Seq(
      (
        ...
        0.111,
        9
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.111=9
      (
        1,
        0.125,
        8
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.125=8
      (
        1,
        0.142,
        7
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.142=7
      (
        1,
        0.166,
        6
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.166=6
      (
        1,
        0.2,
        5
      ...
      (
        ...
        0.25,
        4
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.25=4
      (
        1,
        0.333,
        3
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.333=3
      (
        1,
        0.5,
        2
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.166=2
      (
        1,
        0.6,
        1
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.6=1
      (
        1,
        0.7,
        1
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.7=1
      (
        1,
        0.8,
        1
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.8=1
      (
        1,
        0.9,
        1
      ), // cpu limiting task number = 10/1, gpu limiting task number = 1/0.9=1
      (
        1,
        1.0,
        1
      ) // cpu limiting task number = 10/1, gpu limiting task number = 1/1.0=1
    ).foreach { case (cores: Int, gpus: Double, expectedTaskNumber: Int) =>
      treqs = new TaskResourceRequests().cpus(cores).resource("gpu", gpus)
      rp = new FakedTaskResourceProfile(defaultDp, treqs.requests)
      msg = intercept[SparkException] {
      ...
      assert(
        msg.contains(
          "The configuration of cores (exec = 10 task = 1, runnable tasks = 10) " +
            "will result in wasted resources due to resource gpu limiting the number of runnable " +
            s"tasks per executor to: $expectedTaskNumber. Please adjust your configuration"
        )
      )
  ...
  test(
    ...
    Seq(
      (
        ...
        0.2,
        15
      ), // cpu limiting task number = 60/4=15, gpu limiting task number = 6/0.2 = 30
      (
        7,
        0.2,
        8
      ), // cpu limiting task number = 60/7 = 8, gpu limiting task number = 6/0.2 = 30
      (
        30,
        0.2,
        2
      ), // cpu limiting task number = 60/30 = 2, gpu limiting task number = 6/0.2 = 30
      (
        31,
        0.2,
        1
      ), // cpu limiting task number = 60/31 = 1, gpu limiting task number = 6/0.2 = 30
      (
        55,
        0.2,
        1
      ), // cpu limiting task number = 60/55 = 1, gpu limiting task number = 6/0.2 = 30
      (
        60,
        0.2,
        1
      ) // cpu limiting task number = 60/60 = 1, gpu limiting task number = 6/0.2 = 30
    ).foreach { case (cores: Int, gpus: Double, expectedTaskNumber: Int) =>
      treqs = new TaskResourceRequests().cpus(cores).resource("gpu", gpus)
      rp = new FakedTaskResourceProfile(defaultDp, treqs.requests)
      msg = intercept[SparkException] {
      ...
      assert(
        msg.contains(
          "The configuration of resource: gpu (exec = 6, task = 0.2/5, runnable " +
    ...
    Seq(
      (
        ...
        0.111,
        54
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.111=54
      (
        1,
        0.125,
        48
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.125=48
      (
        1,
        0.142,
        42
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.142=42
      (
        1,
        0.166,
        36
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.166=36
      (
        1,
        0.2,
        30
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.2=30
      (
        1,
        0.25,
        24
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.25=24
      (
        1,
        0.33,
        18
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.33=18
      (
        1,
        0.5,
        12
      ), // cpu limiting task number = 60/1, gpu limiting task number = 6*1/0.5=12
      (
        1,
        0.7,
        6
      ), // cpu limiting task number = 60/1 = 60, gpu limiting task number = 6*1/0.7 = 6
      (
        1,
        1.0,
        6
      ), // cpu limiting task number = 60/1 = 60, gpu limiting task number = 6/1 = 6
      (
        1,
        2.0,
        3
      ), // cpu limiting task number = 60/1 = 60, gpu limiting task number = 6/2 = 3
      (
        1,
        3.0,
        2
      ), // cpu limiting task number = 60/1 = 60, gpu limiting task number = 6/3 = 2
      (
        1,
        4.0,
        1
      ), // cpu limiting task number = 60/1 = 60, gpu limiting task number = 6/4 = 1
      (
        1,
        6.0,
        1
      ) // cpu limiting task number = 60/1 = 60, gpu limiting task number = 6/6 = 1
    ).foreach { case (cores: Int, gpus: Double, expectedTaskNumber: Int) =>
      treqs = new TaskResourceRequests().cpus(cores).resource("gpu", gpus)
      rp = new FakedTaskResourceProfile(defaultDp, treqs.requests)
      msg = intercept[SparkException] {
      ...
      assert(
        msg.contains(
          "The configuration of cores (exec = 60 task = 1, runnable tasks = 60) " +
            "will result in wasted resources due to resource gpu limiting the number of runnable " +
            s"tasks per executor to: $expectedTaskNumber. Please adjust your configuration"
        )
      )
