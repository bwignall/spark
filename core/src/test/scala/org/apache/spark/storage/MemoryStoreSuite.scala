...
case class OffHeapValue(override val estimatedSize: Long)
...
class MemoryStoreSuite
  ...
  private def assertSameContents[T](
      expected: Seq[T],
      actual: Seq[T],
      hint: String
  ): Unit = {
    assert(
      ...
      s"wrong number of values returned in $hint"
    )
  ...
  test("reserve/release unroll memory") {
    ...
    memoryStore.releaseUnrollMemoryForThisTask(
      MemoryMode.ON_HEAP
    ) // release all
    assert(memoryStore.currentUnrollMemoryForThisTask === 0)
  }
  ...
  test("safely unroll blocks") {
    ...
    def putIteratorAsValues[T](
      ...
      val res = memoryStore.putIteratorAsValues(
        blockId,
        iter,
        MemoryMode.ON_HEAP,
        classTag
    ...
    var putResult =
      putIteratorAsValues("unroll", smallList.iterator, ClassTag.Any)
    assert(putResult.isRight)
    assert(memoryStore.currentUnrollMemoryForThisTask === 0)
    assertSameContents(
      smallList,
      memoryStore.getValues("unroll").get.toSeq,
      "getValues"
    )
    ...
    assertSameContents(
      bigList,
      putResult.swap.getOrElse(fail()).toSeq,
      "putIterator"
    )
  ...
  test("safely unroll blocks through putIteratorAsBytes") {
    ...
    def putIteratorAsBytes[T](
      ...
      val res = memoryStore.putIteratorAsBytes(
        blockId,
        iter,
        classTag,
        MemoryMode.ON_HEAP
    ...
    assert(
      memoryStore.currentUnrollMemoryForThisTask === 0
    ) // discard released the unroll memory
  }
  ...
  test("PartiallySerializedBlock.finishWritingToStream") {
    ...
    val deserializedValues = serializerManager
      .dataDeserializeStream[Any](
        "b1",
        new ByteBufferInputStream(bos.toByteBuffer)
      )(ClassTag.Any)
      .toSeq
    assertSameContents(
      bigList,
  ...
  test("multiple unrolls by the same thread") {
    ...
    def putIteratorAsValues(
        blockId: BlockId,
        iter: Iterator[Any]
    ): Either[PartiallyUnrolledIterator[Any], Long] = {
  ...
  test("SPARK-22083: Release all locks in evictBlocksToFreeSpace") {
    ...
    def testFailureOnNthDrop(
        numValidBlocks: Int,
        readLockAfterDrop: Boolean
    ): Unit = {
      val tc = TaskContext.empty()
      val memManager =
        new UnifiedMemoryManager(conf, memStoreSize, memStoreSize.toInt / 2, 1)
      val blockInfoManager = new BlockInfoManager
        ...
        override private[storage] def dropFromMemory[T: ClassTag](
          ...
          if (droppedSoFar < numValidBlocks) {
            ...
            throw new RuntimeException(
              s"Mock error dropping block $droppedSoFar"
            )
          }
        }
      }
      ...
      (1 to numInitialBlocks).foreach { id =>
        ...
        val blockInfo =
          new BlockInfo(StorageLevel.MEMORY_ONLY, ct, tellMaster = false)
      ...
      val blocksStillInMemory = blockInfoManager.entries.filter {
        case (id, info) =>
