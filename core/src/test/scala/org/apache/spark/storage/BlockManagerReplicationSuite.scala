...
trait BlockManagerReplicationBehavior
  ...
  protected lazy val mapOutputTracker =
    new MapOutputTrackerMaster(conf, bcastManager, true)
  ...
  protected implicit def StringToBlockId(value: String): BlockId =
    new TestBlockId(value)
  ...
  protected def makeBlockManager(
    ...
    val memManager =
      memoryManager.getOrElse(UnifiedMemoryManager(conf, numCores = 1))
  ...
  Seq(false, true).foreach { stream =>
    test(
      s"test block replication failures when block is received " +
      ...
      doAnswer(_ => false)
        .when(mockedMemoryManager)
        .acquireStorageMemory(any(), any(), any())
      ...
      assert(
        blockLocations === Set(store1.blockManagerId, store3.blockManagerId)
      )
    }
  }
  ...
  test("block replication - addition and deletion of block managers") {
    val blockSize = 1000
    val storeSize = 10000
    val initialStores = (1 to 2).map { i =>
        ...
        .foreach { testStore =>
          ...
          if (memoryStore.contains(blockId) && !storageLevel.deserialized) {
            memoryStore.getBytes(blockId).get.chunks.foreach { byteBuffer =>
              assert(
                storageLevel.useOffHeap == byteBuffer.isDirect,
          ...
          if (storageLevel.useMemory) {
            ...
            (1 to 10).foreach { i =>
              testStore.putSingle(s"dummy-block-$i", new Array[Byte](1000), sl)
            }
            (1 to 10).foreach { i =>
...
class BlockManagerProactiveReplicationSuite
    extends BlockManagerReplicationBehavior {
  override def createConf(): SparkConf = {
    new SparkConf(false)
      .set("spark.app.id", "test")
      .set(Kryo.KRYO_SERIALIZER_BUFFER_SIZE.key, "1m")
  ...
  (2 to 5).foreach { i =>
    test(
      s"proactive block replication - $i replicas - ${i - 1} block manager deletions"
    ) {
  ...
  def testProactiveReplication(replicationFactor: Int): Unit = {
    val blockSize = 1000
    val storeSize = 10000
    val initialStores = (1 to 10).map { i =>
      makeBlockManager(storeSize, s"store$i")
    }
    ...
    initialStores
      .filter(bm => executorsToRemove.contains(bm.blockManagerId))
      .foreach { bm =>
    ...
    assert(
      newLocations.forall(bmId => !executorsToRemove.contains(bmId)),
    ...
    eventually(timeout(1.second), interval(10.milliseconds)) {
      initialStores
        .filter(bm => newLocations.contains(bm.blockManagerId))
        .foreach { bm =>
          assert(
            bm.blockInfoManager
              .getTaskLockCount(BlockInfo.NON_TASK_WRITER) === 0
          )
        }
    }
  }
...
class DummyTopologyMapper(conf: SparkConf)
    extends TopologyMapper(conf)
    with Logging {
...
class BlockManagerBasicStrategyReplicationSuite
    extends BlockManagerReplicationBehavior {
  override def createConf(): SparkConf = {
    new SparkConf(false)
      .set("spark.app.id", "test")
...
class SortOnHostNameBlockReplicationPolicy extends BlockReplicationPolicy {
