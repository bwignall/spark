...
class BlockManagerInfoSuite extends SparkFunSuite {
  ...
  private def testWithShuffleServiceOnOff(
      testName: String
  ...
  testWithShuffleServiceOnOff("RDD block with MEMORY_AND_DISK") {
    (svcEnabled, bmInfo) =>
      ...
      if (svcEnabled) {
        assert(
          getEssBlockStatus(bmInfo, rddId) ===
  ...
  testWithShuffleServiceOnOff("RDD block with DISK_ONLY") {
    (svcEnabled, bmInfo) =>
      ...
      val exclusiveCachedBlocksForOneMemoryOnly =
        if (svcEnabled) Set() else Set(rddId)
      assert(bmInfo.remainingMem === 30000)
      if (svcEnabled) {
        assert(
          getEssBlockStatus(bmInfo, rddId) === Some(
  ...
  testWithShuffleServiceOnOff("update from MEMORY_ONLY to DISK_ONLY") {
    (svcEnabled, bmInfo) =>
      ...
      assert(
        bmInfo.blocks.asScala === Map(
          rddId -> BlockStatus(StorageLevel.MEMORY_ONLY, 200, 0)
        )
      )
      assert(bmInfo.remainingMem === 29800)
      if (svcEnabled) {
      ...
      assert(
        bmInfo.blocks.asScala === Map(
          rddId -> BlockStatus(StorageLevel.DISK_ONLY, 0, 200)
        )
      )
      assert(bmInfo.remainingMem === 30000)
      if (svcEnabled) {
  ...
  testWithShuffleServiceOnOff("remove block and add another one") {
    (svcEnabled, bmInfo) =>
      ...
      if (svcEnabled) {
        assert(
          getEssBlockStatus(bmInfo, rddId1) ===
      ...
      if (svcEnabled) {
        assert(
          getEssBlockStatus(bmInfo, rddId2) ===
            Some(BlockStatus(StorageLevel.DISK_ONLY, 0, 200))
        )
      }
  }
