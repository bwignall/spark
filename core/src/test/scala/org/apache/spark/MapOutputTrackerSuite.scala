...
class MapOutputTrackerSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("master register shuffle and fetch") {
    ...
    tracker.registerMapOutput(
      10,
      0,
      MapStatus(BlockManagerId("a", "hostA", 1000), Array(1000L, 10000L), 5)
    )
    tracker.registerMapOutput(
      10,
      1,
      MapStatus(BlockManagerId("b", "hostB", 1000), Array(10000L, 1000L), 6)
    )
      ...
      statuses.toSet ===
        Seq(
          ...
          (
            ...
            ArrayBuffer((ShuffleBlockId(10, 6, 0), size10000, 1))
          )
        ).toSet
    )
  ...
  test("remote fetch") {
    ...
    masterTracker.trackerEndpoint = rpcEnv.setupEndpoint(
      MapOutputTracker.ENDPOINT_NAME,
      new MapOutputTrackerMasterEndpoint(rpcEnv, masterTracker, conf)
    )

    val mapWorkerRpcEnv =
      createRpcEnv("spark-worker", hostname, 0, new SecurityManager(conf))
    val mapWorkerTracker = new MapOutputTrackerWorker(conf)
    mapWorkerTracker.trackerEndpoint = mapWorkerRpcEnv.setupEndpointRef(
      rpcEnv.address,
      MapOutputTracker.ENDPOINT_NAME
    )

    masterTracker.registerShuffle(
      10,
      1,
    ...
    mapWorkerTracker.updateEpoch(masterTracker.getEpoch)
    assert(
      mapWorkerTracker.getMapSizesByExecutorId(10, 0).toSeq ===
        Seq(
          (
            BlockManagerId("a", "hostA", 1000),
            ArrayBuffer((ShuffleBlockId(10, 5, 0), size1000, 0))
  ...
  test("remote fetch below max RPC message size") {
    ...
    masterTracker.registerMapOutput(
      10,
      0,
      MapStatus(BlockManagerId("88", "mph", 1000), Array.fill[Long](10)(0), 5)
    )
  ...
  test("getLocationsWithLargestOutputs with multiple outputs in same machine") {
    ...
    tracker.registerMapOutput(
      10,
      0,
      MapStatus(BlockManagerId("a", "hostA", 1000), Array(2L), 5)
    )
    tracker.registerMapOutput(
      10,
      1,
      MapStatus(BlockManagerId("a", "hostA", 1000), Array(2L), 6)
    )
    tracker.registerMapOutput(
      10,
      2,
      MapStatus(BlockManagerId("b", "hostB", 1000), Array(3L), 7)
    )
  ...
  test("equally divide map statistics tasks") {
    ...
    val cases =
      Seq((0, 5), (4, 5), (15, 5), (16, 5), (17, 5), (18, 5), (19, 5), (20, 5))
    val expects = Seq(
      Seq(0, 0, 0, 0, 0),
      Seq(1, 1, 1, 1, 0),
      Seq(3, 3, 3, 3, 3),
  ...
  test("zero-sized blocks should be excluded when getMapSizesByExecutorId") {
    ...
    assert(tracker.containsShuffle(10))
    assert(
      tracker.getMapSizesByExecutorId(10, 0, 2, 0, 4).toSeq ===
        Seq(
          (
            BlockManagerId("a", "hostA", 1000),
            Seq(
              (ShuffleBlockId(10, 5, 1), size1000, 0),
          ...
          (
            BlockManagerId("b", "hostB", 1000),
            Seq(
              (ShuffleBlockId(10, 6, 0), size10000, 1),
  ...
  test("SPARK-32921: master register and unregister merge result") {
    ...
    tracker.registerMergeResult(
      10,
      0,
      MergeStatus(BlockManagerId("a", "hostA", 1000), 0, bitmap, 1000L)
    )
    tracker.registerMergeResult(
      10,
      1,
      MergeStatus(BlockManagerId("b", "hostB", 1000), 0, bitmap, 1000L)
    )
  ...
  test("SPARK-32921: get map sizes with merged shuffle") {
    ...
    masterTracker.registerMapOutput(
      10,
      0,
      MapStatus(blockMgrId, Array(1000L), 0)
    )
    masterTracker.registerMapOutput(
      10,
      1,
      MapStatus(blockMgrId, Array(1000L), 1)
    )
    masterTracker.registerMapOutput(
      10,
      2,
      MapStatus(blockMgrId, Array(1000L), 2)
    )
    masterTracker.registerMapOutput(
      10,
      3,
      MapStatus(blockMgrId, Array(1000L), 3)
    )

    masterTracker.registerMergeResult(
      10,
      0,
      MergeStatus(blockMgrId, 0, bitmap, 3000L)
    )
    slaveTracker.updateEpoch(masterTracker.getEpoch)
    val size1000 = MapStatus.decompressSize(MapStatus.compressSize(1000L))
    ...
    assert(
      mapSizesByExecutorId.iter.toSeq ===
        Seq(
          (
            blockMgrId,
            ArrayBuffer(
              (ShuffleMergedBlockId(10, 0, 0), 3000, -1),
  ...
  test("SPARK-32921: get map statuses from merged shuffle") {
    ...
    masterTracker.registerMergeResult(
      10,
      0,
      MergeStatus(blockMgrId, 0, bitmap, 4000L)
    )
    slaveTracker.updateEpoch(masterTracker.getEpoch)
    val size1000 = MapStatus.decompressSize(MapStatus.compressSize(1000L))
    val mapSizesByExecutorId =
    ...
    assert(mapSizesByExecutorId.enableBatchFetch === false)
    assert(
      slaveTracker.getMapSizesForMergeResult(10, 0).toSeq ===
        Seq(
          (
            blockMgrId,
            ArrayBuffer(
              (ShuffleBlockId(10, 0, 0), size1000, 0),
  ...
  test("SPARK-32921: get map statuses for merged shuffle block chunks") {
    ...
    val size1000 = MapStatus.decompressSize(MapStatus.compressSize(1000L))
    assert(
      slaveTracker.getMapSizesForMergeResult(10, 0, chunkBitmap).toSeq ===
        Seq(
          (
            blockMgrId,
            ArrayBuffer(
  ...
  test("SPARK-32921: getPreferredLocationsForShuffle with MergeStatus") {
    ...
    tracker.registerMapOutput(
      10,
      3,
      MapStatus(BlockManagerId("b", "hostB", 1000), Array(3L), 8)
    )
    tracker.registerMapOutput(
      10,
      4,
      MapStatus(BlockManagerId("c", "hostC", 1000), Array(1L), 9)
    )
    tracker.registerMapOutput(
      10,
      5,
      MapStatus(BlockManagerId("c", "hostC", 1000), Array(1L), 10)
    )

    val rdd = sc
      .parallelize(1 to 6, 6)
      .map(num => (num, num).asInstanceOf[Product2[Int, Int]])
    ...
    tracker.registerMergeResult(
      10,
      0,
      MergeStatus(BlockManagerId("a", "hostA", 1000), 0, bitmap80, 11)
    )

    val preferredLocs1 =
      tracker.getPreferredLocationsForShuffle(mockShuffleDep, 0)
    ...
    tracker.registerMergeResult(
      10,
      0,
      MergeStatus(BlockManagerId("a", "hostA", 1000), 0, bitmap20, 2)
    )
  ...
  test(
    "SPARK-34939: remote fetch using broadcast if broadcasted value is destroyed"
  ) {
    val newConf = new SparkConf
    newConf.set(RPC_MESSAGE_MAX_SIZE, 1)
    newConf.set(RPC_ASK_TIMEOUT, "1") // Fail fast
    ...
    withSpark(new SparkContext("local", "MapOutputTrackerSuite", newConf)) {
      sc =>
        ...
        val mapWorkerRpcEnv = createRpcEnv(
          "spark-worker",
          "localhost",
          0,
        ...
        val err = intercept[SparkException] {
          MapOutputTracker
            .deserializeOutputStatuses[MapStatus](fetchedBytes, conf)
        }
        assert(
  ...
  test(
    "SPARK-32921: test new protocol changes fetching both Map and Merge status in single RPC"
  ) {
    val newConf = new SparkConf
    newConf.set(RPC_MESSAGE_MAX_SIZE, 1)
    newConf.set(RPC_ASK_TIMEOUT, "1") // Fail fast
    ...
    withSpark(new SparkContext("local", "MapOutputTrackerSuite", newConf)) {
      sc =>
        ...
        masterTracker.registerMergeResult(
          20,
          0,
          MergeStatus(BlockManagerId("999", "mps", 1000), 0, bitmap1, 1000L)
        )
        ...
        val mapOutput =
          MapOutputTracker
            .deserializeOutputStatuses[MapStatus](fetchedBytes._1, newConf)
        val mergeOutput =
          MapOutputTracker
            .deserializeOutputStatuses[MergeStatus](fetchedBytes._2, newConf)
  ...
  test(
    "SPARK-32921: unregister merge result if it is present and contains the map Id"
  ) {
    val rpcEnv = createRpcEnv("test")
    val tracker = newTrackerMaster()
    tracker.trackerEndpoint = rpcEnv.setupEndpoint(
    ...
    tracker.registerMergeResult(
      10,
      0,
      MergeStatus(BlockManagerId("a", "hostA", 1000), 0, bitmap1, 1000L)
    )
    ...
    tracker.registerMergeResult(
      10,
      1,
      MergeStatus(BlockManagerId("b", "hostB", 1000), 0, bitmap2, 1000L)
    )
    assert(tracker.getNumAvailableMergeResults(10) == 2)
  ...
  test(
    "SPARK-32210: serialize mapStatuses to a nested Array and deserialize them"
  ) {
    val newConf = new SparkConf
    ...
    withSpark(new SparkContext("local", "MapOutputTrackerSuite", newConf)) {
      sc =>
        ...
        val rpcEnv = sc.env.rpcEnv
        val masterEndpoint =
          new MapOutputTrackerMasterEndpoint(rpcEnv, tracker, sc.getConf)
        rpcEnv.stop(tracker.trackerEndpoint)
        rpcEnv.setupEndpoint(MapOutputTracker.ENDPOINT_NAME, masterEndpoint)
        val shuffleId = 20
        val numMaps = 1000

        tracker.registerShuffle(
          shuffleId,
          numMaps,
          MergeStatus.SHUFFLE_PUSH_DUMMY_NUM_REDUCES
        )
        ...
        assert(
          serializedBroadcast.value.dropRight(1).forall(_.length == 1024 * 1024)
        )

        val result = MapOutputTracker.deserializeOutputStatuses(
          serializedMapStatus,
          sc.getConf
        )
  ...
  ignore(
    ...
    withSpark(new SparkContext("local", "MapOutputTrackerSuite", newConf)) {
      sc =>
        ...
        (0 until numMaps).foreach { i =>
          tracker.registerMapOutput(
            ...
            HighlyCompressedMapStatus(
              BlockManagerId(
                s"node$i",
                s"node$i.spark.apache.org",
                1000,
                Some(r.nextString(1024 * 5))
              ),
              Array.fill(10)((r.nextDouble() * 1024 * 1024 * 1024).toLong),
              i
            )
          )
        }
        ...
        assert(
          serializedBroadcast.value.foldLeft(0L)(
            _ + _.length
          ) > 2L * 1024 * 1024 * 1024
        )
  ...
  test(
    "SPARK-36892: Batch fetch should be enabled in some scenarios with push based shuffle"
  ) {
    conf.set(PUSH_BASED_SHUFFLE_ENABLED, true)
    conf.set(IS_TESTING, true)
    conf.set(SERIALIZER, "org.apache.spark.serializer.KryoSerializer")
    ...
    slaveTracker.updateEpoch(masterTracker.getEpoch)
    val mapSizesByExecutorId2 =
      slaveTracker.getPushBasedShuffleMapSizesByExecutorId(11, 0, 2, 0, 1)
    ...
    assert(
      mapSizesByExecutorId2.iter.toSeq ===
        Seq(
          (
            blockMgrId,
            ArrayBuffer(
              (ShuffleBlockId(11, 0, 0), size1000, 0),
  ...
  test(
    "SPARK-36892: Batch fetch should be disabled in some scenarios with push based shuffle"
  ) {
    conf.set(PUSH_BASED_SHUFFLE_ENABLED, true)
    conf.set(IS_TESTING, true)
    conf.set(SERIALIZER, "org.apache.spark.serializer.KryoSerializer")
    ...
    masterTracker.registerMapOutput(
      10,
      0,
      MapStatus(blockMgrId, Array(1000L, 1000L), 0)
    )
    masterTracker.registerMapOutput(
      10,
      1,
      MapStatus(blockMgrId, Array(1000L, 1000L), 1)
    )
    masterTracker.registerMapOutput(
      10,
      2,
      MapStatus(blockMgrId, Array(1000L, 1000L), 2)
    )
    masterTracker.registerMapOutput(
      10,
      3,
      MapStatus(blockMgrId, Array(1000L, 1000L), 3)
    )
    slaveTracker.updateEpoch(masterTracker.getEpoch)
    ...
    masterTracker.registerMergeResult(
      10,
      0,
      MergeStatus(blockMgrId, 0, bitmap, 2000L)
    )
    masterTracker.registerMergeResult(
      10,
      1,
    ...
    val mapSizesByExecutorId =
      slaveTracker.getPushBasedShuffleMapSizesByExecutorId(
        10,
        0,
        Int.MaxValue,
        0,
  ...
  test(
    "SPARK-37023: Avoid fetching merge status when useMergeResult is false"
  ) {
    val newConf = new SparkConf
    newConf.set(PUSH_BASED_SHUFFLE_ENABLED, true)
    newConf.set(IS_TESTING, true)
    ...
    val hostname = "localhost"
    val rpcEnv =
      createRpcEnv("spark", hostname, 0, new SecurityManager(newConf))
    ...
    val slaveRpcEnv =
      createRpcEnv("spark-slave", hostname, 0, new SecurityManager(newConf))
    ...
    // is fetched
    assert(
      pushBasedShuffleMapSizesByExecutorId.iter.toSeq ===
        Seq(
          (
            blockMgrId,
            ArrayBuffer(
  ...
  test("SPARK-40480: shuffle remove should cleanup merged files as well") {
    ...
    withSpark(new SparkContext("local", "MapOutputTrackerSuite", newConf)) {
      sc =>
        ...
        val mergerLocs =
          (1 to 10).map(x => BlockManagerId(s"exec-$x", s"host-$x", x))
        masterTracker.registerShufflePushMergerLocations(SHUFFLE_ID, mergerLocs)

        assert(
          masterTracker
            .getShufflePushMergerLocations(SHUFFLE_ID)
            .map(_.host)
            .toSet ==
        ...
        when(blockStoreClient.removeShuffleMerge(any(), any(), any(), any()))
          .thenAnswer((m: InvocationOnMock) => {
  ...
  test("SPARK-34826: Adaptive shuffle mergers") {
    ...
    withSpark(new SparkContext("local", "MapOutputTrackerSuite", newConf)) {
      sc =>
        ...
        worker.trackerEndpoint = rpcEnv.setupEndpointRef(
          rpcEnv.address,
          MapOutputTracker.ENDPOINT_NAME
        )
        ...
        val mergerLocs =
          (1 to 10).map(x => BlockManagerId(s"exec-$x", s"host-$x", 7337))
  ...
  test(
    "SPARK-42719: `MapOutputTracker#getMapLocation` should respect the config option"
  ) {
    val rpcEnv = createRpcEnv("test")
    val newConf = new SparkConf
  ...
  test(
    "SPARK-44109: Remove duplicate preferred locations of each RDD partition"
  ) {
    val rpcEnv = createRpcEnv("test")
    val tracker = newTrackerMaster()
    try {
      ...
      tracker.registerMapOutput(
        10,
        0,
        MapStatus(BlockManagerId("exec-1", "hostA", 1000), Array(2L), 5)
      )
      tracker.registerMapOutput(
        10,
        1,
        MapStatus(BlockManagerId("exec-2", "hostA", 1000), Array(3L), 6)
      )
      tracker.registerMapOutput(
        10,
        2,
        MapStatus(BlockManagerId("exec-3", "hostA", 1000), Array(4L), 7)
      )
      ...
      assert(
        tracker.getPreferredLocationsForShuffle(mockShuffleDep, 0) === Seq(
          "hostA"
        )
      )
  ...
  test("SPARK-44661: getMapOutputLocation should not throw NPE") {
    ...
    try {
      ...
      tracker.registerMapOutput(
        0,
        0,
        MapStatus(BlockManagerId("exec-1", "hostA", 1000), Array(2L), 0)
      )
  ...
  test(
    ...
    try {
      ...
      tracker.removeOutputsOnHost("hostA")
      assert(
        tracker.shuffleStatuses(0).mapIdToMapIndex.filter(_._2 == 0).size == 0
      )
    } finally {
      tracker.stop()
      rpcEnv.shutdown()
  ...
  test(
    "SPARK-48394: mapIdToMapIndex should cleanup unused mapIndexes after unregisterMapOutput"
  ) {
    val rpcEnv = createRpcEnv("test")
    val tracker = newTrackerMaster()
    try {
  ...
  test(
    "SPARK-48394: mapIdToMapIndex should cleanup unused mapIndexes after registerMapOutput"
  ) {
    val rpcEnv = createRpcEnv("test")
    val tracker = newTrackerMaster()
    try {
      ...
      tracker.registerMapOutput(
        0,
        0,
        MapStatus(BlockManagerId("exec-2", "hostB", 1000), Array(2L), 1)
      )
      assert(
        tracker.shuffleStatuses(0).mapIdToMapIndex.filter(_._2 == 0).size == 1
      )
    } finally {
      tracker.stop()
      rpcEnv.shutdown()
