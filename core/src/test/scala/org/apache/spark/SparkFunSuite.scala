...
// scalastyle:on
  ...
  protected final def copyAndGetResourceFile(
      fileName: String,
      suffix: String
  ): File = {
  ...
  /** Get a Path relative to the root project. It is assumed that a spark home is set.
    */
  protected final def getWorkspaceFilePath(
      first: String,
      more: String*
  ): Path = {
    if (
  ...
  /** Creates a temporary directory containing a secret file, which is then passed to `f` and
    ...
    */
  protected def withSecretFile(
      contents: String = "test-secret"
  )(f: File => Unit): Unit = {
  ...
  /** Sets all configurations specified in `pairs` in SparkEnv SparkConf, calls `f`, and then
    ...
    */
  protected def withSparkEnvConfs(
      pairs: (String, String)*
  )(f: => Unit): Unit = {
