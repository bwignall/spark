...
class ExternalSorterSuite extends SparkFunSuite with LocalSparkContext {
  ...
  testWithMultipleSer("empty partitions with spilling")(
    emptyPartitionsWithSpilling
  )
  ...
  test("spilling with hash collisions using the Int.MaxValue key") {
    ...
    def mergeValue(buffer: ArrayBuffer[Int], i: Int): ArrayBuffer[Int] =
      buffer += i
    def mergeCombiners(
        buf1: ArrayBuffer[Int],
        buf2: ArrayBuffer[Int]
    ): ArrayBuffer[Int] = {
      buf1 ++= buf2
    }
  ...
  /** Run a test multiple times, each time with a different serializer.
    */
  private def testWithMultipleSer(name: String, loadDefaults: Boolean = false)(
      body: (SparkConf => Unit)
  ): Unit = {
  ...
  private def fewElementsPerPartition(conf: SparkConf): Unit = {
    ...
    assert(
      sorter.partitionedIterator.map(p => (p._1, p._2.toSet)).toSet === expected
    )
    sorter.stop()
  ...
  private def testSpillingInLocalCluster(
      conf: SparkConf,
      numReduceTasks: Int
  ): Unit = {
    val size = 5000
  ...
  private def sortWithoutBreakingSortingContracts(conf: SparkConf): Unit = {
    ...
    def mergeValue(c: ArrayBuffer[String], i: String): ArrayBuffer[String] =
      c += i
    def mergeCombiners(
        c1: ArrayBuffer[String],
        c2: ArrayBuffer[String]
    ): ArrayBuffer[String] =
      c1 ++= c2
