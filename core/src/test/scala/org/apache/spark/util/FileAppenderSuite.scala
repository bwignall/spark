...
class FileAppenderSuite extends SparkFunSuite with BeforeAndAfter {
  ...
  test("basic file appender") {
    ...
    assert(
      Files.toString(testFile, StandardCharsets.UTF_8) === header + testString
    )
  }
  ...
  test("SPARK-35027: rolling file appender - time-based rolling close stream") {
    ...
    val appender =
      FileAppender(inputStream, testFile, sparkConf, closeStreams = true)
    assert(
      appender
        .asInstanceOf[RollingFileAppender]
        .rollingPolicy
        .isInstanceOf[TimeBasedRollingPolicy]
    )
    Thread.sleep(10)
    appender.stop()
    appender.awaitTermination()
  ...
  test("SPARK-35027: rolling file appender - size-based rolling close stream") {
    ...
    assert(
      appender
        .asInstanceOf[RollingFileAppender]
        .rollingPolicy
        .isInstanceOf[SizeBasedRollingPolicy]
    )
    Thread.sleep(10)
    appender.stop()
    appender.awaitTermination()
  ...
  test("rolling file appender - cleaning") {
    ...
    val testInputStream = new PipedInputStream(testOutputStream, 100 * 1000)
    val conf =
      new SparkConf().set(config.EXECUTOR_LOGS_ROLLING_MAX_RETAINED_FILES, 10)
    val appender = new RollingFileAppender(
      testInputStream,
      testFile,
    ...
    logInfo(
      s"All rolled over files generated:${rolledOverFiles.length}\n" +
  ...
  test("file appender selection") {
    ...
    def testAppenderSelection[
        ...
        expectedRollingPolicyParam: Long = -1
    ): Unit = {
      ...
      appender match {
        case rfa: RollingFileAppender =>
          ...
          val policyParam = rollingPolicy match {
            case timeBased: TimeBasedRollingPolicy =>
              timeBased.rolloverIntervalMillis
            case sizeBased: SizeBasedRollingPolicy =>
              sizeBased.rolloverSizeBytes
          }
  ...
  def testRolling(
    ...
    if (isCompressed) {
      assert(
        generatedFiles.exists(
          _.getName.endsWith(RollingFileAppender.GZIP_LOG_SUFFIX)
        )
      )
    }
