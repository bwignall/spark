...
class ChunkedByteBufferSuite extends SparkFunSuite with SharedSparkContext {
  ...
  /** compare two ChunkedByteBuffer:
    ...
    */
  def assertBufferEqual(
      buffer1: ChunkedByteBuffer,
      buffer2: ChunkedByteBuffer
  ): Unit = {
  ...
  test(
    ...
    try {
      ...
      val chunkedByteBuffer =
        new ChunkedByteBuffer(Array(ByteBuffer.allocate(40 * 1024 * 1024)))
  ...
  test("Externalizable: writeExternal() and readExternal()") {
    ...
    val chunkedByteBuffer =
      new ChunkedByteBuffer(byteArrays.map(ByteBuffer.wrap).toArray)
  ...
  test("toInputStream()") {
    ...
    val chunkedByteBuffer =
      new ChunkedByteBuffer(Array(empty, bytes1, empty, bytes2))
