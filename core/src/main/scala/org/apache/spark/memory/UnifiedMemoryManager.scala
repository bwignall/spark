...
private[spark] class UnifiedMemoryManager(
  ...
  private def assertInvariants(): Unit = {
    assert(
      onHeapExecutionMemoryPool.poolSize + onHeapStorageMemoryPool.poolSize == maxHeapMemory
    )
    assert(
  ...
  override def acquireStorageMemory(
    ...
    if (numBytes > maxMemory) {
      ...
      logInfo(
        log"Will not store ${MDC(BLOCK_ID, blockId)} as the required space" +
