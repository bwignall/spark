...
abstract class RDD[T: ClassTag](
  ...
  @volatile @transient private var legacyDependencies
      : WeakReference[Seq[Dependency[_]]] = _
  ...
  /** Gets or computes an RDD partition. Used by RDD.iterator() when an RDD is cached.
    */
  private[spark] def getOrCompute(
      partition: Partition,
      context: TaskContext
  ): Iterator[T] = {
  ...
  def coalesce(
    ...
    require(
      numPartitions > 0,
      s"Number of partitions ($numPartitions) must be positive."
    )
    if (shuffle) {
      ...
      if (initialCount == 0) {
        ...
        if (!withReplacement && num >= initialCount) {
          ...
          while (samples.length < num) {
            logWarning(
              log"Needed to re-sample due to insufficient sample size. " +
  ...
  def glom(): RDD[Array[T]] = withScope {
    new MapPartitionsRDD[Array[T], T](
      this,
      (_, _, iter) => Iterator(iter.toArray)
    )
  }
      ...
      (thisIter, otherIter) =>
        new Iterator[(T, U)] {
          def hasNext: Boolean = (thisIter.hasNext, otherIter.hasNext) match {
            ...
            case _ =>
              throw SparkCoreErrors.canOnlyZipRDDsWithSamePartitionSizeError()
          }
  ...
  def subtract(other: RDD[T]): RDD[T] = withScope {
    subtract(
      other,
      partitioner.getOrElse(new HashPartitioner(partitions.length))
    )
  }
  ...
  def fold(zeroValue: T)(op: (T, T) => T): T = withScope {
    ...
    val mergeResult =
      (_: Int, taskResult: T) => jobResult = op(jobResult, taskResult)
  ...
  def aggregate[U: ClassTag](
    ...
    val mergeResult =
      (_: Int, taskResult: U) => jobResult = combOp(jobResult, taskResult)
  ...
  def treeAggregate[U: ClassTag](
      zeroValue: U
  )(seqOp: (U, T) => U, combOp: (U, U) => U, depth: Int = 2): U = withScope {
    treeAggregate(
      zeroValue,
      seqOp,
      combOp,
      depth,
      finalAggregateOnExecutor = false
    )
  }
  ...
  def treeAggregate[U: ClassTag](
    ...
    if (partitions.length == 0) {
      ...
      while (
        numPartitions > scale + math.ceil(numPartitions.toDouble / scale)
      ) {
        numPartitions /= scale
        ...
        partiallyAggregated = partiallyAggregated
          ...
          .foldByKey(zeroValue, new HashPartitioner(curNumPartitions))(
            cleanCombOp
          )
          .values
      }
      if (
        finalAggregateOnExecutor && partiallyAggregated.partitions.length > 1
      ) {
  ...
  ): PartialResult[BoundedDouble] = withScope {
    require(
      ...
      while (buf.size < num && partsScanned < totalParts) {
        ...
        if (partsScanned > 0) {
          ...
          if (buf.isEmpty) {
            ...
            numPartsToTry =
              Math.ceil(1.5 * left * partsScanned / buf.size).toInt
            numPartsToTry =
    ...
    if (num == 0 || this.getNumPartitions == 0) {
      ...
      this
        ...
        .reduce { (array1, array2) =>
          ...
          collectionUtils
            .mergeOrdered[T](Seq(array1, array2))(ord)
  ...
  def localCheckpoint(): this.type = RDDCheckpointData.synchronized {
    if (
      Utils.isDynamicAllocationEnabled(conf) &&
  ...
  private[spark] final def outputDeterministicLevel
  ...
  protected def getOutputDeterministicLevel: DeterministicLevel.Value = {
    val deterministicLevelCandidates = dependencies.map {
      ...
      case dep: ShuffleDependency[_, _, _]
          if dep.rdd.partitioner.exists(_ == dep.partitioner) =>
