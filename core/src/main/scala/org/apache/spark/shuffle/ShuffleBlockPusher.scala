...
private[spark] class ShuffleBlockPusher(conf: SparkConf) extends Logging {
  private[this] val maxBlockSizeToPush =
    conf.get(SHUFFLE_MAX_BLOCK_SIZE_TO_PUSH)
  private[this] val maxBlockBatchSize =
    conf.get(SHUFFLE_MAX_BLOCK_BATCH_SIZE_FOR_PUSH)
  ...
  private[this] val numBlocksInFlightPerAddress =
    new HashMap[BlockManagerId, Int]()
  private[this] val deferredPushRequests =
    new HashMap[BlockManagerId, Queue[PushRequest]]()
  ...
  private[shuffle] def createErrorHandler(): BlockPushErrorHandler = {
    new BlockPushErrorHandler() {
      ...
      override def shouldRetryError(t: Throwable): Boolean = {
        ...
        if (
          t.getCause != null && t.getCause.isInstanceOf[FileNotFoundException]
        ) {
          return false
        }
        ...
        !(t.isInstanceOf[BlockPushNonFatalFailure] &&
          BlockPushNonFatalFailure.shouldNotRetryErrorCode(
  ...
  private def pushUpToMax(): Unit = synchronized {
    ...
    if (deferredPushRequests.nonEmpty) {
      for ((remoteAddress, defReqQueue) <- deferredPushRequests) {
        while (
          isRemoteBlockPushable(defReqQueue) &&
          !isRemoteAddressMaxedOut(remoteAddress, defReqQueue.front)
        ) {
          val request = defReqQueue.dequeue()
          logDebug(
            s"Processing deferred push request for $remoteAddress with "
    ...
    while (isRemoteBlockPushable(pushRequests)) {
      ...
      if (isRemoteAddressMaxedOut(remoteAddress, request)) {
        logDebug(
          s"Deferring push request for $remoteAddress with ${request.blocks.size} blocks"
        )
        deferredPushRequests
          .getOrElseUpdate(remoteAddress, new Queue[PushRequest]())
          .enqueue(request)
      } else {
        sendRequest(request)
      }
  ...
  private def sendRequest(request: PushRequest): Unit = {
    ...
    reqsInFlight += 1
    numBlocksInFlightPerAddress(request.address) = numBlocksInFlightPerAddress
      .getOrElseUpdate(request.address, 0) + request.blocks.length
    ...
    val blockPushListener = new BlockPushingListener {
      ...
      override def onBlockPushSuccess(
          blockId: String,
          data: ManagedBuffer
      ): Unit = {
      ...
      override def onBlockPushFailure(
          blockId: String,
          exception: Throwable
      ): Unit = {
        ...
        if (!errorHandler.shouldLogError(exception)) {
          ...
          logWarning(
            log"Pushing block ${MDC(BLOCK_ID, blockId)} " +
              log"to ${MDC(HOST_PORT, address)} failed.",
            exception
          )
        }
    ...
    val (blockPushIds, blockPushBuffers) = Utils
      .randomize(
        blockIds.zip(
          sliceReqBufferIntoBlockBuffers(
            request.reqBuffer,
    ...
    SparkEnv.get.blockManager.blockStoreClient.pushBlocks(
      address.host,
      address.port,
      blockPushIds.toArray,
      blockPushBuffers.toArray,
      blockPushListener
    )
  }
  ...
  private def updateStateAndCheckIfPushMore(
    ...
    if (
      pushResult.failure != null && pushResult.failure.getCause
      ...
      if (!unreachableBlockMgrs.contains(address)) {
        ...
        removed += deferredPushRequests
          .remove(address)
          .map(_.length)
          .getOrElse(0)
        logWarning(
          log"Received a ConnectException from ${MDC(HOST_PORT, address)}. " +
    ...
    if (
      pushResult.failure != null && !errorHandler.shouldRetryError(
        pushResult.failure
      )
    ) {
      logDebug(
        s"Encountered an exception from $address which indicates that push needs to " +
          s"stop."
      )
      return false
    } else {
      if (
        reqsInFlight <= 0 && pushRequests.isEmpty && deferredPushRequests.isEmpty
      ) {
        notifyDriverAboutPushCompletion()
      }
    ...
    if (!pushCompletionNotified) {
      SparkEnv.get.executorBackend match {
        case Some(cb: CoarseGrainedExecutorBackend) =>
          cb.notifyDriverAboutPushCompletion(
            shuffleId,
            shuffleMergeId,
            mapIndex
          )
        case Some(eb: ExecutorBackend) =>
          logWarning(
            log"Currently ${MDC(EXECUTOR_BACKEND, eb)} " +
  ...
  private[shuffle] def prepareBlockPushRequests(
    ...
    for (reduceId <- 0 until numPartitions) {
      ...
      logDebug(
        s"Block ${ShufflePushBlockId(shuffleId, shuffleMergeId, partitionId, reduceId)} is of size $blockSize"
      )
      // Skip 0-length blocks and blocks that are large enough
      if (blockSize > 0) {
        val mergerId = math
          .min(
            math.floor(reduceId * 1.0 / numPartitions * numMergers),
            numMergers - 1
          )
          .asInstanceOf[Int]
        ...
        if (
          currentReqSize + blockSize <= maxBlockBatchSize
