...
private[spark] class ExecutorMetricsPoller(
  ...
  def onTaskStart(taskId: Long, stageId: Int, stageAttemptId: Int): Unit = {
    ...
    val countAndPeaks = stageTCMP.compute(
    ...
    logDebug(
      s"stageTCMP: ($stageId, $stageAttemptId) -> ${countAndPeaks.count}"
    )
  }
  ...
  def getTaskMetricPeaks(taskId: Long): Array[Long] = {
    ...
    val metricPeaks =
      new Array[Long](ExecutorMetricType.numMetrics) // initialized to zeros
