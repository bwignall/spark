...
class SparkContext(config: SparkConf) extends Logging {
  ...
  logInfo(
    log"Running Spark version ${MDC(LogKeys.SPARK_VERSION, SPARK_VERSION)}"
  )
  logInfo(
    log"OS info ${MDC(LogKeys.OS_NAME, System.getProperty("os.name"))}," +
      log" ${MDC(LogKeys.OS_VERSION, System.getProperty("os.version"))}, " +
      log"${MDC(LogKeys.OS_ARCH, System.getProperty("os.arch"))}"
  )
  logInfo(
    log"Java version ${MDC(LogKeys.JAVA_VERSION, System.getProperty("java.version"))}"
  )
  ...
  private[spark] val addedFiles =
    new ConcurrentHashMap[String, ScalaConcurrentMap[String, Long]]().asScala
  private[spark] val addedArchives =
    new ConcurrentHashMap[String, ScalaConcurrentMap[String, Long]]().asScala
  private[spark] val addedJars =
    new ConcurrentHashMap[String, ScalaConcurrentMap[String, Long]]().asScala
  ...
  private[spark] def resourceProfileManager: ResourceProfileManager =
    _resourceProfileManager
  ...
  try {
    ...
    _resources =
      getOrDiscoverAllResources(_conf, SPARK_DRIVER_PREFIX, resourcesFileOpt)
    ...
    if (
      master == "yarn" && deployMode == "cluster" && !_conf.contains(
        "spark.yarn.app.id"
      )
    ) {
      throw new SparkException(
    ...
    _eventLogCodec = {
      ...
      if (compress && isEventLogEnabled) {
        Some(_conf.get(EVENT_LOG_COMPRESSION_CODEC))
    ...
    if (jars != null) {
      ...
      if (allAddedJars.nonEmpty) {
        _conf.set(
          ...
          allAddedJars.keys.toSeq.mkString(",")
        )
      }
    }
    ...
    if (files != null) {
      ...
      if (allAddedFiles.nonEmpty) {
        _conf.set(
          ...
          allAddedFiles.keys.toSeq.mkString(",")
        )
      }
    }
    ...
    if (archives != null) {
      ...
      if (allAddedArchives.nonEmpty) {
        _conf.set(
          ...
          allAddedArchives.keys.toSeq.mkString(",")
        )
      }
    }
    ...
    for {
      (envKey, propKey) <- Seq(("SPARK_TESTING", IS_TESTING.key))
    ...
    _shutdownHookRef = ShutdownHookManager.addShutdownHook(
      ...
      try {
        ...
        case e: Throwable =>
          logWarning(
            "Ignoring Exception while stopping SparkContext from shutdown hook",
            e
          )
      }
    }
      ...
      if (executorId == SparkContext.DRIVER_IDENTIFIER) {
        ...
        env.blockManager.master.getExecutorEndpointRef(executorId) match {
          case Some(endpointRef) =>
            Some(
              endpointRef.askSync[Array[ThreadStackTrace]](TriggerThreadDump)
            )
          case None =>
            logWarning(
              log"Executor ${MDC(LogKeys.EXECUTOR_ID, executorId)} " +
  ...
  /** Called by the web UI to obtain executor heap histogram.
    */
  private[spark] def getExecutorHeapHistogram(
      executorId: String
  ): Option[Array[String]] = {
    try {
      if (executorId == SparkContext.DRIVER_IDENTIFIER) {
  ...
  def getJobTags(): Set[String] = {
    Option(getLocalProperty(SparkContext.SPARK_JOB_TAGS))
      ...
      .filter(
        !_.isEmpty
      ) // empty string tag should not happen, but be defensive
  }
  ...
  def range(
    ...
    val numElements: BigInt = {
      ...
      if (
        (safeEnd - safeStart) % step == 0 || (safeEnd > safeStart) != (step > 0)
      ) {
  ...
  def wholeTextFiles(
    ...
    new WholeTextFileRDD(
      ...
      updateConf,
      minPartitions
    ).map(record => (record._1.toString, record._2.toString)).setName(path)
  }
  ...
  def union[T: ClassTag](rdds: Seq[RDD[T]]): RDD[T] = withScope {
    ...
    if (
      nonEmptyRdds.forall(_.partitioner.isDefined) && partitioners.size == 1
    ) {
  ...
  private[spark] def broadcastInternal[T: ClassTag](
    ...
    logInfo(
      log"Created broadcast ${MDC(LogKeys.BROADCAST_ID, bc.id)}" +
  ...
  private def addFile(
    ...
    val schemeCorrectedURI = uri.getScheme match {
      ...
      case "local" =>
        logWarning(
          log"File with 'local' scheme ${MDC(LogKeys.PATH, path)} " +
    ...
    if (
      !Array("http", "https", "ftp", "spark").contains(scheme) && !isArchive
    ) {
    ...
    if (
      ...
      addedFiles
        .getOrElseUpdate(
          jobArtifactUUID,
          new ConcurrentHashMap[String, Long]().asScala
        )
        .putIfAbsent(key, timestamp)
        .isEmpty
    ) {
      logInfo(
        log"Added file ${MDC(LogKeys.PATH, path)} at ${MDC(LogKeys.KEY, key)} with" +
      ...
      logInfo(
        log"Added archive ${MDC(LogKeys.PATH, path)} at ${MDC(LogKeys.KEY, key)}" +
      ...
      logWarning(
        log"The path ${MDC(LogKeys.PATH, path)} " +
  ...
  private[spark] def getExecutorIds(): Seq[String] = {
    schedulerBackend match {
      ...
      case _ =>
        logWarning(
          "Requesting executors is not supported by current scheduler."
        )
        Nil
    }
  }
  ...
  def killExecutors(executorIds: Seq[String]): Boolean = {
    schedulerBackend match {
      case b: ExecutorAllocationClient =>
        require(
          executorAllocationManager.isEmpty,
  ...
  private[spark] def getRDDStorageInfo(
    ...
    rddInfos.foreach { rddInfo =>
      ...
      rddInfo.numCachedPartitions =
        rddStorageInfo.map(_.numCachedPartitions).getOrElse(0)
  ...
  private def addJar(path: String, addedOnSubmit: Boolean): Unit = {
      ...
      try {
        if (!file.exists()) {
          throw new FileNotFoundException(
            s"Jar ${file.getAbsolutePath} not found"
          )
        }
        if (file.isDirectory) {
        ...
        case NonFatal(e) =>
          logError(
            log"Failed to add ${MDC(LogKeys.PATH, path)} to Spark environment",
            e
          )
          Nil
      }
    ...
    if (path == null || path.isEmpty) {
      ...
      val (keys, scheme) = if (path.contains("\\") && Utils.isWindows) {
        ...
        val jarPaths = uriScheme match {
          ...
          case "ivy"   =>
            ...
            DependencyUtils
              .resolveMavenDependencies(URI.create(path))
      ...
      if (keys.nonEmpty) {
        val timestamp =
          if (addedOnSubmit) startTime else System.currentTimeMillis
        val (added, existed) = keys.partition(
          addedJars
            .getOrElseUpdate(
              jobArtifactUUID,
              new ConcurrentHashMap[String, Long]().asScala
            )
            .putIfAbsent(_, timestamp)
            .isEmpty
        )
        if (added.nonEmpty) {
          ...
          logInfo(
            jarMessage + log" ${MDC(LogKeys.PATH, path)}" +
        ...
        if (existed.nonEmpty) {
          val jarMessage =
            if (scheme != "ivy") "JAR" else "dependency jars of Ivy URI"
          logWarning(
            log"The ${MDC(LogKeys.JAR_MESSAGE, jarMessage)} ${MDC(LogKeys.PATH, path)} " +
  ...
  def stop(exitCode: Int): Unit = {
    ...
    logInfo(
      log"SparkContext is stopping with exitCode ${MDC(LogKeys.EXIT_CODE, exitCode)}" +
        log" from ${MDC(LogKeys.STOP_SITE_SHORT_FORM, stopSite.get.shortForm)}."
    )
    if (LiveListenerBus.withinListenerThread.value) {
      throw new SparkException(
        s"Cannot stop SparkContext within listener bus thread."
      )
    }
  ...
  def runJob[T, U: ClassTag](
    ...
    logInfo(
      log"Starting job: ${MDC(LogKeys.CALL_SITE_SHORT_FORM, callSite.shortForm)}"
    )
    if (conf.getBoolean("spark.logLineage", false)) {
      logInfo(
        log"RDD's recursive dependencies:\n" +
  ...
  def runApproximateJob[T, U, R](
    ...
    val cleanedFunc = clean(func)
    val result = dagScheduler.runApproximateJob(
      rdd,
      cleanedFunc,
      evaluator,
      callSite,
  ...
  def setCheckpointDir(directory: String): Unit = {
    ...
    if (!isLocal && Utils.nonLocalPaths(directory).isEmpty) {
      logWarning(
        log"Spark is not running in local mode, therefore the checkpoint directory " +
  ...
  private def setupAndStartListenerBus(): Unit = {
    try {
      conf.get(EXTRA_LISTENERS).foreach { classNames =>
        val listeners = Utils
          .loadExtensions(classOf[SparkListenerInterface], classNames, conf)
  ...
  private def postApplicationEnd(exitCode: Int): Unit = {
    listenerBus.post(
      SparkListenerApplicationEnd(System.currentTimeMillis, Some(exitCode))
    )
  }
  ...
  private def reportHeartBeat(
    ...
    driverUpdates.put(
      EventLoggingListener.DRIVER_STAGE_KEY,
      new ExecutorMetrics(currentMetrics)
    )
    val accumUpdates =
      new Array[(Long, Int, Int, Seq[AccumulableInfo])](0).toImmutableArraySeq
    listenerBus.post(
      SparkListenerExecutorMetricsUpdate("driver", accumUpdates, driverUpdates)
    )
  }
...
object SparkContext extends Logging {
  ...
  private def assertNoOtherContextIsRunning(sc: SparkContext): Unit = {
    SPARK_CONTEXT_CONSTRUCTOR_LOCK.synchronized {
      ...
      contextBeingConstructed.filter(_ ne sc).foreach { otherContext =>
        ...
        val warnMsg =
          log"Another SparkContext is being constructed (or threw an exception in its" +
    ...
    SPARK_CONTEXT_CONSTRUCTOR_LOCK.synchronized {
      if (activeContext.get() == null) {
        ...
        if (config.getAll.nonEmpty) {
          logWarning(
            "Using an existing SparkContext; some configuration may not take effect."
          )
        }
      }
      activeContext.get()
  ...
  private[spark] def numDriverCores(master: String, conf: SparkConf): Int = {
    ...
    master match {
      ...
      case _ =>
        0 // Either driver is not being used, or its core count will be interpolated later
    }
  }

  private[spark] def executorMemoryInMb(conf: SparkConf): Int = {
    conf
      .getOption(EXECUTOR_MEMORY.key)
  ...
  private def createTaskScheduler(
    ...
    def checkResourcesPerTask(executorCores: Int): Unit = {
      ...
      ResourceUtils.warnOnWastedResources(
        defaultProf,
        sc.conf,
        Some(executorCores)
      )
    ...
    master match {
      case "local" =>
        ...
        val scheduler =
          new TaskSchedulerImpl(sc, MAX_LOCAL_TASK_FAILURES, isLocal = true)
      ...
      case LOCAL_CLUSTER_REGEX(numWorkers, coresPerWorker, memoryPerWorker) =>
        ...
        if (sc.executorMemory > memoryPerWorkerInt) {
          throw new SparkException(
            "Asked to launch cluster with %d MiB/worker but requested %d MiB/executor"
              .format(memoryPerWorkerInt, sc.executorMemory)
          )
        }
      ...
      case masterUrl =>
        val cm = getClusterManager(masterUrl) match {
          ...
          case None =>
            throw new SparkException(
              "Could not parse Master URL: '" + master + "'"
            )
        }
        try {
  ...
  private def getClusterManager(url: String): Option[ExternalClusterManager] = {
    ...
    val serviceLoaders =
      ServiceLoader
        .load(classOf[ExternalClusterManager], loader)
        .asScala
  ...
  private def supplementJavaIPv6Options(conf: SparkConf): Unit = {
    def supplement(key: OptionalConfigEntry[String]): Unit = {
      val v = conf.get(key) match {
        case Some(opts) =>
          s"-Djava.net.preferIPv6Addresses=${Utils.preferIPv6} $opts"
...
object WritableConverter {
  ...
  implicit val bytesWritableConverterFn
  ...
  implicit def writableWritableConverterFn[T <: Writable: ClassTag]
      : () => WritableConverter[T] =
    () =>
      new WritableConverter[T](
        _.runtimeClass.asInstanceOf[Class[T]],
        _.asInstanceOf[T]
      )
