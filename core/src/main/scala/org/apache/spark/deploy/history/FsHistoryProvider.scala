...
private[history] class FsHistoryProvider(conf: SparkConf, clock: Clock)
  ...
  private val SAFEMODE_CHECK_INTERVAL_S =
    conf.get(History.SAFEMODE_CHECK_INTERVAL_S)
  ...
  private[history] val fs: FileSystem =
    new Path(logDir).getFileSystem(hadoopConf)
  ...
  private val pendingReplayTasksCount =
  ...
  private val fileCompactor = new EventLogFileCompactor(
    conf,
    hadoopConf,
    fs,
  ...
  private def clearInaccessibleList(expireTimeInSeconds: Long): Unit = {
    ...
    inaccessibleList.asScala.filterInPlace((_, creationTime) =>
      creationTime >= expiredThreshold
    )
  }

  private val activeUIs =
    new mutable.HashMap[(String, Option[String]), LoadedAppUI]()
  ...
  private[history] def startSafeModeCheckThread(
      errorHandler: Option[Thread.UncaughtExceptionHandler]
  ): Thread = {
    ...
    initThread.setUncaughtExceptionHandler(
      errorHandler.getOrElse((_: Thread, e: Throwable) => {
  ...
  private def startPolling(): Unit = {
    ...
    if (!conf.contains(IS_TESTING)) {
      ...
      if (
        conf
          .contains(DRIVER_LOG_DFS_DIR) && conf.get(DRIVER_LOG_CLEANER_ENABLED)
      ) {
        pool.scheduleWithFixedDelay(
          getRunner(() => cleanDriverLogs()),
          0,
  ...
  private[history] def checkForLogs(): Unit = {
    ...
    try {
        ...
        .filter { reader =>
          try {
            ...
            if (shouldReloadLog(info, reader)) {
              ...
              if (
                info.appId.isDefined && reader.lastIndex.isEmpty && fastInProgressParsing
              ) {
                ...
                val updatedAppInfo =
                  new ApplicationInfoWrapper(appInfo.info, attemptList)
            ...
            case _: NoSuchElementException =>
              ...
              try {
                if (
                  conf.get(CLEANER_ENABLED) && reader.modificationTime <
                    clock.getTimeMillis() - conf.get(MAX_LOG_AGE_S) * 1000
                ) {
                  logInfo(
                    log"Deleting expired event log ${MDC(PATH, reader.rootPath.toString)}"
                  )
            ...
            case NonFatal(e) =>
              logWarning(
                log"Error while filtering log ${MDC(PATH, reader.rootPath)}",
                e
              )
              false
          }
      ...
      if (updated.nonEmpty) {
        logDebug(
          s"New/updated attempts found: ${updated.size} ${updated.map(_.rootPath)}"
        )
      }
  ...
  private[history] def shouldReloadLog(
      info: LogInfo,
    ...
    if (info.isComplete != reader.completed) {
      ...
      if (!result && !reader.completed) {
        try {
          ...
          case e: Exception =>
            logDebug(
              s"Failed to check the length for the file : ${info.logPath}",
              e
            )
        }
      }
  ...
  private def cleanAppData(
    ...
    try {
      ...
      listing.synchronized {
        val app = load(appId)
        val (attempt, others) =
          app.attempts.partition(_.info.attemptId == attemptId)
  ...
  private def mergeApplicationListing(
    ...
    var succeeded = false
    try {
      val lastEvaluatedForCompaction: Option[Long] =
        try {
          listing
            .read(classOf[LogInfo], rootPath.toString)
  ...
  ): Unit = doMergeApplicationListingInternal(
    reader,
    scanTime,
    enableOptimizations,
    lastEvaluatedForCompaction
  ...
  private def doMergeApplicationListingInternal(
    ...
    if (lookForEndEvent && listener.applicationInfo.isDefined) {
      ...
      Utils.tryWithResource(
        EventLogFileReader.openEventLog(lastFile.getPath, fs)
      ) { in =>
    ...
    listener.applicationInfo match {
      case Some(app) if !lookForEndEvent || app.attempts.head.info.completed =>
        ...
        if (appCompleted && reader.lastIndex.isEmpty) {
          val inProgressLog =
            logPath.toString() + EventLogFileWriter.IN_PROGRESS
          try {
      ...
      case Some(_) =>
        ...
        logInfo(
          log"Reparsing ${MDC(PATH, logPath)} since end event was not found."
        )
  ...
  private def compact(reader: EventLogFileReader): Unit = {
    ...
    try {
      reader.lastIndex match {
        case Some(lastIndex) =>
          try {
            ...
            if (
              info.lastEvaluatedForCompaction.isEmpty ||
              ...
              listing.write(
                info.copy(lastEvaluatedForCompaction = Some(lastIndex))
              )
            }
          } catch {
            case _: NoSuchElementException =>
      ...
      case e: Exception =>
        logError(
          log"Exception while compacting log for ${MDC(PATH, rootPath)}",
          e
        )
    } finally {
      endProcessing(rootPath)
  ...
  private[history] def checkAndCleanLog(logPath: String): Unit = Utils.tryLog {
    ...
    if (log.lastProcessed <= maxTime && log.appId.isEmpty) {
      logInfo(
        log"Deleting invalid / corrupt event log ${MDC(PATH, log.logPath)}"
      )
  ...
  private[history] def cleanLogs(): Unit = Utils.tryLog {
    ...
    val stale = KVUtils.viewToSeq(
      listing
        ...
        .first(maxTime),
      Int.MaxValue
    ) { l => l.logType == null || l.logType == LogType.EventLogs }
    ...
    if (count > 0) {
      logInfo(
        log"Try to delete ${MDC(NUM_FILES, count)} old event logs" +
          log" to keep ${MDC(MAX_NUM_FILES, maxNum)} logs in total."
      )
      KVUtils.foreach(
        listing.view(classOf[ApplicationInfoWrapper]).index("oldestAttempt")
      ) { app =>
        if (count > 0) {
      ...
      if (count > 0) {
        logWarning(
          log"Fail to clean up according to MAX_LOG_NUM policy " +
  ...
  private def deleteAttemptLogs(
      ...
      toDelete: List[AttemptInfoWrapper]
  ): Int = {
  ...
  private[history] def cleanDriverLogs(): Unit = Utils.tryLog {
    ...
    logFiles.foreach { f =>
      ...
      val deleteFile =
        try {
          ...
          if (
            info.fileSize < f.getLen() || info.lastProcessed < f
              .getModificationTime()
          ) {
            listing.write(
    ...
    val stale = KVUtils.viewToSeq(
      listing
        ...
        .first(maxTime),
      Int.MaxValue
    ) { l => l.logType != null && l.logType == LogType.DriverLogs }
    stale.filterNot(isProcessing).foreach { log =>
  ...
  private def addListing(app: ApplicationInfoWrapper): Unit =
    listing.synchronized {
      ...
      def compareAttemptInfo(
          a1: AttemptInfoWrapper,
          a2: AttemptInfoWrapper
      ): Boolean = {
        a1.info.startTime.getTime() > a2.info.startTime.getTime()
      }

      val attempts =
        oldApp.attempts.filter(_.info.attemptId != attempt.info.attemptId) ++
          List(attempt)
  ...
  private def loadDiskStore(
    ...
    dm.openStore(appId, attempt.info.attemptId).foreach { path =>
      try {
        ...
        case e: Exception =>
          logInfo(
            log"Failed to open existing store for" +
      ...
      hybridStore.switchToDiskStore(
        new HybridStore.SwitchToDiskStoreListener {
          override def onSwitchToDiskStoreSuccess(): Unit = {
            logInfo(
              log"Completely switched to diskStore for app" +
                log" ${MDC(APP_ID, appId)} / ${MDC(LogKeys.APP_ATTEMPT_ID, attempt.info.attemptId)}."
            )
            diskStore.close()
            val newStorePath = lease.commit(appId, attempt.info.attemptId)
            hybridStore.setDiskStore(
          ...
          override def onSwitchToDiskStoreFail(e: Exception): Unit = {
            logWarning(
              log"Failed to switch to diskStore for app ${MDC(APP_ID, appId)} / " +
                log"${MDC(LogKeys.APP_ATTEMPT_ID, attempt.info.attemptId)}",
              e
            )
            diskStore.close()
            lease.rollback()
  ...
  private def createDiskStore(
    ...
    while (newStorePath == null) {
      ...
      try {
        Utils.tryWithResource(
          KVUtils.open(lease.tmpPath, metadata, conf, live = false)
        ) { store =>
          rebuildAppStore(store, reader, attempt.info.lastUpdated.getTime())
        }
    ...
    while (store == null) {
      try {
        ...
        case _: IOException if !retried =>
          ...
          // compaction wouldn't run in short interval, so try again...
          logWarning(
            s"Exception occurred while rebuilding log path ${attempt.logPath} - " +
              "trying again..."
          )
          retried = true
  ...
  private[history] def getAttempt(
    ...
    load(appId).attempts
      ...
      .getOrElse(
        throw new NoSuchElementException(
          s"Cannot find attempt $attemptId of $appId."
        )
      )
  }
  ...
  private def createSecurityManager(
    ...
    secManager.setViewAcls(
      ...
      stringToSeq(attempt.viewAcls.getOrElse(""))
    )
    secManager.setAdminAclsGroups(
      historyUiAdminAclsGroups ++
        stringToSeq(attempt.adminAclsGroups.getOrElse(""))
    )
    secManager.setViewAclsGroups(
      stringToSeq(attempt.viewAclsGroups.getOrElse(""))
    )
    secManager
  }
}
...
private[spark] object FsHistoryProvider {
  ...
  private val ENV_UPDATE_EVENT_PREFIX =
...
private[history] class ApplicationInfoWrapper(
  ...
  def toApplicationInfo(): ApplicationInfo =
    info.copy(attempts = attempts.map(_.info))
...
private[history] class AppListingListener(
  ...
  override def onEnvironmentUpdate(
    ...
    if (!gotEnvUpdate) {
      def emptyStringToNone(strOption: Option[String]): Option[String] =
        strOption match {
          case Some("") => None
          case _        => strOption
        }
      ...
      attempt.adminAcls = emptyStringToNone(
        allProperties.get(UI.ADMIN_ACLS.key)
      )
