...
private[spark] class SparkSubmit extends Logging {
    ...
    if (isShell(appArgs.primaryResource) || isSqlShell(appArgs.mainClass)) {
      ...
      if (
        sparkConf.getBoolean(
          STRUCTURED_LOGGING_ENABLED.key,
          defaultValue = true
        )
      ) {
  ...
  private[deploy] def prepareSubmitEnvironment(
    ...
    val clusterManager: Int = args.maybeMaster match {
      case Some(v) =>
        assert(
          args.maybeRemote.isEmpty || sparkConf.contains("spark.local.connect")
        )
        v match {
          case "yarn"                     => YARN
    ...
    if (clusterManager == YARN) {
      ...
      if (
        !Utils.classIsLoadable(YARN_CLUSTER_SUBMIT_CLASS) && !Utils.isTesting
      ) {
        error(
    ...
    val isStandAloneCluster =
      clusterManager == STANDALONE && deployMode == CLUSTER
    val isKubernetesCluster =
      clusterManager == KUBERNETES && deployMode == CLUSTER
    val isKubernetesClient =
      clusterManager == KUBERNETES && deployMode == CLIENT
    ...
    args.pyFiles =
      Option(args.pyFiles).map(resolveGlobPaths(_, hadoopConf)).orNull
    args.archives =
      Option(args.archives).map(resolveGlobPaths(_, hadoopConf)).orNull
      ...
      if (isKubernetesClusterModeDriver) {
        ...
        def downloadResourcesToCurrentDirectory(
          ...
          val localResources = downloadFileList(
            toDownloads
              .map(Utils.getUriBuilder(_).fragment(null).build().toString)
              .mkString(","),
            targetDir,
            sparkConf,
            hadoopConf
          ...
          (Utils
            ...
            .map { case (localResources, resolvedUri) =>
              ...
              Utils
                .getUriBuilder(localResources)
                .fragment(resolvedUri.getFragment)
                .build()
                .toString
        ...
        def avoidJarDownload(scheme: String): Boolean =
          avoidJarDownloadSchemes.contains("*") || avoidJarDownloadSchemes
            .contains(scheme)
        ...
        val updatedJars = Option(args.jars).map {
          downloadResourcesToCurrentDirectory(
            _,
            avoidDownload = avoidJarDownload
          )
        }.orNull
    ...
    if (args.mainClass == null && !args.isPython && !args.isR) {
      try {
        ...
        Utils.tryWithResource(new JarInputStream(fs.open(new Path(uri)))) {
          jar =>
            args.mainClass =
    ...
    if (args.isPython && deployMode == CLIENT) {
      if (args.primaryResource == PYSPARK_SHELL) {
        ...
        args.childArgs =
          ArrayBuffer(localPrimaryResource, localPyFiles) ++ args.childArgs
      }
    }
    ...
    if (localPyFiles != null) {
      sparkConf.set(
        SUBMIT_PYTHON_FILES,
        localPyFiles.split(",").toImmutableArraySeq
      )
    }
    ...
    val options = List[OptionAssigner](
      ...
      OptionAssigner(
        ...
        ALL_CLUSTER_MGRS,
        CLUSTER,
        confKey = SUBMIT_PYTHON_FILES.key
      ),
    ...
    // reporting.
    if (
      isShell(args.primaryResource) && !sparkConf.contains(
    ...
    if (
      !isKubernetesClusterModeDriver && !isYarnCluster && !args.isPython && !args.isR
    ) {
    ...
    if (
      clusterManager == KUBERNETES && UserGroupInformation.isSecurityEnabled
    ) {
    ...
    sparkConf.set(
      SUBMIT_PYTHON_FILES,
      formattedPyFiles.split(",").toImmutableArraySeq
    )
  ...
  private def setRMPrincipal(sparkConf: SparkConf): Unit = {
    ...
    logInfo(
      log"Setting ${MDC(LogKeys.KEY, key)} to ${MDC(LogKeys.SHORT_USER_NAME, shortUserName)}"
    )
    sparkConf.set(key, shortUserName)
  }

  private def getSubmitClassLoader(
      sparkConf: SparkConf
  ...
  private def runMain(args: SparkSubmitArguments, uninitLog: Boolean): Unit = {
    ...
    try {
      ...
      case e: ClassNotFoundException =>
        logError(
          log"Failed to load class ${MDC(LogKeys.CLASS_NAME, childMainClass)}."
        )
        if (childMainClass.contains("thriftserver")) {
          logInfo(
            log"Failed to load main class ${MDC(LogKeys.CLASS_NAME, childMainClass)}."
          )
          logInfo(
      ...
      case e: NoClassDefFoundError =>
        logError(
          log"Failed to load ${MDC(LogKeys.CLASS_NAME, childMainClass)}",
          e
        )
...
object SparkSubmit extends CommandLineUtils with Logging {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    val submit = new SparkSubmit() {
      self =>
      override protected def parseArguments(
          args: Array[String]
      ...
      override protected def logInfo(entry: LogEntry): Unit = printMessage(
        entry.message
      )

      override protected def logWarning(msg: => String): Unit = printMessage(
        s"Warning: $msg"
      )
      ...
      override protected def logError(msg: => String): Unit = printMessage(
        s"Error: $msg"
      )
  ...
  private[deploy] def isR(res: String): Boolean = {
    res != null && (res.endsWith(".R") || res.endsWith(
      ".r"
    )) || res == SPARKR_SHELL
  }
...
private[spark] object SparkSubmitUtils {
  private[deploy] def getSubmitOperations(
      master: String
