...
private[deploy] object JsonProtocol {
  ...
  private def writeResourcesInfo(
      info: Map[String, ResourceInformation]
  ): JObject = {
    val jsonFields = info.map { case (k, v) =>
      JField(k, v.toJson())
    }
  ...
  def writeApplicationInfo(obj: ApplicationInfo): JObject = {
    ("id" -> obj.id) ~
      ...
      ("resourcesperexecutor" -> obj.desc.resourceReqsPerExecutor.toList
        .map(writeResourceRequirement)) ~
      ("resourcesperslave" -> obj.desc.resourceReqsPerExecutor.toList
        .map(writeResourceRequirement)) ~
  ...
  def writeApplicationDescription(obj: ApplicationDescription): JObject = {
    ("name" -> obj.name) ~
      ...
      ("resourcesperexecutor" -> obj.resourceReqsPerExecutor.toList.map(
        writeResourceRequirement
      )) ~
      ("memoryperslave" -> obj.memoryPerExecutorMB) ~
      ("resourcesperslave" -> obj.resourceReqsPerExecutor.toList.map(
        writeResourceRequirement
      )) ~
      ("user" -> obj.user) ~
    ...
    field match {
      case None =>
        ("url" -> obj.uri) ~
          ...
          ("resources" -> aliveWorkers
            .map(_.resourcesInfo)
            .toList
            .map(writeResourcesInfo)) ~
          ("resourcesused" ->
            aliveWorkers
              .map(_.resourcesInfoUsed)
              .toList
              .map(writeResourcesInfo)) ~
  ...
  def writeClusterUtilization(obj: MasterStateResponse): JObject = {
    ...
    ("waitingDrivers" -> obj.activeDrivers.count(
      _.state == DriverState.SUBMITTED
    )) ~
      ("cores" -> cores) ~
      ("coresused" -> coresUsed) ~
      ("coresutilization" -> (if (cores == 0) 100
                              else 100 * coresUsed / cores)) ~
      ("memory" -> memory) ~
      ("memoryused" -> memoryUsed) ~
      ("memoryutilization" -> (if (memory == 0) 100
                               else 100 * memoryUsed / memory))
  }
}
