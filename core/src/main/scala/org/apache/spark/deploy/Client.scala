...
private class ClientEndpoint(
  ...
  private val waitAppCompletion =
    conf.get(config.STANDALONE_SUBMIT_WAIT_APP_COMPLETION)
    ...
    driverArgs.cmd match {
      case "launch" =>
        ...
        val libraryPathEntries =
          getProperty(libraryPathConf, conf).toSeq.flatMap { cp =>
            cp.split(java.io.File.pathSeparator)
          }
        ...
        val driverResourceReqs = ResourceUtils.parseResourceRequirements(
          conf,
      ...
      case "kill" =>
        ...
        asyncSendToMasterAndForwardReply[KillDriverResponse](
  ...
  /** Send the message to master and forward the reply to self asynchronously.
    */
  private def asyncSendToMasterAndForwardReply[T: ClassTag](
      message: Any
  ): Unit = {
    for (masterEndpoint <- masterEndpoints) {
      masterEndpoint
        ...
        .onComplete {
          ...
          case Failure(e) =>
            logWarning(
              log"Error sending messages to master " +
                log"${MDC(MASTER_URL, masterEndpoint)}",
              e
            )
  ...
  def reportDriverStatus(
    ...
    if (found) {
      ...
      if (!driverStatusReported) {
        ...
        logInfo(
          log"State of ${MDC(DRIVER_ID, submittedDriverID)}" +
        ...
        (workerId, workerHostPort, state) match {
          case (Some(id), Some(hostPort), Some(DriverState.RUNNING)) =>
            logInfo(
              log"Driver running on ${MDC(HOST, hostPort)} (${MDC(WORKER_ID, id)})"
            )
          case _ =>
        }
      }
      ...
      exception match {
        ...
        case _ =>
          state.get match {
            case DriverState.FINISHED | DriverState.FAILED | DriverState.ERROR |
                DriverState.KILLED =>
              logInfo(
                log"State of driver ${MDC(DRIVER_ID, submittedDriverID)}" +
            ...
            case _ =>
              if (!waitAppCompletion) {
                logInfo(
                  "spark-submit not configured to wait for completion, " +
      ...
      logDebug(
        s"The status response is reported from a backup spark instance. So, ignored."
      )
    } else {
      logError(
        log"ERROR: Cluster master did not recognize ${MDC(DRIVER_ID, submittedDriverID)}"
      )
      System.exit(-1)
    }
  }
  ...
  override def onDisconnected(remoteAddress: RpcAddress): Unit = {
    if (!lostMasters.contains(remoteAddress)) {
      logError(
        log"Error connecting to master ${MDC(HOST_PORT, remoteAddress)}."
      )
  ...
  override def start(args: Array[String], conf: SparkConf): Unit = {
    ...
    rpcEnv.setupEndpoint(
      ...
      new ClientEndpoint(
        rpcEnv,
        driverArgs,
        masterEndpoints.toImmutableArraySeq,
        conf
      )
    )
