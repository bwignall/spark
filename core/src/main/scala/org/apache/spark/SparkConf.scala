...
class SparkConf(loadDefaults: Boolean)
  ...
  private[spark] def loadFromSystemProperties(silent: Boolean): SparkConf = {
    ...
    for (
      (key, value) <- Utils.getSystemProperties if key.startsWith("spark.")
    ) {
  ...
  private[spark] def setIfMissing[T](
    ...
    if (
      settings.putIfAbsent(entry.key, entry.rawStringConverter(value)) == null
    ) {
      logDeprecationWarning(entry.key)
    }
    this
  ...
  def registerKryoClasses(classes: Array[Class[_]]): SparkConf = {
    ...
    allClassNames ++= get(KRYO_CLASSES_TO_REGISTER)
      .map(_.trim)
      .filter(!_.isEmpty)
  ...
  private[spark] def contains(entry: ConfigEntry[_]): Boolean = contains(
    entry.key
  )
    ...
    try {
      ...
      case e: NumberFormatException =>
        ...
        throw new NumberFormatException(
          s"Illegal value for config key $key: ${e.getMessage}"
        )
          .initCause(e)
  ...
  private[spark] def validateSettings(): Unit = {
    ...
    sys.props.get("spark.driver.libraryPath").foreach { value =>
      val warning =
        log"""
          |spark.driver.libraryPath was detected (set to '${MDC(
          LogKeys.CONFIG,
          value
        )}').
    ...
    Seq(EXECUTOR_JAVA_OPTIONS.key, "spark.executor.defaultJavaOptions")
      .foreach { executorOptsKey =>
        getOption(executorOptsKey).foreach { javaOpts =>
          ...
          if (javaOpts.contains("-Xmx")) {
            val msg =
              s"$executorOptsKey is not allowed to specify max heap memory settings " +
      ...
      get(SUBMIT_DEPLOY_MODE) match {
        ...
        case e =>
          throw new SparkException(
            s"${SUBMIT_DEPLOY_MODE.key} can only be " +
              "\"cluster\" or \"client\"."
          )
      }
    }
    ...
    val encryptionEnabled =
      get(NETWORK_CRYPTO_ENABLED) || get(SASL_ENCRYPTION_ENABLED)
    require(
...
private[spark] object SparkConf extends Logging {
  ...
  private val configsWithAlternatives = Map[String, Seq[AlternateConfig]](
    ...
    KEYTAB.key -> Seq(AlternateConfig("spark.yarn.keytab", "3.0")),
    PRINCIPAL.key -> Seq(AlternateConfig("spark.yarn.principal", "3.0")),
