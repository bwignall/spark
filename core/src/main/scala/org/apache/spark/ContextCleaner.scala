...
private case class CleanSparkListener(listener: SparkListener)
    extends CleanupTask
...
private[spark] class ContextCleaner(
  ...
  def doCleanupRDD(rddId: Int, blocking: Boolean): Unit = {
    try {
      ...
      case e: Exception =>
        logError(log"Error cleaning RDD ${MDC(RDD_ID, rddId)}", e)
    }
  }
  ...
  def doCleanupShuffle(shuffleId: Int, blocking: Boolean): Unit = {
    try {
      if (mapOutputTrackerMaster.containsShuffle(shuffleId)) {
        ...
        logDebug(
          "Asked to cleanup non-existent shuffle (maybe it was already removed)"
        )
      }
    } catch {
      case e: Exception =>
        logError(log"Error cleaning shuffle ${MDC(SHUFFLE_ID, shuffleId)}", e)
    }
  }
  ...
  def doCleanupBroadcast(broadcastId: Long, blocking: Boolean): Unit = {
    try {
      ...
      case e: Exception =>
        logError(
          log"Error cleaning broadcast ${MDC(BROADCAST_ID, broadcastId)}",
          e
        )
    }
  }
  ...
  def doCleanupAccum(accId: Long, blocking: Boolean): Unit = {
    try {
      ...
      case e: Exception =>
        logError(
          log"Error cleaning accumulator ${MDC(ACCUMULATOR_ID, accId)}",
          e
        )
    }
  }
  ...
  def doCleanCheckpoint(rddId: Int): Unit = {
    try {
      ...
      case e: Exception =>
        logError(
          log"Error cleaning rdd checkpoint data ${MDC(RDD_ID, rddId)}",
          e
        )
    }
  }
  ...
  def doCleanSparkListener(listener: SparkListener): Unit = {
    try {
      ...
      case e: Exception =>
        logError(
          log"Error cleaning Spark listener ${MDC(LISTENER, listener)}",
          e
        )
    }
  }
